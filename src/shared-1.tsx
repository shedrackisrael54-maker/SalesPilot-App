...
// Same optimization, but scales the whole image down to fit rather than cropping it to fill a
// fixed shape. Used anywhere a product photo must never lose part of the subject - c_fill will
// happily cut a person's head off to make a square, which looks broken to a real customer
// deciding whether to buy. c_fit keeps every pixel of the original in frame.
export function getFittedImageUrl(url: string | undefined, width: number): string {
  if (!url) return '';
  if (!url.includes('res.cloudinary.com') || !url.includes('/upload/')) return url;
  return url.replace('/upload/', `/upload/w_${width},q_auto,f_auto,c_fit/`);
}
