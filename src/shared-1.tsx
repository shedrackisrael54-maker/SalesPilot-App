...
export function getOptimizedImageUrl(url: string | undefined, width: number): string {
  ...
  // g_auto tells Cloudinary to find the subject of the photo and crop around it, instead of
  // blindly cutting from the centre - which is what chops the head off a portrait product shot
  // when it gets squeezed into a square card.
  return url.replace('/upload/', `/upload/w_${width},q_auto,f_auto,c_fill,g_auto/`);
}
