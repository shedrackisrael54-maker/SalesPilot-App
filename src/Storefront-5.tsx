...
import { NAIRA, THEME_NICHES, ProductIcon, isValidCSSColor, isColorOptionGroup, getTieredPrice, NIGERIAN_STATES, getOptimizedImageUrl, PLATFORM_LOGO_URL, copyToClipboard } from './shared';
import type { StoreTheme, Product, Review, Order, OrderItem, Bundle, Courier, CheckoutField, WebsiteContent } from './shared';
...
// The free trial length, mirroring TRIAL_DAYS in App.tsx. Kept as a separate copy because
// Storefront.tsx and App.tsx share only types, not runtime code - if one changes, change both.
const TRIAL_DAYS = 7;
...
// they're still within their free trial, OR if they've actually paid for Pro and that
...
// subscription covering the gap - mirrors the same check in App.tsx. Used to take a store
// offline for new purchases once its owner's trial has expired without payment - a customer
// who already placed an order can still track it, this only blocks NEW browsing and buying.
function isTrialExpiredUnpaid(merchant: MerchantInfo | null): boolean {
  if (!merchant?.trialStart) return false;
  const elapsedDays = Math.floor((Date.now() - new Date(merchant.trialStart).getTime()) / (1000 * 60 * 60 * 24));
  if (elapsedDays < TRIAL_DAYS) return false;
  const hasActiveSubscription = merchant.subscriptionExpiresAt && new Date(merchant.subscriptionExpiresAt).getTime() > Date.now();
...
// WhatsApp's own links (wa.me) require the FULL international number, with no leading 0 - a
// merchant typing their number the normal Nigerian way (e.g. 07055863609) would otherwise send
// customers to a broken "number not found" page. This converts that into the format WhatsApp
// actually needs (2347055863609), without requiring merchants to think about country codes at
// all when they type their own number in.
...
async function sendStoreEmail(to: string, subject: string, html: string) {
  try {
    ...
    // A successful fetch only means the request reached the server - it says nothing about
    // whether the server actually managed to send the email. Without checking the response
    // itself, a real failure (wrong API key, Resend rejecting the request, anything) would
    // report nothing at all, anywhere - exactly indistinguishable from an email that quietly
    // never got attempted.
    if (!res.ok) {
      const body = await res.text().catch(() => '');
      console.error(`Email to ${to} failed - status ${res.status}: ${body}`);
    }
  } catch (err) {
...
// Shown to customers instead of the real storefront once the merchant's trial has genuinely
// run out with no payment - an honest, temporary message rather than pretending the store
// doesn't exist at all, since it's a real business that just needs its owner to re-subscribe.
function StoreUnavailable() {
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' as const, alignItems: 'center', justifyContent: 'center', padding: 24, fontFamily: 'Inter, sans-serif', textAlign: 'center' as const }}>
      <Clock size={40} color="#9CA3AF" style={{ marginBottom: 16 }} />
      <h2 style={{ fontSize: 20, fontWeight: 700, color: '#111827', marginBottom: 8 }}>Store temporarily unavailable</h2>
      <p style={{ fontSize: 14, color: '#6B7280', maxWidth: 280 }}>This store isn't accepting new orders right now. Please check back later.</p>
    </div>
  );
}

// A slide-out menu for the storefront home. Everything here goes somewhere real - the shop and
// category links scroll to sections already on the page, About only appears when the merchant
// has actually written it, and Track Order is a real route. A menu item that opens an empty
// page is worse than no menu item at all.
// The storefront hero. Runs edge to edge with no rounded corners so it reads as the top of a
// real shopfront rather than a card floating on a page, and quietly cycles through the
// merchant's own products behind the store name - which does two things a static banner can't:
// it shows a visitor what's actually sold here within seconds of landing, and it makes a store
// with only a handful of products still feel alive. Tapping opens whichever product is showing.
function StoreHero({ merchant, theme, products, slug }: { merchant: MerchantInfo; theme: StoreTheme; products: Product[]; slug: string }) {
  const showcase = products.filter(p => p.imageUrl || (p.images && p.images.length > 0)).slice(0, 8);
  const [index, setIndex] = useState(0);

  useEffect(() => {
    if (showcase.length < 2) return;
    const timer = setInterval(() => setIndex(i => (i + 1) % showcase.length), 4200);
    return () => clearInterval(timer);
  }, [showcase.length]);

  const active = showcase[index];
  const tagline = merchant.websiteContent?.tagline?.trim() || merchant.storeDescription;

  return (
    <div style={{ position: 'relative' as const, width: '100%', minHeight: 260, overflow: 'hidden', background: merchant.coverImageUrl ? '#111' : `linear-gradient(120deg, ${theme.primary}, ${theme.secondary})` }}>
      {/* Product images cross-fade behind everything. Each is absolutely positioned so they
          share the same space and only opacity changes - no layout shift, no jumping. */}
      {showcase.map((p, i) => {
        const img = p.images?.[0] || p.imageUrl;
        return (
          <img
            key={p.id}
            src={getOptimizedImageUrl(img, 900, 1.4)}
            alt={p.name}
      ...
      {showcase.length === 0 && merchant.coverImageUrl && (
        <img src={getOptimizedImageUrl(merchant.coverImageUrl, 900)} alt={merchant.storeName} style={{ position: 'absolute' as const, inset: 0, width: '100%', height: '100%', objectFit: 'cover' as const }} />
      )}

      <div style={{ position: 'absolute' as const, inset: 0, background: 'linear-gradient(to top, rgba(0,0,0,0.78) 0%, rgba(0,0,0,0.35) 45%, rgba(0,0,0,0.18) 100%)' }} />

      <div style={{ position: 'relative' as const, zIndex: 1, minHeight: 260, display: 'flex', flexDirection: 'column' as const, justifyContent: 'flex-end', padding: '20px 16px 18px' }}>
        <p style={{ color: 'rgba(255,255,255,0.75)', fontSize: 10, fontWeight: 800, letterSpacing: 1.5, textTransform: 'uppercase' as const, marginBottom: 6 }}>Welcome to</p>
        <h2 style={{ fontFamily: "'Playfair Display', serif", color: '#fff', fontSize: 26, fontWeight: 800, marginBottom: tagline ? 5 : 12, lineHeight: 1.1 }}>{merchant.storeName || 'Our Store'}</h2>
        {tagline && (
          <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: 12.5, lineHeight: 1.45, maxWidth: 300, marginBottom: 14, display: '-webkit-box' as const, WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' as const, overflow: 'hidden' }}>{tagline}</p>
        )}

        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' as const }}>
          <a href="#sp-products" className="sp-btn" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: theme.accent, color: theme.secondary, borderRadius: 12, padding: '12px 20px', fontWeight: 800, fontSize: 13, textDecoration: 'none', boxShadow: '0 8px 18px rgba(0,0,0,0.25)', transition: 'transform 0.15s ease' }}>
            Shop Now <ArrowRight size={14} />
          </a>
          {active && (
            <Link
              to={`/${slug}/product/${active.id}`}
              className="sp-btn"
              style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.16)', backdropFilter: 'blur(6px)', color: '#fff', borderRadius: 12, padding: '10px 14px', textDecoration: 'none', minWidth: 0, transition: 'transform 0.15s ease' }}
            >
              <span style={{ fontSize: 12.5, fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis' as const, whiteSpace: 'nowrap' as const, maxWidth: 130 }}>{active.name}</span>
              <span style={{ fontSize: 12.5, fontWeight: 800, color: theme.accent, flexShrink: 0 }}>{NAIRA}{(active.salePrice || active.price).toLocaleString()}</span>
            </Link>
          )}
        </div>
        ...
        {showcase.length > 1 && (
          <div style={{ display: 'flex', gap: 5, marginTop: 14 }}>
            {showcase.map((_, i) => (
              <div key={i} style={{ width: i === index ? 18 : 6, height: 4, borderRadius: 2, background: i === index ? theme.accent : 'rgba(255,255,255,0.4)', transition: 'width 0.35s ease, background 0.35s ease' }} />
            ))}
          </div>
        )}
      </div>
...
function StoreMenu({ open, onClose, theme, merchant, slug, onJump }: { open: boolean; onClose: () => void; theme: StoreTheme; merchant: MerchantInfo; slug: string; onJump: (target: string) => void }) {
  if (!open) return null;
  const hasAbout = !!merchant.websiteContent?.aboutText?.trim();
  const items: { label: string; action: () => void }[] = [
    { label: 'Shop', action: () => onJump('sp-products') },
    { label: 'Categories', action: () => onJump('sp-categories') },
    { label: 'New Arrivals', action: () => onJump('sp-new') },
    ...(hasAbout ? [{ label: 'About Us', action: () => onJump('sp-about') }] : []),
    { label: 'Contact', action: () => onJump('sp-contact') },
  ];

  return (
    <div onClick={onClose} style={{ position: 'fixed' as const, inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 200, display: 'flex' }}>
      <div
        onClick={e => e.stopPropagation()}
        style={{
          background: theme.secondary, width: 'min(78vw, 300px)', height: '100%',
          display: 'flex', flexDirection: 'column' as const, padding: '20px 0',
          animation: 'sp-drawer-in 0.28s cubic-bezier(0.22,1,0.36,1) both', overflowY: 'auto' as const,
        }}
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px 20px' }}>
          <span style={{ fontFamily: "'Playfair Display', serif", fontSize: 17, fontWeight: 800, color: theme.accent, overflow: 'hidden', textOverflow: 'ellipsis' as const, whiteSpace: 'nowrap' as const }}>
            {merchant.storeName || 'Menu'}
          </span>
          <button onClick={onClose} style={{ background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: 9, width: 30, height: 30, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }}>
            <X size={15} color="#fff" />
          </button>
        </div>
        ...
        {items.map(it => (
          <button
            ...
            style={{ background: 'none', border: 'none', textAlign: 'left' as const, padding: '15px 20px', fontSize: 15.5, fontWeight: 600, color: '#fff', cursor: 'pointer', borderBottom: '1px solid rgba(255,255,255,0.08)' }}
          >
            {it.label}
          </button>
        ))}
        ...
        <Link
          to={`/${slug}/track`}
          onClick={onClose}
          style={{ padding: '15px 20px', fontSize: 15.5, fontWeight: 600, color: '#fff', textDecoration: 'none', borderBottom: '1px solid rgba(255,255,255,0.08)' }}
        >
          Track My Order
        </Link>
        ...
        {merchant.contactWhatsapp && (
          <a
            href={`https://wa.me/${toWhatsAppNumber(merchant.contactWhatsapp)}?text=${encodeURIComponent('Hi, I have a question about your store')}`}
            target="_blank"
            rel="noreferrer"
            onClick={onClose}
            style={{ display: 'flex', alignItems: 'center', gap: 9, margin: '20px 20px 0', background: theme.accent, color: theme.secondary, borderRadius: 12, padding: '13px 16px', fontSize: 14, fontWeight: 800, textDecoration: 'none' }}
          >
            <MessageCircle size={16} /> Message us
          </a>
        )}
      </div>
      <style>{`@keyframes sp-drawer-in { from { transform: translateX(-100%); } to { transform: translateX(0); } }`}</style>
    </div>
  );
}

function DarkHeader({ theme, merchant, slug, backHref, showBack, onWishlistClick, wishlistActive, onSearchClick, searchActive, onMenuClick }: { theme: StoreTheme; merchant: MerchantInfo; slug: string; backHref?: string; showBack?: boolean; onWishlistClick?: () => void; wishlistActive?: boolean; onSearchClick?: () => void; searchActive?: boolean; onMenuClick?: () => void }) {
    ...
    <div style={{ position: 'sticky' as const, top: 0, zIndex: 30, background: theme.secondary, transition: 'background 0.5s ease' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px 10px', gap: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 9, minWidth: 0 }}>
          ...
          {onMenuClick && (
            <button onClick={onMenuClick} aria-label="Open menu" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 32, height: 32, borderRadius: 9, background: 'rgba(255,255,255,0.1)', border: 'none', cursor: 'pointer', flexShrink: 0, padding: 0 }}>
              <Menu size={17} color="#fff" />
            </button>
          )}
          {(() => {
            // White-label clients paid for a fully branded site, so their own logo always wins -
            // showing SalesPilot's logo on a store someone paid to make entirely theirs would be
            // a real broken promise. Free/basic merchants still fall back to the platform logo.
            const displayLogo = (merchant.whiteLabel || isProAccess(merchant)) ? merchant.logoUrl : (PLATFORM_LOGO_URL || merchant.logoUrl);
            return displayLogo ? (
...
function FloatingWhatsApp({ merchant, theme, productName }: { merchant: MerchantInfo; theme: StoreTheme; productName?: string }) {
  ...
  return (
    <a
      href={`https://wa.me/${toWhatsAppNumber(merchant.contactWhatsapp)}?text=${encodeURIComponent(text)}`}
      target="_blank"
      rel="noreferrer"
      style={{
...
function StoreFooter({ merchant, theme, slug }: { merchant: MerchantInfo; theme: StoreTheme; slug: string }) {
  return (
    <div id="sp-contact" style={{ background: theme.secondary, padding: '28px 20px 24px', transition: 'background 0.5s ease' }}>
      ...
      {/* Location sits first and is tappable. For a customer deciding whether to trust a store
          they've never bought from, a real, findable address does more than any phone number -
          it's the difference between a business and someone selling from a phone. */}
      {merchant.storeAddress && (
        <a
          href={merchant.websiteContent?.mapsLink?.trim() || `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(merchant.storeAddress)}`}
          target="_blank"
          rel="noreferrer"
          style={{ display: 'flex', alignItems: 'flex-start', gap: 9, textDecoration: 'none', background: 'rgba(255,255,255,0.07)', borderRadius: 12, padding: '13px 14px', marginBottom: 14 }}
        >
          <MapPin size={16} color={theme.accent} style={{ marginTop: 1, flexShrink: 0 }} />
          <div style={{ minWidth: 0 }}>
            <p style={{ color: '#fff', fontSize: 13.5, fontWeight: 700, lineHeight: 1.5, marginBottom: 3 }}>{merchant.storeAddress}</p>
            <p style={{ color: theme.accent, fontSize: 11.5, fontWeight: 700 }}>Tap for directions</p>
          </div>
        </a>
      )}
      ...
      {merchant.websiteContent?.openingHours?.trim() && (
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 9, color: 'rgba(255,255,255,0.85)', fontSize: 13, lineHeight: 1.65, marginBottom: 4 }}>
          <Clock size={14} color={theme.accent} style={{ marginTop: 3, flexShrink: 0 }} />
          <span style={{ whiteSpace: 'pre-wrap' as const }}>{merchant.websiteContent.openingHours}</span>
        </div>
      )}

      <div style={{ borderTop: '1px solid rgba(255,255,255,0.12)', marginTop: 20, paddingTop: 16, textAlign: 'center' as const }}>
        {merchant.whiteLabel ? (
          <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 10.5, fontWeight: 700, letterSpacing: 0.5 }}>
            &copy; {new Date().getFullYear()} {(merchant.storeName || '').toUpperCase()}
          </p>
        ) : (
...
// --- Full business-website sections -------------------------------------------------------
// These sit above the product grid and turn a plain catalogue into something that reads like a
// real company website: who this business is, what they offer, why to trust them, and - most
// importantly for a physical shop - exactly where to find them. Every section is written by the
// merchant and every one is optional. A section with no content simply doesn't render, because
// an empty "About Us" heading looks far worse to a customer than no About section at all.

function SectionHeading({ label, title, theme }: { label?: string; title: string; theme: StoreTheme }) {
  return (
    <div style={{ marginBottom: 18 }}>
      {label && (
        <p style={{ fontSize: 11, fontWeight: 800, letterSpacing: 1.3, textTransform: 'uppercase' as const, color: theme.primary, marginBottom: 7 }}>{label}</p>
      )}
      <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 22, fontWeight: 800, color: '#111827', lineHeight: 1.25 }}>{title}</h2>
    </div>
  );
}

function AboutSection({ merchant, theme }: { merchant: MerchantInfo; theme: StoreTheme }) {
  const c = merchant.websiteContent;
  if (!c?.aboutText?.trim()) return null;
  const img = c.aboutImage?.trim();
  return (
    <div id="sp-about" style={{ padding: '38px 20px', background: '#fff' }}>
      <SectionHeading label="About us" title={c.aboutTitle?.trim() || `Welcome to ${merchant.storeName || 'our store'}`} theme={theme} />
      {img && (
        // A real photo of the actual shop does more for trust than any paragraph - it tells a
        // customer this is a genuine place they could walk into.
        <div style={{ position: 'relative' as const, marginBottom: 18, borderRadius: 18, overflow: 'hidden' }}>
          <img src={getOptimizedImageUrl(img, 800)} alt={merchant.storeName} style={{ width: '100%', aspectRatio: '16/10', objectFit: 'cover' as const, display: 'block' }} loading="lazy" />
          <div style={{ position: 'absolute' as const, inset: 0, border: `1px solid rgba(0,0,0,0.06)`, borderRadius: 18, pointerEvents: 'none' as const }} />
        </div>
      )}
      <p style={{ fontSize: 14.5, color: '#4B5563', lineHeight: 1.75, whiteSpace: 'pre-wrap' as const }}>{c.aboutText}</p>
    </div>
  );
}

function OffersSection({ merchant, theme }: { merchant: MerchantInfo; theme: StoreTheme }) {
  const items = (merchant.websiteContent?.offers || []).filter(o => o.title?.trim());
  if (items.length === 0) return null;
  return (
    <div style={{ padding: '38px 20px', background: `${theme.primary}08` }}>
      <SectionHeading label="What we offer" title={merchant.websiteContent?.offerTitle?.trim() || 'What we do'} theme={theme} />
        ...
        {items.map((o, i) => {
          ...
          return (
            <div key={i} style={{ background: '#fff', borderRadius: 16, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
              {img ? (
                <div style={{ position: 'relative' as const }}>
                  <img src={getOptimizedImageUrl(img, 700)} alt={o.title} style={{ width: '100%', aspectRatio: '16/9', objectFit: 'cover' as const, display: 'block' }} loading="lazy" />
                  <div style={{ position: 'absolute' as const, inset: 0, background: 'linear-gradient(to top, rgba(0,0,0,0.72), transparent 62%)' }} />
                  <p style={{ position: 'absolute' as const, bottom: 13, left: 15, right: 15, color: '#fff', fontSize: 16.5, fontWeight: 800, fontFamily: "'Playfair Display', serif" }}>{o.title}</p>
                </div>
              ) : (
                <div style={{ padding: '15px 16px 0' }}>
                  <p style={{ fontSize: 15.5, fontWeight: 700, color: '#111827' }}>{o.title}</p>
                </div>
              )}
              {o.text?.trim() && (
                <p style={{ fontSize: 13.5, color: '#6B7280', lineHeight: 1.65, padding: img ? '14px 16px 16px' : '7px 16px 16px' }}>{o.text}</p>
              )}
            </div>
          );
        })}
...
// A full-width photo band that breaks the page up visually between the text-heavy sections and
// the shopping content. Optional headline sits over it - useful for something aspirational the
// business wants to say without it needing to be a whole section of its own.
function PhotoBand({ merchant }: { merchant: MerchantInfo }) {
  const img = merchant.websiteContent?.bandImage?.trim();
  if (!img) return null;
  const text = merchant.websiteContent?.bandText?.trim();
  return (
    <div style={{ position: 'relative' as const, width: '100%' }}>
      <img src={getOptimizedImageUrl(img, 1000)} alt={merchant.storeName} style={{ width: '100%', aspectRatio: '21/9', objectFit: 'cover' as const, display: 'block' }} loading="lazy" />
      {text && (
        <>
          <div style={{ position: 'absolute' as const, inset: 0, background: 'linear-gradient(to top, rgba(0,0,0,0.6), rgba(0,0,0,0.15))' }} />
          <p style={{ position: 'absolute' as const, bottom: 0, left: 0, right: 0, padding: '0 24px 22px', color: '#fff', fontFamily: "'Playfair Display', serif", fontSize: 20, fontWeight: 800, lineHeight: 1.3, textAlign: 'center' as const }}>{text}</p>
        </>
      )}
    </div>
  );
...
function WhySection({ merchant, theme }: { merchant: MerchantInfo; theme: StoreTheme }) {
  const points = (merchant.websiteContent?.whyPoints || []).filter(p => p.title?.trim());
  if (points.length === 0) return null;
  return (
    <div style={{ padding: '38px 20px', background: '#fff' }}>
      <SectionHeading label="Why choose us" title={merchant.websiteContent?.whyTitle?.trim() || 'Why customers keep coming back'} theme={theme} />
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 12 }}>
        {points.map((p, i) => (
          <div key={i} style={{ background: `${theme.primary}0a`, borderRadius: 15, padding: '18px 16px' }}>
            <div style={{ width: 36, height: 36, borderRadius: 11, background: theme.primary, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 12 }}>
              <CheckCircle2 size={19} color="#fff" />
            </div>
            <p style={{ fontSize: 14.5, fontWeight: 800, color: '#111827', marginBottom: p.text?.trim() ? 6 : 0, lineHeight: 1.3 }}>{p.title}</p>
            {p.text?.trim() && <p style={{ fontSize: 13, color: '#6B7280', lineHeight: 1.6 }}>{p.text}</p>}
          </div>
        ))}
      </div>
    </div>
...
// Real reviews, pulled across every product rather than shown one product at a time, so a
// customer deciding whether to trust this shop sees the whole picture in one place.
function StoreReviewsSection({ merchantUid, theme }: { merchantUid: string; theme: StoreTheme }) {
  const [reviews, setReviews] = useState<Review[]>([]);

  useEffect(() => {
    if (!merchantUid) return;
    const unsub = onSnapshot(collection(db, 'merchants', merchantUid, 'reviews'), (snap) => {
  ...
  return (
    <div style={{ padding: '34px 20px', background: '#fff' }}>
      <SectionHeading label="Reviews" title="What our customers say" theme={theme} />
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 18 }}>
        <div style={{ display: 'flex', gap: 2 }}>
          {[1, 2, 3, 4, 5].map(i => <Star key={i} size={16} color={i <= Math.round(avg) ? '#F59E0B' : '#D1D5DB'} fill={i <= Math.round(avg) ? '#F59E0B' : 'none'} />)}
        </div>
        <span style={{ fontSize: 13, color: '#6B7280', fontWeight: 600 }}>{avg.toFixed(1)} from {reviews.length} review{reviews.length !== 1 ? 's' : ''}</span>
      </div>
      <div style={{ display: 'flex', gap: 12, overflowX: 'auto' as const, paddingBottom: 4 }}>
        {reviews.map(r => (
          <div key={r.id} style={{ flexShrink: 0, width: 232, background: `${theme.primary}08`, borderRadius: 14, padding: 16 }}>
            ...
            {r.comment?.trim() && <p style={{ fontSize: 13, color: '#374151', lineHeight: 1.6, marginBottom: 10 }}>{r.comment}</p>}
            <p style={{ fontSize: 12.5, fontWeight: 700, color: '#111827' }}>{r.customerName}</p>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <Link to={`/${slug}/product/${product.id}`} className="sp-card" style={{ textDecoration: 'none', display: 'block', width: '100%', height: '100%', minWidth: 0 }}>
      <div style={{ background: theme.background, borderRadius: 20, overflow: 'hidden', border: `1px solid ${theme.accent}44`, boxShadow: '0 2px 8px rgba(0,0,0,0.05)', transition: 'transform 0.15s ease', display: 'flex', flexDirection: 'column' as const, height: '100%', minWidth: 0 }}>
        <div style={{ width: '100%', aspectRatio: '3/4', background: `linear-gradient(135deg, ${theme.primary}12, ${theme.secondary}0a)`, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' as const, overflow: 'hidden', flexShrink: 0 }}>
          {product.imageUrl ? (
            <img src={getOptimizedImageUrl(product.imageUrl, 420, 0.75)} alt={product.name} style={{ width: '100%', height: '100%', objectFit: 'cover' as const, display: 'block' }} loading="lazy" />
          ) : (
...
export function CheckoutScreen() {
  ...
  const [paymentSentConfirmed, setPaymentSentConfirmed] = useState(false);
  ...
  if (isTrialExpiredUnpaid(merchant)) return <StoreUnavailable />;
  ...
  const persistOrder = async (paymentMethod: 'Paystack' | 'WhatsApp' | 'Bank Transfer', reference: string) => {
    ...
    // Also send a real phone notification, if this merchant has enabled it on at least one
    // device - fails silently if not set up yet, the in-app bell notification above still works
    // either way.
    if (merchant.fcmTokens && merchant.fcmTokens.length > 0) {
      fetch('https://sales-pilot-payment.vercel.app/api/send-push-notification', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          tokens: merchant.fcmTokens,
          title: 'New order!',
          body: `${order.customerName || 'A customer'} just paid ${NAIRA}${grandTotal.toLocaleString()}`,
        }),
      }).catch(() => {});
    }
    try {
  ...
  const handlePlaceOrder = async () => {
    ...
    if (!merchant.accountNumber || !merchant.bankName) {
      setFormError('This store has not set up a payout account yet. Please contact the seller directly to complete your order.');
      return;
    }
    if (!paymentSentConfirmed) {
      setFormError('Please confirm you have sent the payment before continuing.');
      return;
    }
    ...
    setPlacing(true);
    try {
      await persistOrder('Bank Transfer', reference);
    } catch (err: any) {
      setFormError('Could not save your order right now. Please check your connection and try again.');
    } finally {
      setPlacing(false);
    }
  };
  ...
  if (completedOrder) {
    const waText = `Hi, I just placed an order (Ref: ${completedOrder.reference}) for ${NAIRA}${completedOrder.total.toLocaleString()}${completedOrder.shippingFee > 0 ? ` (includes ${NAIRA}${completedOrder.shippingFee.toLocaleString()} delivery to ${completedOrder.deliveryState})` : ''}. I've sent payment by bank transfer, here's my order. My order link: ${window.location.origin}/${slug}/order/${completedOrder.reference}/${completedOrder.accessToken}`;
    return (
      <div style={{ minHeight: '100vh', background: theme.background, fontFamily: 'Inter, sans-serif', display: 'flex', flexDirection: 'column' as const }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column' as const, alignItems: 'center', justifyContent: 'center', padding: 32, textAlign: 'center' as const }}>
          ...
          <p style={{ fontSize: 13.5, color: '#6B7280', marginBottom: 6, maxWidth: 280 }}>
            We've noted your order and that you've sent payment. The seller will confirm once
            it's received in their account and get your order moving.
          </p>
        ...
        {cart.length === 0 ? (
          ...
          <>
            ...
            {merchant.accountNumber && merchant.bankName ? (
              <div style={{ marginBottom: 20 }}>
                <p style={{ fontSize: 14, fontWeight: 700, color: '#111827', marginBottom: 4 }}>Pay by bank transfer</p>
                <p style={{ fontSize: 11.5, color: '#9CA3AF', marginBottom: 12 }}>Send {NAIRA}{grandTotal.toLocaleString()} to this account, then confirm below.</p>
                <div style={{ background: '#F9FAFB', border: '1px solid #E5E7EB', borderRadius: 14, padding: 16, marginBottom: 14 }}>
                  <div style={{ marginBottom: 10 }}>
                    <p style={{ fontSize: 11, color: '#9CA3AF' }}>Bank</p>
                    <p style={{ fontSize: 14, fontWeight: 700, color: '#111827' }}>{merchant.bankName}</p>
                  </div>
                  <div style={{ marginBottom: 10 }}>
                    <p style={{ fontSize: 11, color: '#9CA3AF' }}>Account number</p>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <p style={{ fontSize: 18, fontWeight: 800, color: '#111827', letterSpacing: 1 }}>{merchant.accountNumber}</p>
                      <button
                        onClick={() => { copyToClipboard(merchant.accountNumber!); setCopiedAccount(true); setTimeout(() => setCopiedAccount(false), 2000); }}
                        style={{ background: theme.primary, color: '#fff', border: 'none', borderRadius: 8, padding: '6px 12px', fontSize: 11.5, fontWeight: 700, cursor: 'pointer' }}
                      >
                  ...
                  <div>
                    <p style={{ fontSize: 11, color: '#9CA3AF' }}>Account name</p>
                    <p style={{ fontSize: 13.5, fontWeight: 700, color: '#111827' }}>{merchant.accountName}</p>
                  </div>
                </div>
                <label style={{ display: 'flex', alignItems: 'flex-start', gap: 8, fontSize: 12.5, color: '#374151', cursor: 'pointer' }}>
                  <input type="checkbox" checked={paymentSentConfirmed} onChange={e => setPaymentSentConfirmed(e.target.checked)} style={{ marginTop: 2 }} />
                  I have sent {NAIRA}{grandTotal.toLocaleString()} to this account
                </label>
              </div>
            ) : (
              <div style={{ background: '#FEF3C7', border: '1px solid #FDE68A', borderRadius: 14, padding: 16, marginBottom: 20 }}>
                <p style={{ fontSize: 12.5, color: '#92400E', lineHeight: 1.5 }}>
                  This store hasn't finished setting up how to receive payment yet. Please contact
                  the seller directly to complete your order.
                </p>
              </div>
            )}
            ...
            >
              {placing ? 'Please wait...' : "I've Sent This Payment"}
            </button>
          </>
        )}
      </div>
...
export function StorefrontScreen() {
  ...
  // Menu links scroll to real sections already on the page rather than navigating away, so a
  // customer never loses their place in the products they were already looking at.
  ...
  return (
    <div style={{ minHeight: '100vh', background: theme.background, fontFamily: 'Inter, sans-serif', transition: 'background 0.5s ease', overflowX: 'hidden' as const, width: '100%', maxWidth: '100vw', boxSizing: 'border-box' as const, display: 'flex', flexDirection: 'column' as const }}>
      <style>{`
      ...
      {/* Everything above the footer grows to fill any leftover space on a short page - so on a
          store with very few products, the page's own lighter background can never show as a
          gap beneath the footer. The footer is always pushed flush against the true bottom of
          the screen, whether the content above it is short or genuinely needs scrolling. */}
      <div style={{ flex: 1 }}>
      {merchant.announcementBanner && (
        <div style={{ background: theme.accent, overflow: 'hidden', whiteSpace: 'nowrap' as const, padding: '8px 0' }}>
          <div style={{ display: 'inline-block', animation: 'sp-marquee-scroll 16s linear infinite' }}>
            <span style={{ color: theme.secondary, fontSize: 11.5, fontWeight: 800, letterSpacing: 0.2, paddingRight: 48 }}>{merchant.announcementBanner}</span>
            <span style={{ color: theme.secondary, fontSize: 11.5, fontWeight: 800, letterSpacing: 0.2, paddingRight: 48 }}>{merchant.announcementBanner}</span>
          </div>
          <style>{`@keyframes sp-marquee-scroll { from { transform: translateX(0); } to { transform: translateX(-50%); } }`}</style>
        </div>
      )}

      <SaleCountdownBanner merchant={merchant} theme={theme} />

      <DarkHeader theme={theme} merchant={merchant} slug={slug!} onWishlistClick={() => setShowWishlist(w => !w)} wishlistActive={showWishlist} onSearchClick={() => setSearchOpen(o => !o)} searchActive={searchOpen} onMenuClick={() => setMenuOpen(true)} />

      <StoreMenu open={menuOpen} onClose={() => setMenuOpen(false)} theme={theme} merchant={merchant} slug={slug!} onJump={jumpTo} />
      ...
      {!showWishlist && (
        <>
          ...
          <StoreHero merchant={merchant} theme={theme} products={products} slug={slug!} />

          {/* The full business-website sections. Trust and context first, so a customer arriving
              cold understands who this business is before they start browsing prices. */}
          <AboutSection merchant={merchant} theme={theme} />
          <OffersSection merchant={merchant} theme={theme} />
          <PhotoBand merchant={merchant} />
          <WhySection merchant={merchant} theme={theme} />
          ...
          {/* Featured Collections - built from real categories. With only a handful of
              categories a grid shows them all at once, which is genuinely better. Past three,
              the grid starts pushing the actual products off the screen, so it becomes a
              horizontal rail instead - categories are navigation, not the destination. */}
          {categories.length > 0 && (() => {
            const scrolls = categories.length > 3;
            const CategoryCard = ({ cat, fixedWidth }: { cat: string; fixedWidth?: boolean }) => {
              ...
              return (
                <button
                  ...
                  className="sp-card"
                  style={{
                    position: 'relative' as const, borderRadius: 18, overflow: 'hidden', border: 'none',
                    cursor: 'pointer', aspectRatio: fixedWidth ? '0.82' : '0.95',
                    background: `linear-gradient(135deg, ${theme.primary}22, ${theme.secondary}22)`,
                    padding: 0, transition: 'transform 0.15s ease',
                    ...(fixedWidth ? { width: 132, flexShrink: 0 } : {}),
                  }}
                >
                  {rep?.imageUrl ? (
                    <img src={getOptimizedImageUrl(rep.imageUrl, 260, fixedWidth ? 0.82 : 0.95)} alt={cat} style={{ width: '100%', height: '100%', objectFit: 'cover' as const, position: 'absolute' as const, inset: 0 }} loading="lazy" />
                  ) : (
                    <div style={{ position: 'absolute' as const, inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <Icon size={34} color={theme.primary} />
                    </div>
                  )}
                  <div style={{ position: 'absolute' as const, inset: 0, background: 'linear-gradient(to top, rgba(0,0,0,0.68), transparent 58%)' }} />
                  <div style={{ position: 'absolute' as const, bottom: 11, left: 11, right: 11, textAlign: 'left' as const }}>
                    <p style={{ color: '#fff', fontSize: 12.5, fontWeight: 800, lineHeight: 1.25, overflow: 'hidden', textOverflow: 'ellipsis' as const, whiteSpace: 'nowrap' as const }}>{cat}</p>
                    <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: 10.5, marginTop: 1 }}>{inCat.length} item{inCat.length !== 1 ? 's' : ''}</p>
                  </div>
                </button>
              );
            };
            ...
            return (
              <div id="sp-categories" style={{ padding: scrolls ? '0 0 24px' : '0 16px 24px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14, padding: scrolls ? '0 16px' : 0 }}>
                  <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 18, fontWeight: 700, color: '#111827' }}>Shop by category</h3>
                </div>
                {scrolls ? (
                  <div style={{ display: 'flex', gap: 11, overflowX: 'auto' as const, padding: '0 16px 4px', WebkitOverflowScrolling: 'touch' as const }}>
                    {categories.map(cat => <CategoryCard key={cat} cat={cat} fixedWidth />)}
                  </div>
                ) : (
          ...
          {/* New Arrivals - a horizontal rail rather than a second grid, so it reads as "here is
              something fresh" without competing with the main product listing above it. Only
              renders when the merchant has actually marked products as new. */}
          {newArrivals.length > 0 && (
            <div id="sp-new" style={{ padding: '8px 0 26px' }}>
              <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 18, fontWeight: 700, color: '#111827', marginBottom: 14, padding: '0 16px' }}>New Arrivals</h3>
              <div style={{ display: 'flex', gap: 12, overflowX: 'auto' as const, padding: '0 16px 4px', WebkitOverflowScrolling: 'touch' as const }}>
                {newArrivals.slice(0, 10).map(p => (
                  <div key={p.id} style={{ width: 148, flexShrink: 0 }}>
                    <ProductCard product={p} theme={theme} slug={slug!} wishlistIds={wishlistIds} />
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Proof and location come after browsing - someone still deciding scrolls down, sees
              real reviews and exactly where the shop is, and has what they need to commit. */}
          <StoreReviewsSection merchantUid={merchant.uid} theme={theme} />
        </>
      )}
      </div>
...
export function ProductDetailScreen() {
  ...
  const handleAddToCart = () => {
    const groupNames = Object.keys(selectedOptions).filter(g => selectedOptions[g]?.length > 0);
    // Cartesian product of every selected value across every option group - e.g. Colors
    // [Red, Blue] x Sizes [M, L] becomes four real combinations: Red/M, Red/L, Blue/M, Blue/L.
    let combinations: { [group: string]: string }[] = [{}];
    for (const g of groupNames) {
      const next: { [group: string]: string }[] = [];
      for (const combo of combinations) {
        for (const v of selectedOptions[g]) {
          next.push({ ...combo, [g]: v });
        }
      }
      combinations = next;
    ...
    combinations.forEach(combo => {
      const combQty = combinations.length === 1 ? quantity : 1;
      addToCart(slug!, product, combQty, Object.keys(combo).length ? combo : undefined);
    });
  ...
  return (
    <div style={{ minHeight: '100vh', background: theme.background, fontFamily: 'Inter, sans-serif', transition: 'background 0.5s ease', overflowX: 'hidden' as const, display: 'flex', flexDirection: 'column' as const }}>
      <style>{`* { box-sizing: border-box; } .sp-btn:active { transform: scale(0.96); }`}</style>

      {/* Everything above the footer grows to fill any leftover space, same reasoning as the
          storefront home page - the footer must always sit flush against the true bottom of
          the screen, never leaving the page's own lighter background exposed beneath it. */}
      <div style={{ flex: 1 }}>
      <SaleCountdownBanner merchant={merchant} theme={theme} />
      ...
      {/* Full-bleed product gallery. Running edge to edge with no corner radius gives the photo
          the whole screen width, which is what a customer actually wants when deciding whether to
          buy - the image is the product, so it gets the room. maxHeight keeps this in check on
          a small/narrow screen though - without it, a tall aspect ratio at full width can grow
          to dominate most of the visible screen before any product details even show, which is
          exactly what was happening on smaller devices. */}
      <div style={{ width: '100%', aspectRatio: '1', background: `linear-gradient(135deg, ${theme.primary}12, ${theme.secondary}0a)`, overflow: 'hidden', position: 'relative' as const }}>
        {gallery.length === 0 ? (
            ...
            >
              {gallery.map((img, i) => (
                <div key={i} style={{ minWidth: '100%', height: '100%', scrollSnapAlign: 'start' as const, flexShrink: 0 }}>
                  <img src={getOptimizedImageUrl(img, 800)} alt={`${product.name} ${i + 1}`} style={{ width: '100%', height: '100%', objectFit: 'contain' as const }} loading="lazy" />
                </div>
              ))}
            </div>
            {gallery.length > 1 && (
      ...
      {gallery.length > 1 && (
        <div style={{ display: 'flex', gap: 10, padding: '12px 16px 6px', overflowX: 'auto' as const }}>
          {gallery.map((img, i) => (
            <button
              key={i}
      ...
      <div style={{ padding: '20px 20px 0' }}>
        ...
        {/* 6 & 7. Color (large square swatches) and Size (rounded-square buttons) - left aligned.
            Each value toggles on/off independently, rather than one replacing another - a
            customer wanting the same design in two colors, or two sizes, can select all of
            them here, and Add to Cart below adds a proper separate line for each combination. */}
        {product.options && product.options.length > 0 && product.options.map(group => {
          const isColor = isColorOptionGroup(group.name);
          const selectedForGroup = selectedOptions[group.name] || [];
          const toggleValue = (v: string) => {
            const current = selectedOptions[group.name] || [];
            const next = current.includes(v) ? current.filter(x => x !== v) : [...current, v];
            setSelectedOptions({ ...selectedOptions, [group.name]: next });
          };
          return (
            <div key={group.name} style={{ marginBottom: 18, textAlign: 'left' as const }}>
              <p style={{ fontSize: 14, fontWeight: 700, color: '#111827', marginBottom: 3, textAlign: 'left' as const }}>{group.name}{selectedForGroup.length ? `: ${selectedForGroup.join(', ')}` : ''}</p>
              <p style={{ fontSize: 11.5, color: '#9CA3AF', marginBottom: 10 }}>Want more than one? Tap all the {group.name.toLowerCase()}s you'd like.</p>
              {isColor ? (
                <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' as const, justifyContent: 'flex-start' as const }}>
                  {group.values.map(v => {
                    const isSelected = selectedForGroup.includes(v);
        ...
        <div style={{ marginBottom: 26 }}>
          ...
          {/* Listed rather than tiled. Four cramped columns forced the labels down to 9.5px to
              fit, which is genuinely hard to read - stacked rows give each point the full width,
              so the text can sit at a normal size right beside its icon. */}
          <div style={{ background: '#F9FAFB', borderRadius: 14, padding: '6px 14px' }}>
            {[
              ...
              <div key={t.label} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 0', borderBottom: i < arr.length - 1 ? '1px solid #EEF0F3' : 'none' }}>
                <t.Icon size={19} color={theme.primary} style={{ flexShrink: 0 }} />
                <p style={{ fontSize: 14, color: '#374151', fontWeight: 600 }}>{t.label}</p>
              </div>
            ))}
          </div>
        </div>
            ...
            {related.map(p => (
              <Link key={p.id} to={`/${slug}/product/${p.id}`} style={{ textDecoration: 'none', width: 140, flexShrink: 0 }}>
                <div style={{ background: theme.background, borderRadius: 16, overflow: 'hidden', border: `1px solid ${theme.accent}44`, boxShadow: '0 4px 14px rgba(0,0,0,0.06)' }}>
                  <div style={{ width: '100%', aspectRatio: '3/4', background: `linear-gradient(135deg, ${theme.primary}12, ${theme.secondary}0a)`, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                    {p.imageUrl ? (
                      <img src={getOptimizedImageUrl(p.imageUrl, 300, 0.75)} alt={p.name} style={{ width: '100%', height: '100%', objectFit: 'cover' as const }} loading="lazy" />
                    ) : (
      ...
      <div style={{ padding: '0 20px', marginBottom: 24 }}>
        <div style={{ background: '#F9FAFB', border: '1px solid #E5E7EB', borderRadius: 14, padding: '18px 16px' }}>
          <p style={{ fontSize: 12.5, color: '#6B7280', lineHeight: 1.5 }}>Have a question before you buy? Message the seller directly, they can help with sizing, availability, or anything else.</p>
          {merchant.contactWhatsapp && (
