...
const PLANS = [
  { id: 'basic', name: 'Basic', desc: 'Everything you need to run your store', prices: { quarterly: 10000, biannual: 18000, annual: 30000 }, features: ['Unlimited product listings', 'Online storefront with secure checkout', 'Automatic inventory tracking, online and offline', 'Point of Sale and branded invoices', 'Coupons, bundles, and tiered pricing', 'Customer reviews and CRM', 'Order tracking for your customers', 'Cart timer and sale countdown tools', 'One brand theme per store category'], popular: false },
  { id: 'pro', name: 'Pro', desc: 'Built for growing businesses', prices: { quarterly: 20000, biannual: 40000, annual: 80000 }, features: ['Everything included in Basic', 'Staff accounts with role based permissions', 'Email marketing campaigns', 'Customer segmentation and groups', 'Advanced analytics with channel and product insights', 'Facebook Pixel and Google Analytics integration', 'Full access to all 32 brand themes', 'Free custom domain with annual billing', 'Priority customer support'], popular: true },
];
...
function getFirstAction(answers: { [key: string]: string }): string {
...
function Onboarding() {
  ...
  const STEP_LABELS = ['Your Business', 'Store Setup', 'First Product', 'Choose Plan', "You're Live"];
      ...
      <div style={{ padding: 24 }}>
        {step === 0 && (
          <div>
            ...
            <button
              ...
              style={{ width: '100%', background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, color: 'white', border: 'none', borderRadius: 14, padding: 16, fontSize: 16, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, opacity: Object.keys(answers).length < BUSINESS_STAGE_QUESTIONS.length ? 0.5 : 1 }}
              onClick={() => setStep(1)}
            >
                ...
                <button
                  ...
                  onClick={async () => {
                    if (store.name && store.category && store.phone && user?.uid) {
                      await updateProfile({ storeName: store.name, storeSlug: store.slug, category: store.category, phone: store.phone });
                      setStep(2);
                    }
                  }}
                >
        ...
        {step === 2 && (
          <div>
            ...
            <p style={{ color: C.gray, marginBottom: 24, fontSize: 14 }}>Let's get your store ready to show real products from the start. You can add more, and edit this one, anytime.</p>
            <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
              <div>
        ...
        {step === 3 && (
          <div>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              ...
              <button style={{ flex: 1, background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, color: 'white', border: 'none', borderRadius: 14, padding: 16, fontSize: 15, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }} onClick={async () => { await updateProfile({ plan: planTab, billing, businessStage: answers, onboardingComplete: true, trialStart: new Date().toISOString() }); setStep(4); }}>Start free trial <ArrowRight size={16} /></button>
            </div>
          </div>
        )}
        {step === 4 && (
          <div style={{ textAlign: 'center' as const, paddingTop: 40 }}>
            ...
            <p style={{ color: C.gray, marginBottom: 28, fontSize: 14 }}>Your 14 day free trial has started, and your first product is already in your store.</p>
            ...
            <div style={{ background: C.white, border: `1.5px solid ${C.border}`, borderRadius: 16, padding: 18, marginBottom: 20, textAlign: 'left' as const }}>
              ...
              <p style={{ fontSize: 14, fontWeight: 600, color: C.dark, lineHeight: 1.5 }}>{getFirstAction(answers)}</p>
            </div>
