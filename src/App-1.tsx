...
import { doc, setDoc, getDoc, onSnapshot, collection, deleteDoc, addDoc, query, orderBy, limit as fsLimit, arrayUnion } from 'firebase/firestore';

import { NAIRA, THEME_NICHES, PRODUCT_CATEGORIES, PRODUCT_ICON_MAP, PRODUCT_ICON_KEYS, ProductIcon, compressImage, uploadToCloudinary, isValidCSSColor, isColorOptionGroup, NIGERIAN_STATES, PAYSTACK_PUBLIC_KEY, PLATFORM_LOGO_URL, getOptimizedImageUrl, SUPPORT_EMAIL, FCM_VAPID_KEY } from './shared';
    ...
    <div style={{ background: C.bg, minHeight: '100vh', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ background: C.white, borderRadius: '24px 24px 0 0', marginTop: -16, padding: 20, paddingBottom: 100 }}>
        ...
        <div style={{ border: `1.5px solid ${C.border}`, borderRadius: 14, padding: 16, marginBottom: 16 }}>
          ...
          {pushStatus === 'enabled' ? (
            <div style={{ background: C.greenLight, color: C.green, borderRadius: 10, padding: '10px 12px', fontSize: 12.5, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}>
...
function Dashboard() {
  ...
  // is listening for the message and displays it manually. This is that missing piece.
  useEffect(() => {
    if (!user?.fcmTokens || user.fcmTokens.length === 0 || !('Notification' in window)) return;
