...
import { doc, setDoc, getDoc, onSnapshot, collection, deleteDoc, addDoc, query, orderBy, limit as fsLimit, arrayUnion, increment } from 'firebase/firestore';
...
function EmailCampaignScreen({ onBack, onNavigate }: { onBack: () => void; onNavigate: (action: string) => void }) {
  ...
  const handleSend = async () => {
    ...
    if (sentCount > 0) {
      try {
        await updateProfile({ messagingCredits: increment(-(sentCount * CREDITS_PER_EMAIL)) });
      } catch (err) {
        ...
        (async () => {
          try {
            ...
            if (result.verified) {
              ...
              await updateProfile({
                messagingCredits: increment(selected.credits),
