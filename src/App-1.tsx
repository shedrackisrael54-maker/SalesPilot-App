...
function OwnerDashboardScreen({ onBack }: { onBack: () => void }) {
  ...
  const handleExtendTrial = async (merchant: any) => {
    ...
    setActionMsg('');
    try {
      const newTrialStart = new Date(Date.now() - (14 - 14) * 24 * 60 * 60 * 1000).toISOString();
      // Extending a trial just means pushing trialStart forward by 14 days from today, giving
      // a fresh 14-day window using the exact same logic the app already uses everywhere else.
      ...
      setActionMsg('Could not extend trial. Please try again.');
    } finally {
  ...
  const handleChangePlan = async (merchant: any, plan: 'basic' | 'pro', days: number) => {
    ...
    try {
      ...
      setActionMsg('Could not change plan. Please try again.');
    } finally {
      setActionLoading(false);
    }
  };
    ...
    return (
      <div style={{ minHeight: '100vh', background: C.bg, fontFamily: 'Inter, sans-serif' }}>
        ...
        <div style={{ background: C.white, borderRadius: '24px 24px 0 0', marginTop: -16, padding: 20, paddingBottom: 100 }}>
          ...
          <p style={{ fontSize: 12, fontWeight: 800, color: C.gray, textTransform: 'uppercase' as const, letterSpacing: 0.6, marginBottom: 10 }}>Controls</p>
          <button disabled={actionLoading} onClick={() => handleExtendTrial(selectedMerchant)} style={{ width: '100%', background: C.bg, border: `1.5px solid ${C.border}`, borderRadius: 12, padding: 14, fontSize: 13.5, fontWeight: 700, color: C.dark, cursor: 'pointer', marginBottom: 10, textAlign: 'left' as const }}>
            Extend trial by 14 days
          </button>
          <button disabled={actionLoading} onClick={() => handleChangePlan(selectedMerchant, 'basic', 30)} style={{ width: '100%', background: C.bg, border: `1.5px solid ${C.border}`, borderRadius: 12, padding: 14, fontSize: 13.5, fontWeight: 700, color: C.dark, cursor: 'pointer', marginBottom: 10, textAlign: 'left' as const }}>
            Give Starter free for 30 days
          </button>
          <button disabled={actionLoading} onClick={() => handleChangePlan(selectedMerchant, 'pro', 30)} style={{ width: '100%', background: C.bg, border: `1.5px solid ${C.border}`, borderRadius: 12, padding: 14, fontSize: 13.5, fontWeight: 700, color: C.dark, cursor: 'pointer', marginBottom: 10, textAlign: 'left' as const }}>
            Give Growth free for 30 days
          </button>
          {actionMsg && <p style={{ fontSize: 12.5, color: C.green, marginTop: 8, fontWeight: 600 }}>{actionMsg}</p>}
        </div>
      </div>
    );
  }
