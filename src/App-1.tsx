...
const DASHBOARD_INFO_CARDS: DashboardInfoCard[] = [
  ...
  },
  {
    ...
    content: [
      ...
      { heading: 'Sell To People Who Already Trust You', text: 'Every past customer is easier to sell to than a stranger. One well-timed message, a new arrival, a quiet offer, can bring back a sale without spending a single Naira chasing someone new.' },
      { text: 'Find it: More, then Email Campaign' },
...
function SplashScreen({ fadingOut }: { fadingOut: boolean }) {
  ...
  return (
    <div style={{ position: 'fixed' as const, inset: 0, background: `linear-gradient(160deg, ${C.navy} 0%, ${C.navyLight} 60%, #0d1f33 100%)`, display: 'flex', flexDirection: 'column' as const, alignItems: 'center', justifyContent: 'center', zIndex: 999, opacity: fadingOut ? 0 : 1, transition: 'opacity 0.5s ease', overflow: 'hidden' }}>
      <div style={{ position: 'absolute' as const, width: 280, height: 280, borderRadius: '50%', background: 'radial-gradient(circle, rgba(16,185,129,0.18), transparent 70%)', top: '12%', left: '-18%', filter: 'blur(6px)' }} />
      <div style={{ position: 'absolute' as const, width: 240, height: 240, borderRadius: '50%', background: 'radial-gradient(circle, rgba(255,255,255,0.08), transparent 70%)', bottom: '8%', right: '-14%', filter: 'blur(6px)' }} />

      <div style={{ position: 'relative' as const, display: 'flex', flexDirection: 'column' as const, alignItems: 'center' }}>
        <div style={{ position: 'absolute' as const, width: 160, height: 160, borderRadius: '50%', background: 'radial-gradient(circle, rgba(255,255,255,0.16), transparent 70%)', animation: 'sp-glow-pulse 2.4s ease-in-out infinite', left: '50%', top: '50%', transform: 'translate(-50%, -50%)' }} />
        <div style={{ animation: 'sp-logo-in 0.8s cubic-bezier(0.34, 1.56, 0.64, 1) forwards', position: 'relative' as const, zIndex: 1 }}>
          <AppLogo size={92} />
        </div>
        <div style={{
          opacity: phase === 'combined' ? 1 : 0,
          transform: phase === 'combined' ? 'translateY(0)' : 'translateY(-6px)',
      ...
      <style>{`
        @keyframes sp-logo-in { from { opacity: 0; transform: scale(0.6); } to { opacity: 1; transform: scale(1); } }
        @keyframes sp-glow-pulse { 0%, 100% { opacity: 0.6; transform: scale(1); } 50% { opacity: 1; transform: scale(1.15); } }
      `}</style>
    </div>
  );
}
...
function SplashGate() {
  ...
  useEffect(() => {
    if ((minTimeElapsed && authReady || forceDone) && !fadingOut) {
      setFadingOut(true);
      try { sessionStorage.setItem('sp_splash_seen', '1'); } catch {}
      const t = setTimeout(() => setShowSplash(false), 400);
      return () => clearTimeout(t);
    }
  }, [minTimeElapsed, authReady, forceDone, fadingOut]);
