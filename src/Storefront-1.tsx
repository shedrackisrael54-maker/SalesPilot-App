...
import {
  ...
  Heart, ShoppingCart, Truck, ShieldCheck, RotateCcw, Sparkles, SlidersHorizontal, Zap, Star, X,
...
// The section that matters most for a physical shop. A customer who wants to visit needs three
// things fast: where, when, and how to get there - so the address, the opening hours and a real
// tappable directions link all sit together rather than being scattered through a footer.
function VisitSection({ merchant, theme }: { merchant: MerchantInfo; theme: StoreTheme }) {
  const c = merchant.websiteContent;
  const hasAnything = merchant.storeAddress?.trim() || c?.openingHours?.trim() || merchant.contactWhatsapp;
  if (!hasAnything) return null;
  const mapsHref = c?.mapsLink?.trim()
    || (merchant.storeAddress?.trim() ? `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(merchant.storeAddress)}` : '');
  return (
    <div style={{ padding: '34px 20px', background: `${theme.primary}08` }}>
      <SectionHeading label="Find us" title="Come and see us" theme={theme} />

      {merchant.storeAddress?.trim() && (
        <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', marginBottom: 18 }}>
          <MapPin size={18} color={theme.primary} style={{ flexShrink: 0, marginTop: 2 }} />
          <div style={{ minWidth: 0 }}>
            <p style={{ fontSize: 11.5, fontWeight: 700, color: '#9CA3AF', textTransform: 'uppercase' as const, letterSpacing: 0.5, marginBottom: 3 }}>Address</p>
            <p style={{ fontSize: 14.5, color: '#111827', lineHeight: 1.55, fontWeight: 600 }}>{merchant.storeAddress}</p>
          </div>
        </div>
      )}
      ...
      {c?.openingHours?.trim() && (
        <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', marginBottom: 18 }}>
          <Clock size={18} color={theme.primary} style={{ flexShrink: 0, marginTop: 2 }} />
          <div style={{ minWidth: 0 }}>
            <p style={{ fontSize: 11.5, fontWeight: 700, color: '#9CA3AF', textTransform: 'uppercase' as const, letterSpacing: 0.5, marginBottom: 3 }}>Opening hours</p>
            <p style={{ fontSize: 14.5, color: '#111827', lineHeight: 1.7, fontWeight: 600, whiteSpace: 'pre-wrap' as const }}>{c.openingHours}</p>
          </div>
        </div>
      )}
      ...
      {merchant.contactWhatsapp && (
        <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', marginBottom: 20 }}>
          <Phone size={18} color={theme.primary} style={{ flexShrink: 0, marginTop: 2 }} />
          <div style={{ minWidth: 0 }}>
            <p style={{ fontSize: 11.5, fontWeight: 700, color: '#9CA3AF', textTransform: 'uppercase' as const, letterSpacing: 0.5, marginBottom: 3 }}>Call us</p>
            <a href={`tel:${merchant.contactWhatsapp.replace(/\D/g, '')}`} style={{ fontSize: 14.5, color: '#111827', fontWeight: 600, textDecoration: 'none' }}>{merchant.contactWhatsapp}</a>
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' as const }}>
        {mapsHref && (
          <a href={mapsHref} target="_blank" rel="noreferrer" className="sp-btn" style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: theme.primary, color: '#fff', borderRadius: 12, padding: '12px 18px', fontSize: 13.5, fontWeight: 700, textDecoration: 'none', transition: 'transform 0.15s ease' }}>
            <MapPin size={15} /> Get directions
          </a>
        )}
        {merchant.contactWhatsapp && (
          <a href={`https://wa.me/${toWhatsAppNumber(merchant.contactWhatsapp)}?text=${encodeURIComponent('Hi, I have a question about your store')}`} target="_blank" rel="noreferrer" className="sp-btn" style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: '#fff', color: theme.primary, border: `1.5px solid ${theme.primary}`, borderRadius: 12, padding: '12px 18px', fontSize: 13.5, fontWeight: 700, textDecoration: 'none', transition: 'transform 0.15s ease' }}>
    ...
    <div style={{ minHeight: '100vh', background: theme.background, fontFamily: 'Inter, sans-serif', transition: 'background 0.5s ease', overflowX: 'hidden' as const, width: '100%', maxWidth: '100vw', boxSizing: 'border-box' as const }}>
      ...
      {!showWishlist && (
        <>
          ...
          <VisitSection merchant={merchant} theme={theme} />
        </>
      )}
