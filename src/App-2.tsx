...
import type { StoreTheme, ThemeNiche, DescriptionSection, OptionGroup, Product, Review, Order, Bundle } from './shared';
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100vh', paddingBottom: 80, fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ padding: '0 16px', marginTop: -16 }}>
        ...
        <div style={{ background: C.white, borderRadius: 20, padding: 20, marginBottom: 20, boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <h3 style={{ fontSize: 16, fontWeight: 700, color: C.dark }}>Recent Orders</h3>
            <button onClick={() => onNavigate('orders')} style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'transparent', border: 'none', color: C.navy, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>View all <ChevronRight size={14} /></button>
          </div>
          {recentOrders.length === 0 ? (
            <div style={{ textAlign: 'center' as const, padding: '24px 0', color: C.gray }}>
              <ShoppingBag size={26} color={C.border} style={{ marginBottom: 8 }} />
              <p style={{ fontSize: 13 }}>No orders yet. They'll show up here once customers checkout on your store.</p>
            </div>
          ) : recentOrders.slice(0, 3).map(order => (
            <div key={order.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0', borderBottom: `1px solid ${C.border}` }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}>
                <div style={{ width: 42, height: 42, borderRadius: '50%', background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, fontWeight: 700, color: 'white', flexShrink: 0 }}>{order.customerName?.[0]?.toUpperCase() || 'C'}</div>
                <div style={{ minWidth: 0 }}>
                  <p style={{ fontSize: 14, fontWeight: 600, color: C.dark, marginBottom: 2, overflow: 'hidden', textOverflow: 'ellipsis' as const, whiteSpace: 'nowrap' as const }}>{order.customerName || 'Customer'}</p>
                  <p style={{ fontSize: 12, color: C.gray }}>{order.createdAt ? new Date(order.createdAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }) : ''}</p>
                </div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
                <p style={{ fontSize: 14, fontWeight: 700, color: C.dark }}>{NAIRA}{(order.total || 0).toLocaleString()}</p>
                <span style={{ background: statusConfig[order.status]?.bg, color: statusConfig[order.status]?.color, fontSize: 11, fontWeight: 700, padding: '4px 10px', borderRadius: 20, whiteSpace: 'nowrap' as const }}>{order.status}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ background: C.white, borderRadius: 20, padding: 20, marginBottom: 20, boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <h3 style={{ fontSize: 16, fontWeight: 700, color: C.dark }}>Best Selling Products</h3>
            <button onClick={() => onNavigate('analytics')} style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'transparent', border: 'none', color: C.navy, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>View all <ChevronRight size={14} /></button>
          </div>
          {bestSelling.length === 0 ? (
            <div style={{ textAlign: 'center' as const, padding: '20px 0', color: C.gray }}>
              <p style={{ fontSize: 13 }}>No sales recorded yet - your top products will show up here once you start selling.</p>
            </div>
          ) : (
            <div style={{ display: 'flex', gap: 12, overflowX: 'auto' as const, paddingBottom: 8 }}>
              {bestSelling.map(({ product, sold }) => (
                <div key={product.id} style={{ minWidth: 150, background: C.bg, borderRadius: 16, padding: 14, flexShrink: 0 }}>
                  <div style={{ width: '100%', height: 100, borderRadius: 12, marginBottom: 10, overflow: 'hidden', background: `linear-gradient(135deg, ${C.navy}22, ${C.navy}44)`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {(product.images?.[0] || product.imageUrl) ? <img src={product.images?.[0] || product.imageUrl} alt={product.name} style={{ width: '100%', height: '100%', objectFit: 'cover' as const }} /> : <ProductIcon iconKey={product.emoji} size={36} color={C.navy} />}
                  </div>
                  <p style={{ fontSize: 13, fontWeight: 600, color: C.dark, marginBottom: 4, overflow: 'hidden', textOverflow: 'ellipsis' as const, whiteSpace: 'nowrap' as const }}>{product.name}</p>
                  <p style={{ fontSize: 13, fontWeight: 700, color: C.navy, marginBottom: 6 }}>{NAIRA}{(product.salePrice || product.price).toLocaleString()}</p>
                  <span style={{ background: C.greenLight, color: C.green, fontSize: 11, fontWeight: 700, padding: '3px 8px', borderRadius: 20 }}>{sold} sold</span>
                </div>
              ))}
            </div>
          )}
...
function ProductForm({ product, onSave, onCancel, externalError }: { product: Product | null; onSave: (p: Product) => void; onCancel: () => void; externalError?: string }) {
  ...
  const handleGenerateDescription = async () => {
    if (!name.trim()) {
      setDescError('Add a product name first so the description matches your product.');
      return;
    }
    setDescError('');
      ...
      const response = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'user',
              content: `You are writing a product description for an online store in Nigeria. The product is called "${name.trim()}"${price ? `, priced around ${NAIRA}${Number(price).toLocaleString()}` : ''}. Write the description as 4 to 5 short sections, each covering a different angle a real shopper would care about (for example: design and quality, how it's used day to day, who it's good for or occasions it suits, and why it makes a good gift or purchase, but adapt these based on what actually fits this product). For each section give a short title of 2 to 4 words, and 1 to 2 full sentences written the way a real person selling the product would naturally describe it, warm and genuine, not robotic or generic. Do not use dashes, bullet points, or markdown formatting inside the sentences. Respond ONLY with valid JSON in this exact shape, nothing else, no preamble, no markdown code fences: {"sections": [{"title": "...", "text": "..."}]}`,
            },
          ],
        }),
      });
      const data = await response.json();
      const raw = data.content?.map((block: any) => block.text || '').join('').trim();
      if (!raw) {
        setDescError('Could not generate a description right now. Please try again.');
        return;
      }
      const cleaned = raw.replace(/```json|```/g, '').trim();
      const parsed = JSON.parse(cleaned);
      if (Array.isArray(parsed.sections) && parsed.sections.length > 0) {
        setDescriptionSections(parsed.sections);
        setOpenSection(0);
      } else {
        setDescError('Could not generate a description right now. Please try again.');
      }
        ...
        <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <label style={{ ...label, marginBottom: 0 }}>Description <span style={{ color: C.gray, fontWeight: 400 }}>(optional)</span></label>
              <button
                onClick={handleGenerateDescription}
                disabled={generatingDesc}
                style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'transparent', border: `1.5px solid ${C.navy}`, borderRadius: 20, padding: '6px 12px', fontSize: 12, fontWeight: 700, color: C.navy, cursor: 'pointer', opacity: generatingDesc ? 0.6 : 1 }}
              >
                {generatingDesc ? 'Writing...' : <><Sparkles size={12} /> Generate with AI</>}
              </button>
            </div>
            ...
            {descriptionSections.length > 0 ? (
              ...
              <textarea
                ...
                placeholder="Tell customers what makes this product great, or tap Generate with AI..."
                value={description}
                onChange={e => setDescription(e.target.value)}
                maxLength={800}
              />
            )}
            {descError && <p style={{ color: '#EF4444', fontSize: 12, marginTop: 6 }}>{descError}</p>}
            {descriptionSections.length === 0 && (
...
function AnalyticsScreen({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    ...
    return () => { unsubProducts(); unsubOrders(); unsubPOS(); unsubInvoices(); unsubExpenses(); };
  }, [user?.uid]);
      ...
      <div style={{ background: C.white, borderRadius: '24px 24px 0 0', marginTop: -16, padding: 20 }}>
        ...
        {loading ? (
          ...
          <>
            ...
            <p style={{ fontSize: 14, fontWeight: 700, color: C.dark, marginBottom: 12 }}>Sales by Channel</p>
...
function MoreScreen({ logout, user, onNavigate }: any) {
  ...
  const menuItems = [
    ...
    { Icon: Bot, label: 'AI Studio', sub: '20 credits remaining', action: 'ai' },
