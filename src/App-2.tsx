...
import { doc, setDoc, getDoc, onSnapshot, collection, deleteDoc, addDoc, query, orderBy, limit as fsLimit, arrayUnion, increment, getDocs, deleteField } from 'firebase/firestore';

import { NAIRA, THEME_NICHES, PRODUCT_CATEGORIES, PRODUCT_ICON_MAP, PRODUCT_ICON_KEYS, ProductIcon, compressImage, uploadToCloudinary, isValidCSSColor, isColorOptionGroup, NIGERIAN_STATES, PAYSTACK_PUBLIC_KEY, PLATFORM_LOGO_URL, getOptimizedImageUrl, SUPPORT_EMAIL, FCM_VAPID_KEY, copyToClipboard, LANDING_IMAGES, OPTION_PRESETS } from './shared';
...
function AppLogo({ size }: { size: number }) {
  ...
  if (PLATFORM_LOGO_URL && !imageFailed) {
    // objectFit 'contain' rather than 'cover' so a wide, full-wordmark logo shows completely
    // instead of being cropped to a square - a square-ish icon still fills the space the same way.
    return <img src={PLATFORM_LOGO_URL} alt="SalesPilot" onError={() => setImageFailed(true)} style={{ height: size, maxWidth: size * 2.6, borderRadius: size >= 44 ? 12 : 8, objectFit: 'contain' as const, flexShrink: 0 }} />;
  }
  return (
...
// How long every new merchant's free trial lasts. Defined once here and used everywhere so
// the number can never drift out of sync between the access checks, the countdown banner, the
// onboarding copy and the marketing page - which is exactly how a "14 days" promise ends up
// silently locking someone out on day 7.
const TRIAL_DAYS = 7;

// A merchant has "Pro access" if they're still within their free trial (full access to
...
function isProAccess(user: any): boolean {
  ...
  if (user.trialStart) {
    ...
    if (elapsedDays < TRIAL_DAYS) return true;
  }
...
// True only once the free trial has genuinely run out AND there's no active paid
// subscription covering the gap - this is the one real condition that should actually lock a
// merchant out, not just nag them. A merchant who paid before their trial ended, or who is
// still within their trial, is never affected by this.
function isTrialExpiredUnpaid(user: any): boolean {
  if (!user?.trialStart) return false;
  // The platform owner's own account is never locked out - being shut out of your own product
  // would be absurd, and it keeps the lockout screen out of demos and screen recordings.
  // Every other merchant, including branded clients, follows the normal rule.
  if (isOwner(user)) return false;
  const elapsedDays = Math.floor((Date.now() - new Date(user.trialStart).getTime()) / (1000 * 60 * 60 * 24));
  if (elapsedDays < TRIAL_DAYS) return false;
  const hasActiveSubscription = user.subscriptionExpiresAt && new Date(user.subscriptionExpiresAt).getTime() > Date.now();
...
// The real brand graphic shown on the splash screen - hosted on the platform's own Cloudinary
// account, same as the logo and dashboard card images, so it never depends on an external link
// that could break.
const SPLASH_GRAPHIC_URL = 'https://res.cloudinary.com/xd2hkwf8/image/upload/v1786216032/file_000000009728820ca7a1c61dbadc5015_eppnpd.png';
...
function AuthProvider({ children }: any) {
  ...
  const updateProfile = async (data: any) => {
    if (!user?.uid) return;
    // Firestore rejects any field set to literally undefined, which is how every "clear this
    // setting" action in the app expresses itself (an emptied announcement banner, a removed
    // logo, a deleted tracking ID). Converting those to deleteField() makes clearing actually
    // work, instead of failing the whole save with a confusing connection error.
    const payload: any = {};
    Object.keys(data).forEach(key => {
      payload[key] = data[key] === undefined ? deleteField() : data[key];
    });
    await setDoc(doc(db, 'merchants', user.uid), payload, { merge: true });
    setUser((prev: any) => {
      const next = { ...prev };
      Object.keys(data).forEach(key => {
        if (data[key] === undefined) delete next[key];
...
function DashboardScreen({ user, onNavigate }: any) {
  ...
  // The owner's own account isn't on a trial, so the countdown banner has nothing true to say
  // there - and leaving it visible puts a "your trial has ended" warning in every demo and
  // screen recording of the product.
  if (user?.trialStart && !isOwner(user)) {
    ...
    <div style={{ background: C.bg, minHeight: '100vh', paddingBottom: 80, fontFamily: 'Inter, sans-serif' }}>
      <div style={{ background: `linear-gradient(135deg, ${C.navy} 0%, ${C.navyLight} 100%)`, padding: '16px 20px 24px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          <div>
            <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13, marginBottom: 2 }}>{user?.isStaff ? `Signed in as ${user.staffRole}` : `${getTimeGreeting()},`}</p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
            {[
              ...
              <button key={item.label} onClick={() => onNavigate?.(item.action)} style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 14, padding: '14px 4px', display: 'flex', flexDirection: 'column' as const, alignItems: 'center', gap: 8, cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
                {/* An outlined circle rather than a solid filled square - the icon carries its
                    own color and the ring gives it presence without the flat "icon in a colored
                    box" look repeated on every card in the app. */}
                <div style={{ width: 40, height: 40, borderRadius: '50%', border: `1.5px solid ${item.color}33`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
  ...
  if (selectedOrder) {
    return (
      <div style={{ background: C.bg, minHeight: '100vh', paddingBottom: 80, fontFamily: 'Inter, sans-serif' }}>
        ...
        <div style={{ background: C.white, borderRadius: '24px 24px 0 0', marginTop: -16, padding: 20, minHeight: 'calc(100vh - 90px)' }}>
    ...
    <div style={{ background: C.bg, minHeight: '100vh', paddingBottom: 80, fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ background: C.white, borderRadius: '24px 24px 0 0', marginTop: -16, padding: 20, minHeight: 'calc(100vh - 90px)' }}>
        <div style={{ display: 'flex', gap: 8, overflowX: 'auto' as const, paddingBottom: 12, marginBottom: 16 }}>
          {filters.map(f => (
            <button key={f} onClick={() => setActiveFilter(f)} style={{ background: activeFilter === f ? C.navy : C.white, color: activeFilter === f ? 'white' : C.dark, border: activeFilter === f ? 'none' : `1.5px solid ${C.border}`, borderRadius: 10, padding: '9px 16px', fontSize: 13, fontWeight: activeFilter === f ? 700 : 600, cursor: 'pointer', whiteSpace: 'nowrap' as const, boxShadow: activeFilter === f ? '0 3px 8px rgba(20,42,69,0.25)' : 'none', transition: 'all 0.15s ease' }}>{f}</button>
          ))}
        </div>
        {ordersLoading && (
...
function ProductForm({ product, onSave, onDelete, onDuplicate, isDuplicate, onCancel, externalError }: { product: Product | null; onSave: (p: Product) => void; onDelete?: (id: string) => Promise<void>; onDuplicate?: (p: Product) => void; isDuplicate?: boolean; onCancel: () => void; externalError?: string }) {
  const { user } = useAuth();
  // A duplicate is a brand new product wearing an existing product's details, so it must behave
  // as an add, not an edit - otherwise Save would overwrite the original.
  const isEdit = !!product && !isDuplicate;
  const [deleting, setDeleting] = useState(false);

  // Deleting from inside the form matters because this is where a merchant already is when
  // they realise a product should go - having to back out to a list to do it is the kind of
  // small friction that leaves dead products sitting in a store for months.
  const handleDelete = async () => {
    ...
    try {
      ...
      setSaveErrorLocal(err?.message || 'Could not delete this product.');
      setDeleting(false);
    }
  };
  ...
  // Toggles a single preset choice on or off. Creates the option group on first tap, and
  // removes the group entirely when its last choice is cleared, so a merchant never ends up
  // with an empty "Size" option sitting there doing nothing.
  const togglePresetValue = (groupName: string, value: string) => {
    setOptionsError('');
    const existingIndex = options.findIndex(g => g.name.toLowerCase() === groupName.toLowerCase());
    if (existingIndex === -1) {
      if (options.length >= 2) {
        setOptionsError('You can only add up to 2 option types (for example Size and Color).');
        return;
      }
      setOptions([...options, { name: groupName, values: [value] }]);
      return;
    }
    const group = options[existingIndex];
    const has = group.values.some(v => v.toLowerCase() === value.toLowerCase());
    const nextValues = has ? group.values.filter(v => v.toLowerCase() !== value.toLowerCase()) : [...group.values, value];
    if (nextValues.length === 0) {
      setOptions(options.filter((_, i) => i !== existingIndex));
      return;
    }
    setOptions(options.map((g, i) => i === existingIndex ? { ...g, values: nextValues } : g));
  };

  const isPresetActive = (groupName: string, value: string) =>
    options.some(g => g.name.toLowerCase() === groupName.toLowerCase() && g.values.some(v => v.toLowerCase() === value.toLowerCase()));

  // Only show presets that actually make sense for what they're selling - shoe sizes on a
  // food product would just be noise. Colours apply to everything.
  const relevantPresets = OPTION_PRESETS.filter(p => p.categories.length === 0 || p.categories.includes(category));
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100vh', fontFamily: 'Inter, sans-serif' }}>
      <div style={{ background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, padding: '20px 20px 28px', display: 'flex', alignItems: 'center', gap: 14 }}>
        ...
        <h2 style={{ color: 'white', fontSize: 19, fontWeight: 700, flex: 1 }}>{isDuplicate ? 'Duplicate Product' : isEdit ? 'Edit Product' : 'Add Product'}</h2>
        {shareLink && (
        ...
        <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 11, color: C.gray, marginTop: 6 }}>
              Track profit and know when to restock.
            </p>
          </div>
          ...
          <div>
            ...
            {/* Tap-to-add presets. This is the fast path most sellers need - tap the sizes you
                actually stock and you're done, rather than typing each one out. Typing still
                works below for anything unusual. */}
            {relevantPresets.map(preset => (
              <div key={preset.label} style={{ marginBottom: 14 }}>
                <p style={{ fontSize: 12.5, fontWeight: 700, color: C.dark, marginBottom: 8 }}>{preset.label}</p>
                <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: 7 }}>
                  {preset.values.map(v => {
                    const active = isPresetActive(preset.name, v);
                    const swatch = isColorOptionGroup(preset.name) && isValidCSSColor(v);
                    return (
                      <button
                        key={v}
                        onClick={() => togglePresetValue(preset.name, v)}
                        style={{
                          display: 'flex', alignItems: 'center', gap: 6,
                          background: active ? C.navy : C.white,
                          color: active ? 'white' : C.dark,
                          border: `1.5px solid ${active ? C.navy : C.border}`,
                          borderRadius: 20, padding: swatch ? '7px 13px 7px 8px' : '8px 14px',
                          fontSize: 13, fontWeight: 600, cursor: 'pointer',
                        }}
                      >
                        {swatch && <span style={{ width: 15, height: 15, borderRadius: '50%', background: v, border: `1px solid ${active ? 'rgba(255,255,255,0.4)' : C.border}`, flexShrink: 0 }} />}
                        {v}
                      </button>
                    );
                  })}
            ...
            {options.length === 0 && (
              <p style={{ fontSize: 13, color: C.gray, fontStyle: 'italic' as const }}>Tap any choice above, or add your own below. Leave blank to sell this product as-is.</p>
            )}
          ...
          <button disabled={saving || uploading || deleting} onClick={handleSave} style={{ background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, color: 'white', border: 'none', borderRadius: 14, padding: 16, fontSize: 16, fontWeight: 700, cursor: 'pointer', marginTop: 8, opacity: saving || uploading || deleting ? 0.7 : 1 }}>
            {saving ? 'Saving...' : isEdit ? 'Save changes' : 'Add product'}
          </button>
          {isEdit && onDuplicate && product && (
            <button disabled={saving || uploading || deleting} onClick={() => onDuplicate(product)} style={{ background: C.bg, color: C.navy, border: `1.5px solid ${C.border}`, borderRadius: 14, padding: 14, fontSize: 15, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
              <Copy size={16} /> Duplicate this product
            </button>
          )}
          {isEdit && onDelete && (
            <button disabled={saving || uploading || deleting} onClick={handleDelete} style={{ background: '#FEE2E2', color: '#EF4444', border: 'none', borderRadius: 14, padding: 14, fontSize: 15, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, opacity: saving || uploading || deleting ? 0.7 : 1 }}>
...
function ProductsScreen() {
  ...
  const [view, setView] = useState<'list' | 'add' | 'edit' | 'duplicate'>('list');
  ...
  // Opens the form pre-filled with everything from an existing product, as a brand new one.
  // Nothing is saved until they hit Add, so a mistaken tap costs nothing. The name gets
  // "(copy)" appended so two identical names never sit in the list confusing the merchant.
  const handleDuplicate = (p: Product) => {
    setSaveError('');
    setEditing({ ...p, id: Date.now().toString(), name: `${p.name} (copy)` });
    setView('duplicate');
  };
  ...
  if (view === 'duplicate' && editing) {
    return <ProductForm product={editing} isDuplicate onSave={handleSave} onCancel={() => { setView('list'); setEditing(null); }} externalError={saveError} />;
  }
  if (view === 'edit' && editing) {
    return <ProductForm product={editing} onSave={handleSave} onDelete={handleDelete} onDuplicate={handleDuplicate} onCancel={() => { setView('list'); setEditing(null); }} externalError={saveError} />;
  }
              ...
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' as const }}>
                  ...
                  {typeof product.quantity === 'number' && (
                    <span style={{
                      background: product.quantity <= 0 ? '#FEE2E2' : product.quantity <= 5 ? C.orangeLight : C.bg,
                      color: product.quantity <= 0 ? '#EF4444' : product.quantity <= 5 ? C.orange : C.gray,
  ...
  const handleSave = async () => {
    ...
    try {
      await onSave({
        ...
        ...(category === 'Other' && note.trim() ? { note: note.trim() } : {}),
      });
    } catch (err: any) {
...
function SettingsScreen({ onBack }: { onBack: () => void }) {
  ...
  // A genuine, clean restart for notification setup - wipes every token this account has ever
  // saved, so a fresh "Enable on This Device" afterward is guaranteed to register a truly new
  // token, with nothing old left sitting alongside it that could still be interfering.
  const handleResetTokens = async () => {
    setResettingTokens(true);
    setTestPushResult('');
    try {
      await updateProfile({ fcmTokens: [] });
      setPushStatus('idle');
    } catch (err: any) {
      setTestPushResult(`Could not reset: ${err?.message || 'unknown error'}`);
    } finally {
  ...
  const handleEnablePush = async () => {
    ...
    try {
      ...
      // Using updateProfile here (not a raw setDoc) matters - it also updates the local user
      // state kept in memory, not just the database. Without that, the foreground notification
      // listener elsewhere in the app - which depends on reading fcmTokens from that same local
      // state - would keep seeing the old, empty value until a full page reload, even though
      // the token saved correctly. That gap is exactly what silently swallowed a real test
      // notification: the send succeeded because it reads the database directly, but nothing
      // in the open app was actually listening yet to display it.
      const newTokens = Array.from(new Set([...(user.fcmTokens || []), token]));
      await updateProfile({ fcmTokens: newTokens });
          ...
          {pushStatus === 'enabled' ? (
            <div>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' as const }}>
                ...
                <button disabled={resettingTokens} onClick={handleResetTokens} style={{ background: C.bg, color: C.dark, border: `1.5px solid ${C.border}`, borderRadius: 10, padding: '10px 16px', fontSize: 13, fontWeight: 700, cursor: 'pointer', opacity: resettingTokens ? 0.7 : 1 }}>
                  {resettingTokens ? 'Resetting...' : 'Reset & Start Fresh'}
                </button>
              </div>
              {testPushResult && (
...
function CompleteStoreScreen({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100vh', paddingBottom: 80, fontFamily: 'Inter, sans-serif' }}>
      ...
      {/* One continuous white panel for everything below the header, rather than a separate
          grey-blue strip for the progress card sitting between the header and the checklist -
          that in-between tone was what made the screen read as cut into pieces. */}
      <div style={{ background: C.white, borderRadius: '24px 24px 0 0', marginTop: -16, padding: 20, minHeight: 'calc(100vh - 90px)' }}>
        <div style={{ background: C.bg, borderRadius: 16, padding: 16, marginBottom: 20 }}>
          ...
          <div style={{ height: 8, background: C.white, borderRadius: 20, overflow: 'hidden' }}>
...
function BrandStudioScreen({ onBack }: { onBack: () => void }) {
  ...
  const [appliedThemeId, setAppliedThemeId] = useState<string>(user?.themeId || 'gallery-stone');
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);
  const [error, setError] = useState('');
...
// Small presentational helpers for the analytics screen. Kept module-level rather than nested
// so they aren't redefined on every render of a screen that already does a lot of work.
function AnSection({ title, note, children, tight }: { title: string; note?: string; children: any; tight?: boolean }) {
  return (
    <div style={{ padding: tight ? '18px 20px' : '22px 20px', borderTop: `1px solid ${C.border}` }}>
      <p style={{ fontSize: 16, fontWeight: 700, color: C.dark, marginBottom: note ? 4 : 16 }}>{title}</p>
      {note && <p style={{ fontSize: 13, color: C.gray, marginBottom: 16, lineHeight: 1.5 }}>{note}</p>}
      {children}
    </div>
  );
}

function AnEmpty({ text }: { text: string }) {
  return <p style={{ fontSize: 13, color: C.gray, lineHeight: 1.5 }}>{text}</p>;
}
...
function AnalyticsScreen({ onBack }: { onBack: () => void }) {
  ...
  const periodExpenseList = expenses.filter(e => inPeriod(e.date));
  const totalExpenses = periodExpenseList.reduce((s, e) => s + (e.amount || 0), 0);

  const invoiceTotal = (inv: any) => (inv.items || []).reduce((x: number, it: any) => x + it.quantity * it.unitPrice, 0);
  const onlineRevenue = periodOrders.reduce((s, o) => s + (o.total || 0), 0);
  const offlineRevenue = periodPOS.reduce((s, sale) => s + (sale.total || 0), 0) + periodInvoices.reduce((s, inv) => s + invoiceTotal(inv), 0);
  const moneyIn = onlineRevenue + offlineRevenue;
  // "Money kept" is revenue minus recorded expenses. It is deliberately NOT called profit -
  // SalesPilot doesn't track a cost price per product, so true profit can't be calculated, and
  // labelling this "profit" would badly overstate it for anyone who hasn't logged their costs.
  const moneyKept = moneyIn - totalExpenses;
  const keptPct = moneyIn > 0 ? Math.round((moneyKept / moneyIn) * 100) : 0;
  const discountGiven = periodOrders.reduce((s, o) => s + (o.discount || 0), 0);
  const orderCount = periodOrders.length + periodPOS.length + periodInvoices.length;

  // Daily revenue across the selected period, combining all three sale sources. This is what
  // powers the trend line - the one thing that answers "is my business growing or shrinking",
  // which no single total figure can ever show.
  const dailyTotals: { [date: string]: number } = {};
  const addDaily = (dateStr: string | undefined, amount: number) => {
    if (!dateStr) return;
    const d = dateStr.slice(0, 10);
    dailyTotals[d] = (dailyTotals[d] || 0) + amount;
  };
  periodOrders.forEach(o => addDaily(o.createdAt, o.total || 0));
  periodPOS.forEach(s => addDaily(s.date, s.total || 0));
  periodInvoices.forEach(inv => addDaily(inv.date, invoiceTotal(inv)));

  const dailySeries = (() => {
    const days = period === 'Today' ? 1 : period === 'This Week' ? 7 : 30;
    const out: { date: string; value: number }[] = [];
    for (let i = days - 1; i >= 0; i--) {
      const d = new Date(Date.now() - i * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
      out.push({ date: d, value: dailyTotals[d] || 0 });
    }
    return out;
  })();

  const halfPoint = Math.floor(dailySeries.length / 2);
  const firstHalf = dailySeries.slice(0, halfPoint).reduce((s, p) => s + p.value, 0);
  const secondHalf = dailySeries.slice(halfPoint).reduce((s, p) => s + p.value, 0);
  const trendChange = firstHalf > 0 ? Math.round(((secondHalf - firstHalf) / firstHalf) * 100) : (secondHalf > 0 ? 100 : 0);
  const peakDay = dailySeries.reduce((b, p) => (p.value > b.value ? p : b), dailySeries[0] || { date: '', value: 0 });

  // Which weekday actually earns most - genuinely actionable, since it tells a merchant when to
  // post new stock, when to run a promotion, and when they can afford to rest.
  ...
  Object.entries(dailyTotals).forEach(([date, amount]) => {
    ...
    if (!isNaN(idx)) byWeekday[idx].value += amount;
  });
  const orderedWeekdays = [...byWeekday.slice(1), byWeekday[0]];
  const bestWeekday = orderedWeekdays.reduce((b, d) => (d.value > b.value ? d : b), orderedWeekdays[0]);
  const maxWeekday = Math.max(...orderedWeekdays.map(d => d.value), 1);

  // Units and revenue per product, across all three sale sources
  ...
  const bestSellers = Object.values(productStats).sort((a, b) => b.revenue - a.revenue).slice(0, 5);
  const maxSellerRevenue = Math.max(...bestSellers.map(p => p.revenue), 1);
  ...
  // Customers, their spend, and how many came back for a second purchase
  const customerSpend: { [key: string]: { name: string; spent: number; orders: number } } = {};
  const addCustomer = (rawName: string | undefined, amount: number) => {
    const key = rawName?.trim().toLowerCase();
    if (!key) return;
    if (!customerSpend[key]) customerSpend[key] = { name: rawName!.trim(), spent: 0, orders: 0 };
    customerSpend[key].spent += amount;
    customerSpend[key].orders += 1;
  };
  periodOrders.forEach(o => addCustomer(o.customerName, o.total || 0));
  periodPOS.forEach(s => addCustomer(s.customerName, s.total || 0));
  periodInvoices.forEach(inv => addCustomer(inv.customerName, invoiceTotal(inv)));
  const customerList = Object.values(customerSpend);
  const customerCount = customerList.length;
  const returningCustomers = customerList.filter(c => c.orders > 1).length;
  const avgSpend = customerCount > 0 ? moneyIn / customerCount : 0;
  const topCustomers = [...customerList].sort((a, b) => b.spent - a.spent).slice(0, 3);

  const periodVisits = Object.entries(dailyVisits).filter(([date]) => inPeriod(date)).reduce((sum, [, count]) => sum + count, 0);
  const conversionRate = periodVisits > 0 ? (periodOrders.length / periodVisits) * 100 : 0;

  // Expenses grouped by category, so a merchant can see what is actually eating their money
  const expenseByCategory: { [cat: string]: number } = {};
  periodExpenseList.forEach(e => {
  ...
  const CHART_COLORS = [C.purple, C.navy, C.green, C.orange, C.blue];
  const periodWord = period === 'Today' ? 'today' : period === 'This Week' ? 'this week' : period === 'This Month' ? 'this month' : 'so far';

  // Trend line geometry
  const CW = 320, CH = 118, CP = 6;
  const maxDaily = Math.max(...dailySeries.map(p => p.value), 1);
  const linePoints = dailySeries.map((p, i) => [
    CP + (dailySeries.length > 1 ? (i / (dailySeries.length - 1)) : 0.5) * (CW - CP * 2),
    CH - CP - (p.value / maxDaily) * (CH - CP * 2),
  ]);
  const linePath = linePoints.map(([x, y], i) => `${i === 0 ? 'M' : 'L'}${x.toFixed(1)},${y.toFixed(1)}`).join(' ');
  const areaPath = linePoints.length > 1
    ? `${linePath} L${linePoints[linePoints.length - 1][0].toFixed(1)},${CH} L${linePoints[0][0].toFixed(1)},${CH} Z`
    : '';
  const peakIndex = dailySeries.reduce((b, p, i) => (p.value > dailySeries[b].value ? i : b), 0);

  const attentionItems = [
    outOfStockCount > 0 && { Icon: Package, text: `${outOfStockCount} product${outOfStockCount !== 1 ? 's have' : ' has'} finished. Restock or hide from your store.`, tone: '#EF4444', bg: '#FEE2E2' },
    lowStockCount > 0 && { Icon: AlertTriangle, text: `${lowStockCount} product${lowStockCount !== 1 ? 's are' : ' is'} running low.`, tone: C.orange, bg: C.orangeLight },
    pendingOrderCount > 0 && { Icon: Clock, text: `${pendingOrderCount} order${pendingOrderCount !== 1 ? 's are' : ' is'} waiting for you to confirm payment.`, tone: C.blue, bg: C.blueLight },
  ].filter(Boolean) as { Icon: any; text: string; tone: string; bg: string }[];

  const statCards = [
    { Icon: ShoppingBag, label: 'Sales made', value: String(orderCount), sub: 'completed' },
    { Icon: Users, label: 'Customers', value: String(customerCount), sub: 'who bought' },
    { Icon: Wallet, label: 'Average sale', value: `${NAIRA}${Math.round(avgSpend).toLocaleString()}`, sub: 'per customer' },
    { Icon: Eye, label: 'Store visits', value: periodVisits.toLocaleString(), sub: 'people looked' },
    { Icon: Repeat, label: 'Came back', value: String(returningCustomers), sub: 'bought again' },
    { Icon: Tag, label: 'Discounts', value: `${NAIRA}${discountGiven.toLocaleString()}`, sub: 'from coupons' },
  ];

  return (
    <div style={{ background: C.bg, minHeight: '100vh', paddingBottom: 80, fontFamily: 'Inter, sans-serif' }}>
      <style>{`
        @keyframes an-draw { from { stroke-dashoffset: 1400; } to { stroke-dashoffset: 0; } }
        @keyframes an-rise { from { transform: scaleY(0); } to { transform: scaleY(1); } }
        .an-line { stroke-dasharray: 1400; animation: an-draw 1.4s cubic-bezier(0.22,1,0.36,1) both; }
        .an-bar { transform-origin: bottom; animation: an-rise 0.7s cubic-bezier(0.22,1,0.36,1) both; }
        .an-num { font-variant-numeric: tabular-nums; letter-spacing: -0.02em; }
        @media (prefers-reduced-motion: reduce) { .an-line, .an-bar { animation: none; } }
      `}</style>
      ...
      <div style={{ background: C.white, borderRadius: '24px 24px 0 0', marginTop: -16 }}>
        <div style={{ display: 'flex', gap: 8, overflowX: 'auto' as const, padding: '18px 20px' }}>
          {periods.map(p => (
            <button key={p} onClick={() => setPeriod(p)} style={{ background: period === p ? C.navy : C.white, color: period === p ? 'white' : C.dark, border: period === p ? 'none' : `1.5px solid ${C.border}`, borderRadius: 10, padding: '9px 16px', fontSize: 12.5, fontWeight: period === p ? 700 : 600, cursor: 'pointer', whiteSpace: 'nowrap' as const, boxShadow: period === p ? '0 3px 8px rgba(20,42,69,0.25)' : 'none', transition: 'all 0.15s ease' }}>{p}</button>
          ))}
        </div>

        {loading ? (
          <div style={{ textAlign: 'center' as const, padding: '40px 0', color: C.gray }}>Loading your numbers...</div>
        ) : (
          <>
            {/* Trend - the hero. Answers "am I growing", which no single total ever can. */}
            <div style={{ background: C.navy, borderRadius: 20, padding: '20px 18px 12px', margin: '0 20px 14px', overflow: 'hidden' }}>
              <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.65)', marginBottom: 6 }}>Money came in {periodWord}</p>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, flexWrap: 'wrap' as const, marginBottom: 3 }}>
                <p className="an-num" style={{ fontSize: 'clamp(30px, 8.5vw, 38px)', fontWeight: 800, color: 'white', lineHeight: 1 }}>{NAIRA}{moneyIn.toLocaleString()}</p>
                {dailySeries.length > 1 && trendChange !== 0 && (
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: trendChange >= 0 ? 'rgba(16,185,129,0.2)' : 'rgba(239,68,68,0.2)', color: trendChange >= 0 ? '#6EE7B7' : '#FCA5A5', fontSize: 12.5, fontWeight: 700, padding: '4px 9px', borderRadius: 20 }}>
                    {trendChange >= 0 ? <TrendingUp size={12} /> : <TrendingDown size={12} />} {trendChange >= 0 ? '+' : ''}{trendChange}%
                  </span>
                )}
              </div>
              <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)', marginBottom: 14 }}>
                {dailySeries.length > 1 ? 'Recent days compared to the days before' : 'Sales recorded today'}
              </p>
              {moneyIn === 0 ? (
                <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.55)', padding: '18px 0 22px', lineHeight: 1.6 }}>
                  No sales recorded {periodWord} yet. Once money starts coming in, your daily trend appears here.
                </p>
              ) : (
                <>
                  <svg viewBox={`0 0 ${CW} ${CH}`} style={{ width: '100%', height: 'auto', display: 'block', overflow: 'visible' }}>
                    ...
                    {areaPath && <path d={areaPath} fill="url(#anFill)" />}
                    <path className="an-line" d={linePath} fill="none" stroke={C.green} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
                    {linePoints[peakIndex] && <circle cx={linePoints[peakIndex][0]} cy={linePoints[peakIndex][1]} r="4.5" fill="white" stroke={C.green} strokeWidth="2.5" />}
                  </svg>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6 }}>
                    <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.45)' }}>{dailySeries.length} days</span>
                    <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.45)' }}>Best day: {NAIRA}{peakDay.value.toLocaleString()}</span>
                  </div>
                </>
              )}
            </div>
            ...
            {/* Money kept */}
            <div style={{ margin: '0 20px 22px', border: `1px solid ${C.border}`, borderRadius: 18, padding: 18 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                <Wallet size={16} color={C.green} />
                <p style={{ fontSize: 13.5, color: C.gray, fontWeight: 600 }}>Money you kept</p>
              </div>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 16, flexWrap: 'wrap' as const }}>
                <p className="an-num" style={{ fontSize: 32, fontWeight: 800, color: C.dark, lineHeight: 1 }}>{NAIRA}{moneyKept.toLocaleString()}</p>
                {moneyIn > 0 && <span style={{ background: C.greenLight, color: C.green, fontSize: 12, fontWeight: 700, padding: '3px 9px', borderRadius: 20 }}>{keptPct}% of sales</span>}
              </div>
              <div style={{ display: 'flex', height: 12, borderRadius: 7, overflow: 'hidden', background: C.border, marginBottom: 12 }}>
                <div style={{ width: `${Math.max(keptPct, 0)}%`, background: C.green }} />
                <div style={{ flex: 1, background: '#EF4444' }} />
              </div>
              <div style={{ display: 'flex', gap: 18 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                    <div style={{ width: 9, height: 9, borderRadius: 3, background: C.green, flexShrink: 0 }} />
                    <span style={{ fontSize: 12.5, color: C.gray }}>Money in</span>
                  </div>
                  <p className="an-num" style={{ fontSize: 16, fontWeight: 700, color: C.dark }}>{NAIRA}{moneyIn.toLocaleString()}</p>
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                    <div style={{ width: 9, height: 9, borderRadius: 3, background: '#EF4444', flexShrink: 0 }} />
                    <span style={{ fontSize: 12.5, color: C.gray }}>You spent</span>
                  </div>
                  <p className="an-num" style={{ fontSize: 16, fontWeight: 700, color: C.dark }}>{NAIRA}{totalExpenses.toLocaleString()}</p>
                </div>
              </div>
              {totalExpenses === 0 && (
                <div style={{ background: C.orangeLight, borderRadius: 11, padding: '11px 13px', marginTop: 14 }}>
                  <p style={{ fontSize: 12.5, color: '#92400E', lineHeight: 1.5 }}>
                    You have not recorded any expenses yet, so this shows everything you earned. Record
                    what you spend on stock, transport and packaging to see what you are really keeping.
                  </p>
                </div>
              )}
            </div>
            ...
            {/* Key numbers */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 11, padding: '0 20px 22px' }}>
              {statCards.map((it, i) => (
                <div key={i} style={{ background: C.bg, borderRadius: 15, padding: '15px 14px' }}>
                  <it.Icon size={17} color={C.navy} style={{ marginBottom: 10 }} />
                  <p className="an-num" style={{ fontSize: 21, fontWeight: 800, color: C.dark, lineHeight: 1, marginBottom: 5 }}>{it.value}</p>
                  <p style={{ fontSize: 12.5, color: C.dark, fontWeight: 600 }}>{it.label}</p>
                  <p style={{ fontSize: 11.5, color: C.gray }}>{it.sub}</p>
                </div>
              ))}
            </div>

            {/* Needs attention - turns a report into something actionable */}
            {attentionItems.length > 0 && (
              <AnSection title="Needs your attention" tight>
                <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 9 }}>
                  {attentionItems.map((it, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 11, background: it.bg, borderRadius: 12, padding: '13px 14px' }}>
                      <it.Icon size={17} color={it.tone} style={{ flexShrink: 0 }} />
                      <p style={{ fontSize: 13.5, color: C.dark, fontWeight: 600, lineHeight: 1.4 }}>{it.text}</p>
                    </div>
                  ))}
                </div>
              </AnSection>
            ...
            {/* Conversion */}
            {isPro && periodVisits > 0 && (
              <AnSection title="Visitors who actually bought" note={`${periodOrders.length} online order${periodOrders.length !== 1 ? 's' : ''} from ${periodVisits.toLocaleString()} visit${periodVisits !== 1 ? 's' : ''}.`}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  <div style={{ flex: 1, height: 34, background: C.bg, borderRadius: 10, overflow: 'hidden' }}>
                    <div style={{ width: `${Math.min(Math.max(conversionRate, 2), 100)}%`, height: '100%', background: C.blue, borderRadius: 10 }} />
                  </div>
                  <p className="an-num" style={{ fontSize: 24, fontWeight: 800, color: C.dark, flexShrink: 0 }}>{conversionRate.toFixed(1)}%</p>
                </div>
                <p style={{ fontSize: 12.5, color: C.gray, marginTop: 12, lineHeight: 1.5 }}>
                  {conversionRate < 2
                    ? 'People are looking but not buying. Check your prices, photos and delivery options.'
                    : 'A healthy share of your visitors are buying. Keep sharing your store link.'}
                </p>
              </AnSection>
            )}

            {/* Online vs in person */}
            <AnSection title="Where your sales came from">
              {moneyIn === 0 ? (
                <AnEmpty text={`No sales recorded ${periodWord} yet.`} />
              ) : (() => {
                const onlinePct = Math.round((onlineRevenue / moneyIn) * 100);
                const R = 54, CIRC = 2 * Math.PI * R;
                return (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 20, flexWrap: 'wrap' as const }}>
                    <div style={{ position: 'relative' as const, width: 136, height: 136, flexShrink: 0 }}>
                      <svg viewBox="0 0 136 136" style={{ width: 136, height: 136, transform: 'rotate(-90deg)' }}>
                        <circle cx="68" cy="68" r={R} fill="none" stroke={C.green} strokeWidth="18" />
                        <circle cx="68" cy="68" r={R} fill="none" stroke={C.navy} strokeWidth="18" strokeDasharray={`${(onlinePct / 100) * CIRC} ${CIRC}`} />
                      </svg>
                      <div style={{ position: 'absolute' as const, inset: 0, display: 'flex', flexDirection: 'column' as const, alignItems: 'center', justifyContent: 'center' }}>
                        <p className="an-num" style={{ fontSize: 17, fontWeight: 800, color: C.dark, lineHeight: 1 }}>{NAIRA}{moneyIn >= 1000 ? `${Math.round(moneyIn / 1000)}k` : moneyIn}</p>
                        <p style={{ fontSize: 10.5, color: C.gray, marginTop: 2 }}>total</p>
                      </div>
                    </div>
                    <div style={{ flex: 1, minWidth: 130 }}>
                      <div style={{ marginBottom: 16 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 3 }}>
                          <div style={{ width: 10, height: 10, borderRadius: 3, background: C.navy, flexShrink: 0 }} />
                          <span style={{ fontSize: 13, color: C.gray }}>Your online store</span>
                        </div>
                        <p className="an-num" style={{ fontSize: 18, fontWeight: 700, color: C.dark }}>{NAIRA}{onlineRevenue.toLocaleString()}</p>
                        <p style={{ fontSize: 12, color: C.gray }}>{onlinePct}% of sales</p>
                      </div>
                      <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 3 }}>
                          <div style={{ width: 10, height: 10, borderRadius: 3, background: C.green, flexShrink: 0 }} />
                          <span style={{ fontSize: 13, color: C.gray }}>Sold in person</span>
                        </div>
                        <p className="an-num" style={{ fontSize: 18, fontWeight: 700, color: C.dark }}>{NAIRA}{offlineRevenue.toLocaleString()}</p>
                        <p style={{ fontSize: 12, color: C.gray }}>{100 - onlinePct}% of sales</p>
                      </div>
                    </div>
                  </div>
                );
            ...
            {period !== 'Today' && (
              <AnSection
                ...
                note={bestWeekday.value > 0 ? `${bestWeekday.day} earns the most. Worth posting new stock then.` : 'Once you have a few sales, your strongest day will show here.'}
              >
                <div style={{ display: 'flex', alignItems: 'flex-end', gap: 7, height: 128 }}>
                  {orderedWeekdays.map((d, i) => {
                    const isBest = bestWeekday.value > 0 && d.day === bestWeekday.day;
                    return (
                      <div key={d.day} style={{ flex: 1, display: 'flex', flexDirection: 'column' as const, alignItems: 'center', height: '100%' }}>
                        <div style={{ flex: 1, width: '100%', display: 'flex', alignItems: 'flex-end' }}>
                          <div className="an-bar" style={{ width: '100%', height: `${Math.max((d.value / maxWeekday) * 100, 4)}%`, borderRadius: '7px 7px 3px 3px', background: isBest ? C.navy : `${C.navy}26`, animationDelay: `${i * 60}ms` }} />
                        </div>
                        <span style={{ fontSize: 11, color: isBest ? C.dark : C.gray, fontWeight: isBest ? 700 : 500, marginTop: 8 }}>{d.day}</span>
                      </div>
                    );
                  })}
                </div>
            ...
            {/* Best earners */}
            <AnSection title="Your best earners" note="Ranked by money brought in, not units sold.">
              {bestSellers.length === 0 ? (
                <AnEmpty text={`No products sold ${periodWord} yet.`} />
              ) : bestSellers.map((p, i) => (
                <div key={i} style={{ marginBottom: i < bestSellers.length - 1 ? 15 : 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 12, marginBottom: 7 }}>
                    <p style={{ fontSize: 13.5, fontWeight: 600, color: C.dark, overflow: 'hidden', textOverflow: 'ellipsis' as const, whiteSpace: 'nowrap' as const }}>{p.name}</p>
                    <p className="an-num" style={{ fontSize: 13.5, fontWeight: 700, color: C.dark, flexShrink: 0 }}>{NAIRA}{p.revenue.toLocaleString()}</p>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ flex: 1, height: 8, background: C.bg, borderRadius: 5, overflow: 'hidden' }}>
                      <div style={{ width: `${(p.revenue / maxSellerRevenue) * 100}%`, height: '100%', background: i === 0 ? C.navy : `${C.navy}70`, borderRadius: 5 }} />
                    </div>
                    <span style={{ fontSize: 11.5, color: C.gray, flexShrink: 0, minWidth: 46, textAlign: 'right' as const }}>{p.qty} sold</span>
                  </div>
                </div>
              ))}
            </AnSection>
            ...
            {expenseEntries.length > 0 && (
              <AnSection title="Where your money went" note="Your recorded expenses for this period.">
                <div style={{ display: 'flex', height: 14, borderRadius: 8, overflow: 'hidden', marginBottom: 16 }}>
                  {expenseEntries.map(([name, v], i) => (
                    <div key={name} style={{ width: `${(v / totalExpenses) * 100}%`, background: CHART_COLORS[i % CHART_COLORS.length] }} />
                  ))}
                </div>
                {expenseEntries.map(([name, amount], i) => (
                  <div key={name} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '9px 0', borderBottom: i < expenseEntries.length - 1 ? `1px solid ${C.border}` : 'none' }}>
                    <div style={{ width: 10, height: 10, borderRadius: 3, background: CHART_COLORS[i % CHART_COLORS.length], flexShrink: 0 }} />
                    <span style={{ fontSize: 13.5, color: C.dark, fontWeight: 600, flex: 1 }}>{name}</span>
                    <span style={{ fontSize: 12, color: C.gray }}>{Math.round((amount / totalExpenses) * 100)}%</span>
                    <span className="an-num" style={{ fontSize: 13.5, fontWeight: 700, color: C.dark, minWidth: 68, textAlign: 'right' as const }}>{NAIRA}{amount.toLocaleString()}</span>
                  </div>
                ))}
              </AnSection>
            )}
            ...
            {/* Marketing channels */}
            <AnSection title="How customers found you" note="Only counts orders from your online store.">
              {!isPro ? (
                <div style={{ background: C.bg, borderRadius: 14, padding: 20, textAlign: 'center' as const }}>
                  <Lock size={20} color={C.gray} style={{ marginBottom: 8 }} />
                  <p style={{ fontSize: 13, color: C.gray, lineHeight: 1.5 }}>See exactly where your customers are coming from, WhatsApp, Instagram and more. Available on the Growth plan.</p>
                </div>
              ) : channelEntries.length === 0 ? (
                <AnEmpty text={`No online orders recorded ${periodWord} yet.`} />
              ) : (
                <>
                  <div style={{ display: 'flex', height: 14, borderRadius: 8, overflow: 'hidden', marginBottom: 16 }}>
                    {channelEntries.map(([name, v], i) => (
                      <div key={name} style={{ width: `${(v / channelTotal) * 100}%`, background: CHART_COLORS[i % CHART_COLORS.length] }} />
                    ))}
                  </div>
                  {channelEntries.map(([name, amount], i) => (
                    <div key={name} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '9px 0', borderBottom: i < channelEntries.length - 1 ? `1px solid ${C.border}` : 'none' }}>
                      <div style={{ width: 10, height: 10, borderRadius: 3, background: CHART_COLORS[i % CHART_COLORS.length], flexShrink: 0 }} />
                      <span style={{ fontSize: 13.5, color: C.dark, fontWeight: 600, flex: 1 }}>{name}</span>
                      <span style={{ fontSize: 12, color: C.gray }}>{Math.round((amount / channelTotal) * 100)}%</span>
                      <span className="an-num" style={{ fontSize: 13.5, fontWeight: 700, color: C.dark, minWidth: 68, textAlign: 'right' as const }}>{NAIRA}{amount.toLocaleString()}</span>
                    </div>
                  ))}
                </>
            ...
            {topCustomers.length > 0 && (
              <AnSection title="Your best customers" note="Worth treating well. Winning back someone who already trusts you costs nothing.">
                {topCustomers.map((c, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 0', borderBottom: i < topCustomers.length - 1 ? `1px solid ${C.border}` : 'none' }}>
                    <div style={{ width: 34, height: 34, borderRadius: '50%', background: C.navy, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <span style={{ color: 'white', fontSize: 13, fontWeight: 700 }}>{c.name.charAt(0).toUpperCase()}</span>
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontSize: 13.5, fontWeight: 600, color: C.dark, overflow: 'hidden', textOverflow: 'ellipsis' as const, whiteSpace: 'nowrap' as const }}>{c.name}</p>
                      <p style={{ fontSize: 11.5, color: C.gray }}>{c.orders} order{c.orders !== 1 ? 's' : ''}</p>
                    </div>
                    <p className="an-num" style={{ fontSize: 14, fontWeight: 700, color: C.dark, flexShrink: 0 }}>{NAIRA}{c.spent.toLocaleString()}</p>
                  </div>
                ))}
              </AnSection>
            )}
            ...
            <AnSection
              title="Not selling"
              note={noMovers.length > 0 ? `These are in your store but nobody bought them ${periodWord}. Try a discount, or bundle them with something popular.` : undefined}
            >
              {noMovers.length === 0 ? (
                <AnEmpty text="Every active product has sold at least once in this period." />
              ) : noMovers.map(p => (
                <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '12px 0', borderBottom: `1px solid ${C.border}` }}>
...
function InvoiceScreen({ onBack }: { onBack: () => void }) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      // Firestore rejects any field whose value is literally undefined, so optional fields have
      // to be left out of the object entirely rather than set to undefined. Spreading them in
      // conditionally is what keeps a blank phone or note from failing the whole save.
      const inv: any = {
      ...
      setError(`Could not create invoice: ${err?.code || err?.message || 'unknown error'}`);
  ...
  if (view === 'detail' && selected) return (
    <div style={{ background: C.bg, minHeight: '100vh', fontFamily: 'Inter, sans-serif', paddingBottom: 100 }}>
      ...
      <div style={{ background: C.white, borderRadius: '24px 24px 0 0', marginTop: -16, padding: 20, minHeight: 'calc(100vh - 90px)' }}>
        ...
        {(() => {
          // Built to read like a real payment receipt - a checkmark and clear status line up
          // top, a labelled "Bill To", each item lined up against its price, then a divider
          // before the total - rather than a flat dump of numbers with no visual structure.
          const itemLines = selected.items
            .map(i => {
              const lineTotal = `${NAIRA}${(i.quantity * i.unitPrice).toLocaleString()}`;
              return `${i.description} × ${i.quantity}\n${lineTotal}`;
            })
            .join('\n\n');
          const total = selected.items.reduce((s, i) => s + i.quantity * i.unitPrice, 0);
          const statusLine = selected.paid ? '✅ *PAID*' : '🕒 *PENDING PAYMENT*';
          const formattedDate = new Date(selected.createdAt).toLocaleDateString('en-NG', { day: 'numeric', month: 'short', year: 'numeric' });
          const invoiceText =
            `*${user?.storeName || 'Invoice'}*\n` +
            `Invoice ${selected.invoiceNumber}\n` +
            `${formattedDate}\n` +
            `━━━━━━━━━━━━━━\n\n` +
            `*Bill To:* ${selected.customerName}\n\n` +
            `${itemLines}\n\n` +
            `━━━━━━━━━━━━━━\n` +
            `*Total: ${NAIRA}${total.toLocaleString()}*\n` +
            `${statusLine}` +
            (!selected.paid && selected.dueDate ? `\nDue ${new Date(selected.dueDate).toLocaleDateString('en-NG', { day: 'numeric', month: 'short', year: 'numeric' })}` : '') +
            (selected.note ? `\n\n${selected.note}` : '') +
      ...
      <div style={{ background: C.white, borderRadius: '24px 24px 0 0', marginTop: -16, padding: 20, minHeight: 'calc(100vh - 90px)' }}>
        ...
        {allSegments.length > 0 && (
          <div style={{ display: 'flex', gap: 8, overflowX: 'auto' as const, paddingBottom: 4, marginBottom: 16 }}>
            <button onClick={() => setActiveSegmentFilter(null)} style={{ background: !activeSegmentFilter ? C.navy : C.white, color: !activeSegmentFilter ? 'white' : C.dark, border: !activeSegmentFilter ? 'none' : `1.5px solid ${C.border}`, borderRadius: 10, padding: '8px 14px', fontSize: 12, fontWeight: !activeSegmentFilter ? 700 : 600, cursor: 'pointer', whiteSpace: 'nowrap' as const, flexShrink: 0, boxShadow: !activeSegmentFilter ? '0 3px 8px rgba(20,42,69,0.25)' : 'none', transition: 'all 0.15s ease' }}>All</button>
            {allSegments.map(s => (
              <button key={s} onClick={() => setActiveSegmentFilter(s)} style={{ background: activeSegmentFilter === s ? C.navy : C.white, color: activeSegmentFilter === s ? 'white' : C.dark, border: activeSegmentFilter === s ? 'none' : `1.5px solid ${C.border}`, borderRadius: 10, padding: '8px 14px', fontSize: 12, fontWeight: activeSegmentFilter === s ? 700 : 600, cursor: 'pointer', whiteSpace: 'nowrap' as const, flexShrink: 0, boxShadow: activeSegmentFilter === s ? '0 3px 8px rgba(20,42,69,0.25)' : 'none', transition: 'all 0.15s ease' }}>{s}</button>
            ))}
          </div>
        )}
...
function MoreScreen({ logout, user, onNavigate }: any) {
  ...
  if (subActive) {
    ...
    const elapsedDays = Math.floor((Date.now() - new Date(user.trialStart).getTime()) / (1000 * 60 * 60 * 24));
    const daysLeft = Math.max(TRIAL_DAYS - elapsedDays, 0);
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100vh', paddingBottom: 80, fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ background: C.white, borderRadius: '24px 24px 0 0', marginTop: -20, padding: 20 }}>
        ...
        {/* Nobody who has already paid for everything should be nagged to upgrade. That means
            branded clients who bought outright, and the platform owner's own account - which
            also keeps the prompt out of screen recordings and demos. */}
        {!user?.whiteLabel && !isOwner(user) && (
...
// Reads the real local time and returns the greeting that actually matches it, instead of a
// fixed "Good morning" that was showing at every hour regardless of when someone opened the app.
...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const messaging = getMessaging();
        // navigator.serviceWorker.ready resolves to whichever service worker is actually
        // controlling this page right now - unlike getRegistration(path), which expects a
        // page URL to match against a registration's scope, not a script filename. Since the
        // service worker registers with no explicit scope (defaulting to the whole site), a
        // lookup keyed on the script's own path never actually matched, silently leaving
        // registration empty and skipping showNotification() every time, with no error.
  ...
  if (isTrialExpiredUnpaid(user) && moreView !== 'subscription') {
    return (
      <div style={{ minHeight: '100vh', background: C.bg, fontFamily: 'Inter, sans-serif', display: 'flex', flexDirection: 'column' as const, alignItems: 'center', justifyContent: 'center', padding: 24, textAlign: 'center' as const }}>
        <div style={{ width: 64, height: 64, borderRadius: '50%', background: C.orangeLight, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 18 }}>
          <Clock size={30} color={C.orange} />
        </div>
        <h2 style={{ fontSize: 20, fontWeight: 800, color: C.dark, marginBottom: 8 }}>Your free trial has ended</h2>
        <p style={{ fontSize: 14, color: C.gray, marginBottom: 28, maxWidth: 280, lineHeight: 1.5 }}>
          Upgrade now to get back into your dashboard and bring your storefront back online for
          customers.
        </p>
        <button onClick={() => setMoreView('subscription')} style={{ background: C.navy, color: 'white', border: 'none', borderRadius: 14, padding: '15px 32px', fontSize: 15, fontWeight: 700, cursor: 'pointer', marginBottom: 14 }}>
          Upgrade Now
        </button>
        <button onClick={logout} style={{ background: 'none', border: 'none', color: C.gray, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
          Log out
        </button>
      </div>
    );
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: 'Inter, sans-serif' }}>
      <div style={{ background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, padding: '48px 24px 80px', textAlign: 'center' as const }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: 14 }}>Start your 7-day free trial today</p>
      </div>
      <div style={{ background: C.white, borderRadius: '28px 28px 0 0', marginTop: -28, padding: '28px 24px', minHeight: '60vh' }}>
        <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
          {error && <div style={{ background: '#FEE2E2', color: '#EF4444', borderRadius: 12, padding: '12px 14px', fontSize: 13, fontWeight: 600, wordBreak: 'break-word' as const, overflowWrap: 'break-word' as const, maxWidth: '100%', boxSizing: 'border-box' as const }}>{error}</div>}
          ...
          {/* When a code arrives automatically from a shared link, showing it as an editable text
              box makes people think something's wrong and delete it - the field is only shown
              for someone typing a code in themselves (e.g. one told to them in a video), since
              in that case they need to see what they're entering. */}
          {!getReferralCode() && (
            <div>
...
// The password reset link in the email points here instead of Firebase's own hosted page at
// authDomain. Firebase's oobCode is verified and consumed directly through the SDK's own API
// calls (identitytoolkit.googleapis.com) - those calls don't depend on authDomain resolving at
// all, only the authorized-domains list (which salespilot.com.ng is already on). This means a
// merchant can reset their password even while the authDomain itself is unreachable, which is
// exactly the situation that locked people out before this page existed.
function ResetPasswordPage() {
  ...
  useEffect(() => {
    ...
    verifyPasswordResetCode(auth, oobCode)
      .then(accountEmail => { setEmail(accountEmail); setStatus('ready'); })
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      if (err.code === 'auth/expired-action-code') setError('This reset link has expired. Request a new one.');
      else if (err.code === 'auth/weak-password') setError('Please choose a stronger password.');
      else setError('Could not reset your password. Please request a new link and try again.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: 'Inter, sans-serif' }}>
      <div style={{ background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, padding: '48px 24px 80px', textAlign: 'center' as const }}>
        ...
        {status === 'ready' && <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: 14 }}>for {email}</p>}
      </div>
      <div style={{ background: C.white, borderRadius: '28px 28px 0 0', marginTop: -28, padding: '28px 24px', minHeight: '60vh' }}>
        {status === 'checking' && (
          <p style={{ textAlign: 'center' as const, color: C.gray, fontSize: 14, padding: '40px 0' }}>Checking your link...</p>
        )}

        {status === 'invalid' && (
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
            <div style={{ background: '#FEE2E2', color: '#EF4444', borderRadius: 12, padding: '14px 16px', fontSize: 13.5, fontWeight: 600, lineHeight: 1.5 }}>
              This link is invalid or has expired. Go back and request a new password reset email.
            </div>
            <button style={{ background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, color: 'white', border: 'none', borderRadius: 14, padding: 16, fontSize: 16, fontWeight: 700, cursor: 'pointer' }} onClick={() => navigate('/login')}>Back to sign in</button>
          </div>
        )}
        ...
        {status === 'ready' && (
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
            {error && <div style={{ background: '#FEE2E2', color: '#EF4444', borderRadius: 12, padding: '12px 14px', fontSize: 13, fontWeight: 600 }}>{error}</div>}
            <div>
              <label style={{ fontSize: 13, fontWeight: 600, color: C.dark, display: 'block', marginBottom: 8 }}>New password</label>
              <div style={{ position: 'relative' as const }}>
                <input style={inp} type={showPassword ? 'text' : 'password'} placeholder="At least 6 characters" value={password} onChange={e => setPassword(e.target.value)} />
                <button onClick={() => setShowPassword(s => !s)} style={{ position: 'absolute' as const, right: 14, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: C.gray }}>
                  {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
                </button>
              </div>
            </div>
            <div>
              <label style={{ fontSize: 13, fontWeight: 600, color: C.dark, display: 'block', marginBottom: 8 }}>Confirm new password</label>
              <input style={inp} type={showPassword ? 'text' : 'password'} placeholder="Re-enter your new password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} />
            </div>
            <button disabled={saving} style={{ background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, color: 'white', border: 'none', borderRadius: 14, padding: 16, fontSize: 16, fontWeight: 700, cursor: 'pointer', opacity: saving ? 0.7 : 1 }} onClick={handleReset}>{saving ? 'Saving...' : 'Set new password'}</button>
          </div>
        )}
        ...
        {status === 'done' && (
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
            ...
            <button style={{ background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, color: 'white', border: 'none', borderRadius: 14, padding: 16, fontSize: 16, fontWeight: 700, cursor: 'pointer' }} onClick={() => navigate('/login')}>Sign in</button>
          </div>
        )}
      </div>
    </div>
...
const PLANS = [
  ...
  { id: 'pro', name: 'Growth', desc: 'Built for growing businesses', prices: { quarterly: 15000, biannual: 27500, annual: 50000 }, features: ['Everything included in Starter', 'Staff accounts with role based permissions', 'Customer segments and groups', 'Sales by Marketing Channel analytics', 'Website visit tracking', 'Facebook Pixel and Google Analytics integration', 'Full access to all 32 brand themes', 'Priority customer support'], popular: true },
];
...
function SubscriptionScreen({ onBack }: { onBack: () => void }) {
  ...
  const displayPrice = activePlan.prices[billing];
        ...
        {step === 3 && (
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
              ...
              <div><label style={{ fontSize: 13, fontWeight: 600, color: C.dark, display: 'block', marginBottom: 8 }}>Quantity in stock <span style={{ color: C.gray, fontWeight: 400 }}>(optional)</span></label><input style={inp} type="number" placeholder="e.g. 10" value={prodQty} onChange={e => setProdQty(e.target.value)} /><p style={{ fontSize: 11, color: C.gray, marginTop: 6 }}>Track profit and know when to restock.</p></div>
        ...
        {step === 4 && (
          <div>
            ...
            <p style={{ fontSize: 12.5, color: C.gray, textAlign: 'center' as const, marginBottom: 20, lineHeight: 1.5 }}>
              Not sure yet? Every plan includes a free 7 day trial, no payment required to get started.
            </p>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              ...
              <button style={{ flex: 1, background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, color: 'white', border: 'none', borderRadius: 14, padding: 16, fontSize: 15, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }} onClick={async () => { await updateProfile({ plan: planTab, billing, businessStage: answers, onboardingComplete: true, trialStart: new Date().toISOString() }); (window as any).fbq?.('track', 'CompleteRegistration'); setStep(5); }}>Start free trial <ArrowRight size={16} /></button>
            </div>
          </div>
        )}
        {step === 5 && (
          <div style={{ textAlign: 'center' as const, paddingTop: 40, position: 'relative' as const, overflow: 'hidden' }}>
            {/* A genuine burst of falling confetti behind the message, rather than a single
                static icon - this is the one moment in the whole signup flow where the merchant
                should feel like something real just happened, not like a form was submitted. */}
            <div style={{ position: 'absolute' as const, inset: 0, pointerEvents: 'none' as const, overflow: 'hidden' }}>
              {Array.from({ length: 32 }).map((_, i) => {
                const colors = [C.green, C.navy, '#F59E0B', '#EC4899', '#8B5CF6', '#06B6D4'];
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      animation: `sp-confetti-fall ${duration}s ease-in ${delay}s both`,
                    }}
                  />
                );
              })}
            ...
            <div style={{ width: 80, height: 80, background: C.greenLight, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px', position: 'relative' as const, animation: 'sp-pop-in 0.5s cubic-bezier(0.34, 1.56, 0.64, 1) both' }}><Rocket size={36} color={C.green} /></div>
            <style>{`
              @keyframes sp-confetti-fall {
                0% { transform: translateY(0) translateX(0) rotate(0deg); opacity: 1; }
                100% { transform: translateY(420px) translateX(var(--drift, 20px)) rotate(540deg); opacity: 0; }
              }
              @keyframes sp-pop-in {
                0% { transform: scale(0); }
            ...
            <p style={{ color: C.gray, marginBottom: 28, fontSize: 14 }}>Your 7 day free trial has started, and your first product is already in your inventory.</p>
            {sellsOnline ? (
...
// Design tokens for the public marketing page. The palette is pulled straight from the real
// product (the same navy and green a merchant sees inside the app every day) so the page and
// the thing it's selling feel like one product, not an ad for a different one. The cool grey
// ground is deliberate - a warm cream page would fight the screenshots, which are the most
// persuasive thing here and need to sit on something neutral.
const L = {
  ink: '#0B1B2B',
  navy: '#142A45',
  green: '#10B981',
...
function LandingFonts() {
  return (
    <style>{`
      ...
      .lp-d { font-family: 'Bricolage Grotesque', 'Inter', sans-serif; letter-spacing: -0.02em; }
      .lp-b { font-family: 'Inter', sans-serif; }
      @keyframes lp-bubble { from { opacity: 0; transform: translateY(12px) scale(0.96); } to { opacity: 1; transform: translateY(0) scale(1); } }
      @keyframes lp-fade { from { opacity: 0; transform: translateY(16px); } to { opacity: 1; transform: translateY(0); } }
      .lp-shot { box-shadow: 0 24px 60px rgba(11,27,43,0.18), 0 2px 8px rgba(11,27,43,0.08); }
      @media (prefers-reduced-motion: reduce) {
...
// The signature device of the page: the merchant's own inbox, rendered back at them. These are
// the real questions Nigerian sellers field all day, in the exact words customers use. Nothing
// here imitates any specific messaging app - they're plain bubbles, so the recognition comes
// from the words themselves rather than from copying someone else's interface.
function ChatBubble({ text, delay }: { text: string; delay: number }) {
  return (
    <div style={{
      alignSelf: 'flex-start', maxWidth: '84%',
      background: 'white', border: `1px solid ${L.line}`,
      borderRadius: '16px 16px 16px 4px', padding: '11px 15px',
      fontSize: 14.5, color: L.ink, lineHeight: 1.45,
      boxShadow: '0 1px 2px rgba(11,27,43,0.05)',
      animation: `lp-bubble 0.5s cubic-bezier(0.22,1,0.36,1) ${delay}s both`,
    }}>{text}</div>
  );
}

function Section({ children, dark, tint }: { children: any; dark?: boolean; tint?: boolean }) {
  return (
    <div style={{ background: dark ? L.ink : tint ? L.mist : 'white', padding: '64px 20px' }}>
      <div style={{ maxWidth: 620, margin: '0 auto' }}>{children}</div>
    </div>
  );
}

function Eyebrow({ children, onDark }: { children: any; onDark?: boolean }) {
  return (
    <p className="lp-b" style={{ fontSize: 11.5, fontWeight: 700, letterSpacing: 1.4, textTransform: 'uppercase' as const, color: onDark ? L.green : L.greenDeep, marginBottom: 14 }}>{children}</p>
  );
}

function H2({ children, onDark }: { children: any; onDark?: boolean }) {
  return (
    <h2 className="lp-d" style={{ fontSize: 'clamp(26px, 6vw, 34px)', fontWeight: 800, lineHeight: 1.15, color: onDark ? 'white' : L.ink, marginBottom: 18 }}>{children}</h2>
  );
}

function P({ children, onDark }: { children: any; onDark?: boolean }) {
  return (
    <p className="lp-b" style={{ fontSize: 16, lineHeight: 1.68, color: onDark ? 'rgba(255,255,255,0.72)' : L.body, marginBottom: 16 }}>{children}</p>
  );
}

function Shot({ url, caption }: { url: string; caption: string }) {
  return (
    <div style={{ margin: '30px 0 6px' }}>
      <img src={url} alt={caption} loading="lazy" className="lp-shot" style={{ width: '100%', borderRadius: 18, display: 'block', border: `1px solid ${L.line}` }} />
      <p className="lp-b" style={{ fontSize: 12.5, color: L.body, textAlign: 'center' as const, marginTop: 12 }}>{caption}</p>
    </div>
  );
}

function TickList({ items, onDark }: { items: string[]; onDark?: boolean }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 13, marginTop: 6 }}>
      {items.map((t, i) => (
        <div key={i} style={{ display: 'flex', gap: 11, alignItems: 'flex-start' }}>
          <CheckCircle2 size={18} color={L.green} style={{ flexShrink: 0, marginTop: 2 }} />
          <span className="lp-b" style={{ fontSize: 15.5, lineHeight: 1.55, color: onDark ? 'rgba(255,255,255,0.86)' : L.ink }}>{t}</span>
        </div>
      ))}
    </div>
  );
...
function CTAButton({ onClick, label, big }: { onClick: () => void; label: string; big?: boolean }) {
  return (
    <button onClick={onClick} className="lp-b" style={{
      background: L.green, color: 'white', border: 'none', borderRadius: 13,
      padding: big ? '17px 34px' : '14px 26px', fontSize: big ? 16.5 : 15, fontWeight: 700,
...
export function LandingPage() {
  ...
  const FAQS = [
    { q: 'Do I need a laptop?', a: 'No. Everything runs on your phone, from adding products to recording sales.' },
    { q: 'Do I need CAC registration?', a: 'No. You can start selling today without registering a company.' },
    { q: 'What if I only sell in person?', a: 'Still works. Turn the online store off and use the POS, stock tracking and profit reports on their own.' },
    { q: 'Do you take a percentage of my sales?', a: 'No. Customers pay into your own bank account. We never touch your money and we take no cut.' },
    { q: 'How long does setup take?', a: 'Most sellers have their store live the same day. Add your products, pick a theme, share your link.' },
    { q: 'Can I cancel?', a: 'Yes, anytime. Nothing is locked in.' },
  ];

  return (
    <div style={{ background: 'white', minHeight: '100vh', overflowX: 'hidden' as const }} className="lp-b">
      <LandingFonts />

      {/* Header */}
      <div style={{ position: 'sticky' as const, top: 0, zIndex: 40, background: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(10px)', borderBottom: `1px solid ${L.line}` }}>
        <div style={{ maxWidth: 620, margin: '0 auto', padding: '12px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <img src={SPLASH_GRAPHIC_URL} alt="SalesPilot" style={{ height: 30, width: 'auto', maxWidth: 84, objectFit: 'contain' as const, display: 'block' }} />
            <span className="lp-d" style={{ fontSize: 17, fontWeight: 800, color: L.ink }}>SalesPilot</span>
          </div>
          <button onClick={() => navigate('/signup')} className="lp-b" style={{ background: L.ink, color: 'white', border: 'none', borderRadius: 10, padding: '9px 16px', fontSize: 13.5, fontWeight: 600, cursor: 'pointer' }}>
            Get Started
          </button>
        </div>
      </div>
      ...
      <div style={{ padding: '56px 20px 12px' }}>
        <div style={{ maxWidth: 620, margin: '0 auto', animation: 'lp-fade 0.8s cubic-bezier(0.22,1,0.36,1) both' }}>
          <Eyebrow>Built for Nigerian sellers</Eyebrow>
          <h1 className="lp-d" style={{ fontSize: 'clamp(34px, 8.5vw, 52px)', fontWeight: 800, lineHeight: 1.06, color: L.ink, marginBottom: 20 }}>
            You're losing sales inside your own DMs.
            <span style={{ display: 'block', color: L.body, fontWeight: 500, fontSize: 'clamp(20px, 4.5vw, 28px)', marginTop: 12, lineHeight: 1.25 }}>
              And you'd never know it.
            </span>
          </h1>
          <p className="lp-b" style={{ fontSize: 17, lineHeight: 1.65, color: L.body, marginBottom: 28 }}>
            Give your customers a real store instead. Sell online and in person, know what you
            actually made, and get paid straight into your own bank account.
          </p>
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' as const, alignItems: 'center' }}>
            <CTAButton onClick={() => navigate('/signup')} label="Start free for 7 days" big />
            <span className="lp-b" style={{ fontSize: 13.5, color: L.body }}>No card needed</span>
          </div>
        </div>
      </div>

      {/* The signature: their own inbox, read back to them */}
      <Section tint>
        <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 9, marginBottom: 26 }}>
          <ChatBubble text="Hi, is this still available?" delay={0} />
          <ChatBubble text="How much is the blue one?" delay={0.12} />
          <ChatBubble text="Do you have it in size M?" delay={0.24} />
          <ChatBubble text="Last price?" delay={0.36} />
          <ChatBubble text="Hello??" delay={0.48} />
        </div>
        <H2>Every day. The same five questions.</H2>
        <P>
          By afternoon you have had twenty conversations and made two sales. The rest asked, you
          answered, and they went quiet. It is not that you are slow. You are running a whole
          business inside a chat app.
        </P>
      </Section>
      ...
      <Section>
        ...
        <H2>The parts nobody talks about.</H2>
        <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 0, marginTop: 22 }}>
          {[
            'A real order gets buried under twenty other messages',
            'You sold the last one in the shop, then someone ordered it online',
            'Money enters your account, but you are guessing what is actually profit',
            'Someone wanted to buy at 2am. You were asleep. That sale never happened',
            'Your records are your memory and a notebook',
            'You cannot let anyone help you, because everything lives in your phone',
          ].map((t, i) => (
            <div key={i} style={{ display: 'flex', gap: 13, padding: '15px 0', borderBottom: i < 5 ? `1px solid ${L.line}` : 'none', alignItems: 'flex-start' }}>
              <div style={{ width: 6, height: 6, borderRadius: '50%', background: L.amber, flexShrink: 0, marginTop: 8 }} />
              <span className="lp-b" style={{ fontSize: 15.5, lineHeight: 1.55, color: L.ink }}>{t}</span>
            </div>
          ))}
        </div>
      </Section>

      {/* The payoff - storefront screenshot */}
      <Section dark>
        <Eyebrow onDark>The alternative</Eyebrow>
        <H2 onDark>Or you could just send them this.</H2>
        <P onDark>
          One link. Every product, every price, every size and colour already there. They browse,
          they choose, they order. You did not type a single message.
        </P>
        <Shot url="https://res.cloudinary.com/xd2hkwf8/image/upload/v1786318280/file_00000000175c8243ac84c92fb7ab7b05_vt12fj.png" caption="A real SalesPilot storefront" />
      </Section>

      {/* Bigger baskets */}
      <Section tint>
        <Eyebrow>The part most sellers miss</Eyebrow>
        <H2>One customer came for a shirt. They left with ten.</H2>
        <P>
          Think about what actually happens in your DMs. Someone asks about one shirt. You send
          the price. They ask the size. You send another photo. Back and forth, and eventually
          they buy that one shirt.
        </P>
        <P>
          They never saw the other things you sell. You were too busy answering questions to show
          them.
        </P>
        <P>
          Now imagine they open your store instead. They came for one thing, spotted five more
          they liked, and added them all without asking you anything.
        </P>
        <p className="lp-d" style={{ fontSize: 19, fontWeight: 700, color: L.ink, lineHeight: 1.4, marginTop: 22, paddingLeft: 16, borderLeft: `3px solid ${L.green}` }}>
          That is the difference between a chat and a shop.
        </p>
      </Section>

      {/* Trust */}
      <Section>
        <Eyebrow>Why some buyers go quiet</Eyebrow>
        <H2>Serious buyers are checking if you are real.</H2>
        <P>
          Enough people have been burned that buyers now hesitate before sending money to anyone
          they cannot verify. It is why honest sellers end up screenshotting bank alerts and old
          chats, just to prove they are genuine.
        </P>
        <P>
          A real store link does that work for you. It shows your products, your name, your
          contact, your reviews. Nobody has to take your word for anything.
        </P>
        <div style={{ background: L.mist, borderRadius: 14, padding: '18px 20px', marginTop: 22 }}>
          <p className="lp-b" style={{ fontSize: 14.5, color: L.ink, lineHeight: 1.6 }}>
            Studies have found roughly <strong>75% of people judge how credible a business is from
            its website alone</strong>. Global research, but the instinct is the same everywhere.
          </p>
        </div>
      </Section>
      ...
      {/* Keep WhatsApp */}
      <Section tint>
        <Eyebrow>You are not giving anything up</Eyebrow>
        <H2>Keep your WhatsApp. This just gives you something to send.</H2>
        <P>
          We are not replacing how you talk to customers. WhatsApp is where the trust gets built,
          and that is not changing.
        </P>
        <P>
          The only difference is what you send them. Instead of retyping prices and re-sending the
          same photos thirty times a day, you send one link and let them look properly.
        </P>
      </Section>

      {/* Online + in person */}
      <Section>
        <Eyebrow>Both sides of your business</Eyebrow>
        <H2>Sell in the shop. Sell online. Same stock, same numbers.</H2>
        <P>
          Most tools make you pick one. Real businesses here do both, a shop people walk into and
          customers who order from their phone.
        </P>
        <P>
          Sell something at the counter and your online stock drops. Someone orders online and
          your shelf count updates. No double-selling, and no apologising to a customer because
          something was actually finished.
        </P>
        <Shot url="https://res.cloudinary.com/xd2hkwf8/image/upload/v1786318347/file_00000000351481f49b83ef1d4a33057c_awhben.png" caption="Your dashboard, online and in-person sales together" />
      </Section>

      {/* Offline-only sellers */}
      <Section tint>
        <Eyebrow>No website? Still for you</Eyebrow>
        <H2>Do not want to sell online at all? You still need this.</H2>
        <P>
          Maybe you have a shop and that is exactly how you like it. No website, no online orders,
          no stress. Fine. Switch the storefront off and use everything else.
        </P>
        <TickList items={[
          'Record every sale as it happens at your counter',
          'Know exactly what is left without counting by hand',
          'See real profit, not just money that came in',
          'Scan barcodes at checkout instead of searching',
          'Send proper invoices to customers who want one',
          'Get alerted when something is running low, before it finishes',
          'Add your staff so they can serve customers without seeing your money',
        ]} />
        <Shot url="https://res.cloudinary.com/xd2hkwf8/image/upload/v1786318466/file_00000000421081f4aaef3027f28aadbb_qr8rwy.png" caption="Real numbers, not guesses" />
      </Section>

      {/* Money */}
      <Section dark>
        <Eyebrow onDark>About your money</Eyebrow>
        <H2 onDark>Your money goes to your bank. Not ours.</H2>
        <P onDark>
          Customers pay you directly. We never hold your money, never release it on a schedule,
          and never take a percentage of what you sell.
        </P>
        <TickList onDark items={[
          'Customers pay straight into your own account',
          'No platform fees. You keep 100% of every sale',
          'No waiting for anyone to pay you out',
          'Every payment attached to a real order record',
        ]} />
      </Section>

      {/* Cost objection */}
      <Section>
        <Eyebrow>The real reason people do not have one</Eyebrow>
        <H2>A proper store should not cost ₦800,000.</H2>
        <P>
          Most people do not skip having a store because they do not want one. They skip it
          because they think it needs a developer, a laptop, and hundreds of thousands of naira
          up front, then more every year to keep it alive.
        </P>
        <P>
          That is what we set out to remove.
        </P>
        <TickList items={[
          'No developer, no laptop, no technical skill',
          'No CAC registration needed',
          'Runs entirely on the phone you already have',
          'Set up today, not in six weeks',
        ]} />
      </Section>
      ...
      <Section tint>
        ...
        <H2>Everything, already built in.</H2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 12, marginTop: 22 }}>
          {[
            { Icon: Store, t: 'Your own store link' },
            { Icon: CreditCard, t: 'Point of Sale' },
            { Icon: Package, t: 'Stock that syncs itself' },
            { Icon: TrendingUp, t: 'Real profit tracking' },
            { Icon: Camera, t: 'Barcode scanning' },
            { Icon: FileText, t: 'Branded invoices' },
            { Icon: Tag, t: 'Coupons and bundles' },
            { Icon: Truck, t: 'Delivery at checkout' },
            { Icon: Star, t: 'Customer reviews' },
            { Icon: Users, t: 'Staff accounts' },
            { Icon: Bell, t: 'Instant order alerts' },
            { Icon: Sparkles, t: '32 store themes' },
          ].map((f, i) => (
            <div key={i} style={{ background: 'white', border: `1px solid ${L.line}`, borderRadius: 13, padding: '15px 14px' }}>
              <f.Icon size={18} color={L.greenDeep} />
              <p className="lp-b" style={{ fontSize: 13.5, fontWeight: 600, color: L.ink, marginTop: 9, lineHeight: 1.35 }}>{f.t}</p>
            </div>
          ))}
        </div>
      </Section>
      ...
      <Section>
        ...
        <H2>Pay every three months. Not every month.</H2>
        <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 14, marginTop: 24 }}>
          <div style={{ border: `1px solid ${L.line}`, borderRadius: 16, padding: 22 }}>
            <p className="lp-d" style={{ fontSize: 15, fontWeight: 700, color: L.ink }}>Starter</p>
            <p className="lp-d" style={{ fontSize: 32, fontWeight: 800, color: L.ink, margin: '6px 0 2px' }}>{NAIRA}7,500</p>
            <p className="lp-b" style={{ fontSize: 13, color: L.body, marginBottom: 16 }}>every 3 months</p>
            <TickList items={['Your online storefront', 'Point of Sale and invoices', 'Inventory and profit tracking', 'Coupons, bundles, reviews', 'Delivery options at checkout']} />
          </div>
          <div style={{ border: `2px solid ${L.green}`, borderRadius: 16, padding: 22, position: 'relative' as const }}>
            <span className="lp-b" style={{ position: 'absolute' as const, top: -10, left: 22, background: L.green, color: 'white', fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: 20 }}>MOST POPULAR</span>
            <p className="lp-d" style={{ fontSize: 15, fontWeight: 700, color: L.ink }}>Growth</p>
            <p className="lp-d" style={{ fontSize: 32, fontWeight: 800, color: L.ink, margin: '6px 0 2px' }}>{NAIRA}15,000</p>
            <p className="lp-b" style={{ fontSize: 13, color: L.body, marginBottom: 16 }}>every 3 months</p>
            <TickList items={['Everything in Starter', 'Staff accounts with permissions', 'Customer segments and groups', 'Full analytics and tracking', 'All 32 brand themes', 'Priority support']} />
          </div>
        </div>
        <p className="lp-b" style={{ fontSize: 14, color: L.body, textAlign: 'center' as const, marginTop: 20 }}>
          Both start with 7 days free. No card needed.
        </p>
      </Section>
      ...
      <Section tint>
        ...
        <div style={{ marginTop: 20 }}>
          {FAQS.map((f, i) => (
            <div key={i} style={{ borderBottom: `1px solid ${L.line}` }}>
              <button onClick={() => setOpenFaq(openFaq === i ? null : i)} className="lp-b" style={{ width: '100%', background: 'none', border: 'none', padding: '17px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, cursor: 'pointer', textAlign: 'left' as const }}>
                <span style={{ fontSize: 15.5, fontWeight: 600, color: L.ink }}>{f.q}</span>
                <ChevronDown size={17} color={L.body} style={{ flexShrink: 0, transform: openFaq === i ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }} />
              </button>
              {openFaq === i && (
                <p className="lp-b" style={{ fontSize: 15, color: L.body, lineHeight: 1.6, padding: '0 0 18px' }}>{f.a}</p>
              )}
            </div>
          ))}
        </div>
      ...
      <Section dark>
        <div style={{ textAlign: 'center' as const }}>
          <h2 className="lp-d" style={{ fontSize: 'clamp(28px, 7vw, 38px)', fontWeight: 800, lineHeight: 1.15, color: 'white', marginBottom: 16 }}>
            Stop running your business from your inbox.
          </h2>
          <p className="lp-b" style={{ fontSize: 16, color: 'rgba(255,255,255,0.7)', marginBottom: 28, lineHeight: 1.6 }}>
            7 days free. No card needed. Your store can be live today.
          </p>
          <CTAButton onClick={() => navigate('/signup')} label="Create my store" big />
        </div>
      </Section>

      {/* Footer */}
      <div style={{ background: L.ink, borderTop: '1px solid rgba(255,255,255,0.08)', padding: '22px 20px', textAlign: 'center' as const }}>
        <p className="lp-b" style={{ fontSize: 12.5, color: 'rgba(255,255,255,0.45)' }}>
          &copy; {new Date().getFullYear()} SalesPilot &middot; {SUPPORT_EMAIL}
        </p>
      </div>
    </div>
  );
...
export function ReferralStatsPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FFFFFF', fontFamily: 'Inter, sans-serif', display: 'flex', flexDirection: 'column' as const, alignItems: 'center', padding: '48px 20px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 32 }}>
        <img src={SPLASH_GRAPHIC_URL} alt="SalesPilot" style={{ height: 32, width: 'auto', maxWidth: 90, objectFit: 'contain' as const, display: 'block' }} />
...
const ownerInput = { width: '100%', padding: '10px 12px', borderRadius: 9, border: `1.5px solid ${C.border}`, fontSize: 13, outline: 'none', boxSizing: 'border-box' as const, marginBottom: 12, background: C.white, color: C.dark };
const ownerLabel = { fontSize: 11.5, fontWeight: 700, color: C.gray, display: 'block' as const, marginBottom: 6, textTransform: 'uppercase' as const, letterSpacing: 0.4 };
const ownerAddBtn = { background: C.white, border: `1px dashed ${C.border}`, borderRadius: 9, padding: '10px 13px', fontSize: 12.5, fontWeight: 700, color: C.navy, cursor: 'pointer', marginBottom: 16, width: '100%', textAlign: 'left' as const };
...
function OwnerDashboardScreen({ onBack }: { onBack: () => void }) {
  ...
  // The dashboard's data is a one-time fetch, not a live listener - it would otherwise show
  // whatever was loaded when the screen first mounted, even hours later, hiding real new
  // signups until the app was fully restarted. Refreshing whenever the tab/app regains focus
  // means reopening this screen, or switching back to it, always shows current numbers.
  useEffect(() => {
  ...
  const handleExtendTrial = async (merchant: any) => {
    ...
    try {
      ...
      // fresh trial window using the exact same logic the app already uses everywhere else.
      await setDoc(doc(db, 'merchants', merchant.uid), { trialStart: new Date().toISOString() }, { merge: true });
      setActionMsg(`Trial extended by ${TRIAL_DAYS} days.`);
      loadData();
    } catch (err: any) {
  ...
  // Website content and white-label controls for the premium one-off clients (the businesses
  // who pay a one-time setup fee for a fully branded site). Their content is written here by
  // the owner, not by the client - they paid precisely so they never have to fill in a form.
  const [wc, setWc] = useState<any>({});
  const [wcSaving, setWcSaving] = useState(false);

  useEffect(() => {
    if (selectedMerchant) {
      setWc(selectedMerchant.websiteContent || {});
      setActionMsg('');
    }
  }, [selectedMerchant?.uid]);

  const updateWc = (patch: any) => setWc((prev: any) => ({ ...prev, ...patch }));

  const updateList = (key: 'offers' | 'whyPoints', index: number, field: 'title' | 'text' | 'image', value: string) => {
    setWc((prev: any) => {
      const list = [...(prev[key] || [])];
  ...
  const addListItem = (key: 'offers' | 'whyPoints') => setWc((prev: any) => ({ ...prev, [key]: [...(prev[key] || []), { title: '', text: '' }] }));
  const removeListItem = (key: 'offers' | 'whyPoints', index: number) => setWc((prev: any) => ({ ...prev, [key]: (prev[key] || []).filter((_: any, i: number) => i !== index) }));

  const handleSaveWebsiteContent = async (merchant: any) => {
    setWcSaving(true);
    setActionMsg('');
    try {
      await setDoc(doc(db, 'merchants', merchant.uid), { websiteContent: wc }, { merge: true });
      setActionMsg('Website content saved.');
      loadData();
    } catch (err: any) {
      setActionMsg(`Could not save: ${err?.code || err?.message || 'unknown error'}`);
    } finally {
      setWcSaving(false);
    }
  };

  // Marks an account as a premium branded client: hides the SalesPilot credit from their
  // storefront, always shows their own logo, and gives them a full year of access with no
  // trial expiry or upgrade prompts, since they've already paid for the whole thing up front.
  const handleToggleWhiteLabel = async (merchant: any, on: boolean) => {
    setActionLoading(true);
    setActionMsg('');
    try {
      const payload: any = { whiteLabel: on };
      if (on) {
        payload.plan = 'pro';
        payload.subscriptionExpiresAt = new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString();
      }
      await setDoc(doc(db, 'merchants', merchant.uid), payload, { merge: true });
      setActionMsg(on ? 'Marked as a branded client, with 1 year of access.' : 'Branded client status removed.');
      loadData();
      setSelectedMerchant({ ...merchant, ...payload });
    } catch (err: any) {
      setActionMsg(`Could not update: ${err?.code || err?.message || 'unknown error'}`);
    } finally {
      setActionLoading(false);
    }
  };
  ...
  // Deletes a merchant's account data - their profile, products, orders and every other
  // subcollection. This cannot delete their actual Firebase Auth login (that needs admin
  // permissions the app doesn't have), so the email/password would technically still work
  // afterward, just against an empty account with nothing in it. Fine for clearing out test
  // accounts; a real merchant should be told this before anything is deleted.
  const handleDeleteMerchant = async (merchant: any) => {
    setDeletingMerchant(true);
    setActionMsg('');
    try {
      const subcollections = ['products', 'orders', 'coupons', 'bundles', 'reviews', 'staff', 'analytics', 'invoices', 'expenses', 'pos_sales', 'customers', 'notifications'];
      for (const sub of subcollections) {
        const snap = await getDocs(collection(db, 'merchants', merchant.uid, sub));
      ...
      setActionMsg(`Could not delete: ${err?.code || err?.message || 'unknown error'}`);
    } finally {
      ...
      <div style={{ minHeight: '100vh', background: C.bg, fontFamily: 'Inter, sans-serif' }}>
        ...
        <div style={{ background: C.white, borderRadius: '24px 24px 0 0', marginTop: -16, padding: 20, paddingBottom: 100 }}>
          ...
          <div style={{ background: C.blueLight, borderRadius: 12, padding: 14, marginBottom: 20 }}>
            ...
            <p style={{ fontSize: 12, color: C.dark, marginBottom: 10, lineHeight: 1.5 }}>Every new merchant already gets their free trial automatically. Use this only if this specific merchant needs a bit more time before they decide.</p>
            <button disabled={actionLoading} onClick={() => handleExtendTrial(selectedMerchant)} style={{ width: '100%', background: C.white, border: `1.5px solid ${C.border}`, borderRadius: 10, padding: 12, fontSize: 13, fontWeight: 700, color: C.dark, cursor: 'pointer', textAlign: 'left' as const }}>
              Give this merchant a fresh free trial
            </button>
          </div>
          ...
          <div style={{ background: C.purpleLight, borderRadius: 12, padding: 14, marginBottom: 20 }}>
            <p style={{ fontSize: 12.5, fontWeight: 700, color: C.purple, marginBottom: 4 }}>Branded client</p>
            <p style={{ fontSize: 12, color: C.dark, marginBottom: 10, lineHeight: 1.5 }}>
              For businesses who paid the one-off setup fee for a fully branded website. Turning this
              on hides the SalesPilot credit from their storefront, always shows their own logo, and
              gives them a full year of access with no trial or upgrade prompts.
            </p>
            <button
              disabled={actionLoading}
              onClick={() => handleToggleWhiteLabel(selectedMerchant, !selectedMerchant.whiteLabel)}
              style={{ width: '100%', background: selectedMerchant.whiteLabel ? C.purple : C.white, color: selectedMerchant.whiteLabel ? C.white : C.dark, border: `1.5px solid ${selectedMerchant.whiteLabel ? C.purple : C.border}`, borderRadius: 10, padding: 12, fontSize: 13, fontWeight: 700, cursor: 'pointer', textAlign: 'left' as const }}
            >
              {selectedMerchant.whiteLabel ? 'Branded client, on. Tap to turn off' : 'Mark as a branded client'}
            </button>
            {selectedMerchant.whiteLabel && selectedMerchant.subscriptionExpiresAt && (
              <p style={{ fontSize: 11.5, color: C.dark, marginTop: 9, fontWeight: 600 }}>
                Renewal due {new Date(selectedMerchant.subscriptionExpiresAt).toLocaleDateString()}
              </p>
            )}
          </div>

          <div style={{ background: C.bg, borderRadius: 12, padding: 14, marginBottom: 20 }}>
            <p style={{ fontSize: 12.5, fontWeight: 700, color: C.navy, marginBottom: 4 }}>Website content</p>
            <p style={{ fontSize: 12, color: C.gray, marginBottom: 14, lineHeight: 1.5 }}>
              This is what turns their storefront into a proper business website. Anything left blank
              simply will not appear, so a half-filled section never shows up looking broken.
            </p>

            <label style={ownerLabel}>Tagline</label>
            <input style={ownerInput} placeholder="e.g. Quality fabrics since 2015" value={wc.tagline || ''} onChange={e => updateWc({ tagline: e.target.value })} />

            <label style={ownerLabel}>About heading</label>
            <input style={ownerInput} placeholder="e.g. Welcome to Bella Boutique" value={wc.aboutTitle || ''} onChange={e => updateWc({ aboutTitle: e.target.value })} />

            <label style={ownerLabel}>About text</label>
            <textarea style={{ ...ownerInput, minHeight: 96, resize: 'vertical' as const, fontFamily: 'inherit' }} placeholder="Who they are, how long they've been trading, what they're known for..." value={wc.aboutText || ''} onChange={e => updateWc({ aboutText: e.target.value })} />

            <label style={ownerLabel}>About photo</label>
            <input style={ownerInput} placeholder="Cloudinary link, their shop front or interior" value={wc.aboutImage || ''} onChange={e => updateWc({ aboutImage: e.target.value })} />

            <label style={ownerLabel}>What we offer, heading</label>
            <input style={ownerInput} placeholder="e.g. What we do" value={wc.offerTitle || ''} onChange={e => updateWc({ offerTitle: e.target.value })} />

            {(wc.offers || []).map((o: any, i: number) => (
              <div key={i} style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 10, padding: 11, marginBottom: 9 }}>
                <input style={{ ...ownerInput, marginBottom: 7 }} placeholder="Title, e.g. Custom tailoring" value={o.title || ''} onChange={e => updateList('offers', i, 'title', e.target.value)} />
                <textarea style={{ ...ownerInput, minHeight: 56, marginBottom: 7, resize: 'vertical' as const, fontFamily: 'inherit' }} placeholder="One or two sentences" value={o.text || ''} onChange={e => updateList('offers', i, 'text', e.target.value)} />
                <input style={{ ...ownerInput, marginBottom: 7 }} placeholder="Photo link (optional)" value={o.image || ''} onChange={e => updateList('offers', i, 'image', e.target.value)} />
                <button onClick={() => removeListItem('offers', i)} style={{ background: '#FEE2E2', border: 'none', borderRadius: 8, padding: '7px 12px', fontSize: 12, fontWeight: 700, color: '#EF4444', cursor: 'pointer' }}>Remove</button>
              </div>
            ))}
            <button onClick={() => addListItem('offers')} style={ownerAddBtn}>+ Add something they offer</button>

            <label style={ownerLabel}>Why choose us, heading</label>
            <input style={ownerInput} placeholder="e.g. Why customers keep coming back" value={wc.whyTitle || ''} onChange={e => updateWc({ whyTitle: e.target.value })} />

            {(wc.whyPoints || []).map((p: any, i: number) => (
              <div key={i} style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 10, padding: 11, marginBottom: 9 }}>
                <input style={{ ...ownerInput, marginBottom: 7 }} placeholder="Title, e.g. Original products only" value={p.title || ''} onChange={e => updateList('whyPoints', i, 'title', e.target.value)} />
                <textarea style={{ ...ownerInput, minHeight: 56, marginBottom: 7, resize: 'vertical' as const, fontFamily: 'inherit' }} placeholder="One or two sentences" value={p.text || ''} onChange={e => updateList('whyPoints', i, 'text', e.target.value)} />
                <button onClick={() => removeListItem('whyPoints', i)} style={{ background: '#FEE2E2', border: 'none', borderRadius: 8, padding: '7px 12px', fontSize: 12, fontWeight: 700, color: '#EF4444', cursor: 'pointer' }}>Remove</button>
              </div>
            ))}
            <button onClick={() => addListItem('whyPoints')} style={ownerAddBtn}>+ Add a reason to choose them</button>

            <label style={ownerLabel}>Wide photo band</label>
            <input style={ownerInput} placeholder="Cloudinary link, a wide shot of the shop or products" value={wc.bandImage || ''} onChange={e => updateWc({ bandImage: e.target.value })} />

            <label style={ownerLabel}>Text over the band <span style={{ fontWeight: 400, color: C.gray }}>(optional)</span></label>
            <input style={ownerInput} placeholder="e.g. Serving Benin City since 2015" value={wc.bandText || ''} onChange={e => updateWc({ bandText: e.target.value })} />

            <label style={ownerLabel}>Opening hours</label>
            <textarea style={{ ...ownerInput, minHeight: 76, resize: 'vertical' as const, fontFamily: 'inherit' }} placeholder={'Mon - Fri: 9am - 6pm\nSat: 10am - 4pm\nSun: Closed'} value={wc.openingHours || ''} onChange={e => updateWc({ openingHours: e.target.value })} />

            <label style={ownerLabel}>Google Maps link <span style={{ fontWeight: 400, color: C.gray }}>(optional)</span></label>
            <input style={ownerInput} placeholder="Paste their Google Maps link" value={wc.mapsLink || ''} onChange={e => updateWc({ mapsLink: e.target.value })} />
            <p style={{ fontSize: 11.5, color: C.gray, marginTop: -6, marginBottom: 14, lineHeight: 1.5 }}>
              Leave this blank and the Get Directions button will simply search their address instead.
            </p>

            <button
              disabled={wcSaving}
              onClick={() => handleSaveWebsiteContent(selectedMerchant)}
              style={{ width: '100%', background: C.navy, color: 'white', border: 'none', borderRadius: 10, padding: 13, fontSize: 13.5, fontWeight: 700, cursor: 'pointer', opacity: wcSaving ? 0.7 : 1 }}
            >
              {wcSaving ? 'Saving...' : 'Save website content'}
            </button>
          </div>

          {/* Deleting is separated visually from everything else on this screen, and requires
              typing the store's own name to confirm - a single tap here would otherwise be one
              accidental touch away from permanently erasing a real merchant's business. */}
          <div style={{ background: '#FEF2F2', border: `1.5px solid #FECACA`, borderRadius: 12, padding: 14, marginBottom: 20 }}>
            <p style={{ fontSize: 12.5, fontWeight: 700, color: '#991B1B', marginBottom: 4 }}>Delete this account</p>
            <p style={{ fontSize: 12, color: C.dark, marginBottom: 10, lineHeight: 1.5 }}>
              Removes this merchant's store, products, orders and all their other data permanently.
              This cannot be undone. Their email could technically still sign in afterward, but to a
              completely empty account.
            </p>
            <input
              value={deleteConfirmText}
              onChange={e => setDeleteConfirmText(e.target.value)}
              placeholder={`Type "${selectedMerchant.storeName || selectedMerchant.name || 'DELETE'}" to confirm`}
              style={{ width: '100%', padding: '10px 12px', borderRadius: 9, border: '1.5px solid #FECACA', fontSize: 13, outline: 'none', marginBottom: 10, boxSizing: 'border-box' as const, background: C.white }}
            />
            <button
              disabled={deletingMerchant || deleteConfirmText.trim().toLowerCase() !== (selectedMerchant.storeName || selectedMerchant.name || 'DELETE').trim().toLowerCase()}
              onClick={() => handleDeleteMerchant(selectedMerchant)}
              style={{ width: '100%', background: '#EF4444', color: 'white', border: 'none', borderRadius: 10, padding: 12, fontSize: 13, fontWeight: 700, cursor: 'pointer', opacity: (deletingMerchant || deleteConfirmText.trim().toLowerCase() !== (selectedMerchant.storeName || selectedMerchant.name || 'DELETE').trim().toLowerCase()) ? 0.5 : 1 }}
            >
              {deletingMerchant ? 'Deleting...' : 'Delete this account permanently'}
            </button>
          </div>
    ...
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: 'Inter, sans-serif', paddingBottom: 40 }}>
      <div style={{ background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, padding: '18px 20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <h1 style={{ color: 'white', fontSize: 18, fontWeight: 700, flex: 1 }}>Owner Dashboard</h1>
          {/* This screen loads its numbers once, not live - a manual refresh means a real new
              signup is never more than one tap away from showing up correctly. */}
          <button onClick={loadData} disabled={loading} style={{ background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: 9, width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', opacity: loading ? 0.5 : 1 }}>
...
function SplashScreen() {
  ...
  return (
    <div style={{
      position: 'fixed' as const, inset: 0, background: '#FFFFFF',
      display: 'flex', flexDirection: 'column' as const, alignItems: 'center', justifyContent: 'center',
      zIndex: 999, overflow: 'hidden',
    }}>
      {/* Soft ambient colour wash - keeps the white feeling warm and intentional rather than
          blank, without ever competing with the graphic itself. */}
      <div style={{
        position: 'absolute' as const, width: 460, height: 460, borderRadius: '50%',
        background: `radial-gradient(circle, ${C.green}14 0%, transparent 70%)`,
        top: '12%', left: '-25%', filter: 'blur(10px)',
        animation: 'sp-drift 9s ease-in-out infinite alternate',
      }} />
      <div style={{
        position: 'absolute' as const, width: 400, height: 400, borderRadius: '50%',
        background: `radial-gradient(circle, ${C.navy}10 0%, transparent 70%)`,
        bottom: '10%', right: '-22%', filter: 'blur(10px)',
        animation: 'sp-drift 11s ease-in-out infinite alternate-reverse',
      }} />

      <div style={{ position: 'relative' as const, display: 'flex', flexDirection: 'column' as const, alignItems: 'center', padding: '0 32px' }}>
        <div style={{ animation: 'sp-graphic-in 1s cubic-bezier(0.22, 1, 0.36, 1) forwards', position: 'relative' as const, overflow: 'visible' as const, padding: 4 }}>
          <img
            src={SPLASH_GRAPHIC_URL}
            alt="SalesPilot"
            style={{
              width: 'min(40vw, 155px)', height: 'auto', maxHeight: '30vh',
        ...
        <div style={{
          ...
          transform: phase === 'combined' ? 'translateY(0)' : 'translateY(10px)',
          transition: 'opacity 0.7s ease, transform 0.7s cubic-bezier(0.22, 1, 0.36, 1)',
          marginTop: 22, display: 'flex', flexDirection: 'column' as const, alignItems: 'center',
        }}>
          <span style={{
            fontSize: 32, fontWeight: 800, letterSpacing: -0.4,
            lineHeight: 1.25, paddingTop: 4, paddingBottom: 2, display: 'inline-block' as const,
            background: `linear-gradient(100deg, ${C.navy} 0%, ${C.navyLight} 45%, ${C.green} 100%)`,
            WebkitBackgroundClip: 'text' as const, backgroundClip: 'text' as const,
            WebkitTextFillColor: 'transparent' as const, color: C.navy,
          }}>SalesPilot</span>
          <div style={{
            width: 40, height: 3, borderRadius: 3, margin: '13px 0 12px',
            background: `linear-gradient(90deg, ${C.navy}, ${C.green})`,
          }} />
          <span style={{ fontSize: 13.5, color: C.navyLight, letterSpacing: 0.5, fontWeight: 600, opacity: 0.85 }}>Your business, in your pocket</span>
        </div>
      </div>

      <style>{`
        @keyframes sp-graphic-in {
...
// Works out whether the current URL is a merchant's public storefront rather than the
// SalesPilot app itself. Anything that isn't one of the app's own routes is a store slug.
function isStorefrontRoute(): boolean {
  try {
    const first = window.location.pathname.split('/').filter(Boolean)[0];
    if (!first) return false;
    const appRoutes = ['login', 'signup', 'dashboard', 'onboarding', 'ref'];
    return !appRoutes.includes(first.toLowerCase());
  } catch {
    return false;
  }
}
...
function SplashGate() {
  ...
  const [showSplash, setShowSplash] = useState(() => {
    // A customer opening a merchant's store link should land on that merchant's shop, not sit
    // through a SalesPilot logo first. The splash belongs to the merchant-facing app only -
    // showing it on a storefront makes someone else's brand look like it belongs to us, and
    // costs a real shopper several seconds before they see anything they came for.
    if (isStorefrontRoute()) return false;
