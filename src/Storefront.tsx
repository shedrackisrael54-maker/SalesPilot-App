...
import { NAIRA, THEME_NICHES, ProductIcon, isValidCSSColor, isColorOptionGroup, getTieredPrice, NIGERIAN_STATES, PAYSTACK_PUBLIC_KEY, getCourierCustomerPrice } from './shared';
...
export function StorefrontScreen() {
  ...
  // to the same spot instead of dumping them at the top of a long page.
  useEffect(() => {
    if (!slug) return;
    const handleScroll = () => {
    ...
    <div style={{ minHeight: '100vh', background: theme.background, fontFamily: 'Inter, sans-serif', transition: 'background 0.5s ease', overflowX: 'hidden' as const, width: '100%', maxWidth: '100vw', boxSizing: 'border-box' as const }}>
      ...
      {!showWishlist && (
        <>
          ...
          {/* Hero - wide banner with static abstract background (no continuous animation, for scroll performance) */}
          <div style={{
            ...
            <div style={{ position: 'relative' as const, zIndex: 1 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' as const }}>
                <div style={{ minWidth: 0 }}>
                  ...
                  {merchant.storeDescription && (
                    <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: 12, lineHeight: 1.4, maxWidth: 220 }}>{merchant.storeDescription}</p>
                  )}
                </div>
            ...
            <div style={{ padding: '0 16px 24px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: 12 }}>
                {categories.map(cat => {
                  ...
                  return (
                    <button key={cat} onClick={() => goToCategory(cat)} className="sp-card" style={{ position: 'relative' as const, borderRadius: 18, overflow: 'hidden', border: 'none', cursor: 'pointer', aspectRatio: '0.95', width: '100%', minWidth: 0, maxWidth: '100%', background: `linear-gradient(135deg, ${theme.primary}22, ${theme.secondary}22)`, padding: 0, transition: 'transform 0.15s ease' }}>
                      {rep?.imageUrl ? (
                        <img src={rep.imageUrl} alt={cat} style={{ width: '100%', height: '100%', objectFit: 'cover' as const, position: 'absolute' as const, inset: 0 }} />
                      ) : (
