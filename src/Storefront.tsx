  ...
  return (
    <div style={{ padding: '34px 20px', background: `${theme.primary}08` }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 12 }}>
        {items.map((o, i) => (
          <div key={i} style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 14, padding: '16px 17px' }}>
            <p style={{ fontSize: 15, fontWeight: 700, color: '#111827', marginBottom: o.text?.trim() ? 6 : 0 }}>{o.title}</p>
            {o.text?.trim() && <p style={{ fontSize: 13.5, color: '#6B7280', lineHeight: 1.6 }}>{o.text}</p>}
          </div>
        ))}
      </div>
    </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
        {points.map((p, i) => (
          <div key={i} style={{ display: 'flex', gap: 13, alignItems: 'flex-start' }}>
            <div style={{ width: 30, height: 30, borderRadius: '50%', background: `${theme.primary}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <CheckCircle2 size={16} color={theme.primary} />
            </div>
            <div style={{ minWidth: 0 }}>
              <p style={{ fontSize: 14.5, fontWeight: 700, color: '#111827', marginBottom: p.text?.trim() ? 4 : 0 }}>{p.title}</p>
              {p.text?.trim() && <p style={{ fontSize: 13.5, color: '#6B7280', lineHeight: 1.6 }}>{p.text}</p>}
            </div>
          </div>
        ))}
      </div>
