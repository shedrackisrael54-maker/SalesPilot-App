...
import { NAIRA, THEME_NICHES, ProductIcon, isValidCSSColor, isColorOptionGroup, getTieredPrice, NIGERIAN_STATES, PAYSTACK_PUBLIC_KEY, getCourierCustomerPrice, getOptimizedImageUrl } from './shared';
    ...
    <div style={{ position: 'sticky' as const, top: 0, zIndex: 30, background: theme.secondary, transition: 'background 0.5s ease' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px 10px', gap: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 9, minWidth: 0 }}>
          ...
          {merchant.logoUrl && (
            <img src={merchant.logoUrl} alt={merchant.storeName} style={{ width: 30, height: 30, borderRadius: 8, objectFit: 'cover' as const, flexShrink: 0 }} />
          )}
          <div style={{ minWidth: 0, display: 'flex', alignItems: 'baseline', gap: 7 }}>
            ...
            {merchant.storeDescription && (
              <p style={{ fontSize: 10, color: 'rgba(255,255,255,0.55)', fontWeight: 500, whiteSpace: 'nowrap' as const, overflow: 'hidden', textOverflow: 'ellipsis' as const, maxWidth: 110 }}>
                {merchant.storeDescription}
              </p>
            )}
          </div>
...
function ProductCard({ product, theme, slug, wishlistIds }: { product: Product; theme: StoreTheme; slug: string; wishlistIds: string[] }) {
  ...
  return (
    <Link to={`/${slug}/product/${product.id}`} className="sp-card" style={{ textDecoration: 'none', display: 'block', width: '100%', height: '100%', minWidth: 0 }}>
      <div style={{ background: theme.background, borderRadius: 20, overflow: 'hidden', border: `1px solid ${theme.accent}44`, boxShadow: '0 6px 20px rgba(0,0,0,0.07)', transition: 'transform 0.15s ease, box-shadow 0.3s ease', display: 'flex', flexDirection: 'column' as const, height: '100%', minWidth: 0 }}>
        <div style={{ width: '100%', aspectRatio: '1', background: `linear-gradient(135deg, ${theme.primary}12, ${theme.secondary}0a)`, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' as const, overflow: 'hidden', flexShrink: 0 }}>
          {product.imageUrl ? (
            <img src={product.imageUrl} alt={product.name} style={{ width: '100%', height: '100%', objectFit: 'cover' as const, display: 'block' }} loading="lazy" />
          ) : (
...
// The storefront hero. A visitor's real first question is "what does this place actually
// sell" - a static banner with just a store name can't answer that. This cycles through the
// merchant's own product photos automatically, so a visitor sees real inventory within seconds
// of landing, and a store with only a handful of products still feels alive. Bolder and taller
// than the previous static version, with a real Shop Now button and a live price pill for
// whichever product is currently showing.
function StoreHero({ merchant, theme, products, slug }: { merchant: MerchantInfo; theme: StoreTheme; products: Product[]; slug: string }) {
  const showcase = products.filter(p => p.imageUrl).slice(0, 8);
  const [index, setIndex] = useState(0);

  useEffect(() => {
    if (showcase.length < 2) return;
    const timer = setInterval(() => setIndex(i => (i + 1) % showcase.length), 3800);
  ...
  return (
    <div style={{ position: 'relative' as const, width: '100%', minHeight: 280, overflow: 'hidden', background: `linear-gradient(120deg, ${theme.primary}, ${theme.secondary})` }}>
      {showcase.map((p, i) => (
        <img
          key={p.id}
          src={getOptimizedImageUrl(p.imageUrl, 900, 1.2)}
          alt={p.name}
      ...
      <div style={{ position: 'absolute' as const, inset: 0, background: 'linear-gradient(to top, rgba(0,0,0,0.8) 0%, rgba(0,0,0,0.35) 45%, rgba(0,0,0,0.15) 100%)' }} />

      <div style={{ position: 'relative' as const, zIndex: 1, minHeight: 280, display: 'flex', flexDirection: 'column' as const, justifyContent: 'flex-end', padding: '24px 18px 20px' }}>
        <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: 10.5, fontWeight: 800, letterSpacing: 1.5, textTransform: 'uppercase' as const, marginBottom: 7 }}>Welcome to</p>
        <h2 style={{ fontFamily: "'Playfair Display', serif", color: '#fff', fontSize: 30, fontWeight: 800, marginBottom: merchant.storeDescription ? 6 : 14, lineHeight: 1.08 }}>{merchant.storeName || 'Our Store'}</h2>
        {merchant.storeDescription && (
          <p style={{ color: 'rgba(255,255,255,0.88)', fontSize: 13, lineHeight: 1.5, maxWidth: 300, marginBottom: 16 }}>{merchant.storeDescription}</p>
        )}

        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' as const }}>
          <a href="#sp-products" className="sp-btn" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: theme.accent, color: theme.secondary, borderRadius: 13, padding: '13px 22px', fontWeight: 800, fontSize: 13.5, textDecoration: 'none', boxShadow: '0 8px 20px rgba(0,0,0,0.3)', transition: 'transform 0.15s ease' }}>
          ...
          {active && (
            <Link
              ...
              className="sp-btn"
              style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.16)', backdropFilter: 'blur(6px)', color: '#fff', borderRadius: 13, padding: '11px 15px', textDecoration: 'none', minWidth: 0, transition: 'transform 0.15s ease' }}
            >
        ...
        {showcase.length > 1 && (
          <div style={{ display: 'flex', gap: 5, marginTop: 16 }}>
            {showcase.map((_, i) => (
              <div key={i} style={{ width: i === index ? 20 : 6, height: 4, borderRadius: 2, background: i === index ? theme.accent : 'rgba(255,255,255,0.4)', transition: 'width 0.35s ease, background 0.35s ease' }} />
            ))}
          </div>
        )}
      </div>
    ...
    <div style={{ minHeight: '100vh', background: theme.background, fontFamily: 'Inter, sans-serif', transition: 'background 0.5s ease', overflowX: 'hidden' as const, width: '100%', maxWidth: '100vw', boxSizing: 'border-box' as const }}>
      ...
      {!showWishlist && (
        <>
                      ...
                      <div style={{ display: 'flex', marginBottom: 10 }}>
                        {bundleProducts.slice(0, 3).map((p, i) => (
                          <div key={p.id} style={{ width: 44, height: 44, borderRadius: 10, overflow: 'hidden', border: '2px solid #fff', marginLeft: i > 0 ? -14 : 0, background: '#F3F4F6', flexShrink: 0 }}>
                            {(p.images?.[0] || p.imageUrl) ? <img src={p.images?.[0] || p.imageUrl} alt={p.name} style={{ width: '100%', height: '100%', objectFit: 'cover' as const }} /> : <ProductIcon iconKey={p.emoji} size={20} color={theme.primary} />}
                          </div>
                        ))}
                      </div>
          ...
          {/* Shop by category - a horizontal scrolling row rather than a fixed grid, so
              categories read as quick shortcuts to browse through, not the main event
              competing with the actual products for space on the page. Each card has a fixed
              width regardless of how many categories exist, so one or two categories never
              stretch to fill the row - they just leave empty space to the right, exactly like
              a real horizontal list would. */}
          {categories.length > 0 && (
            <div style={{ padding: '20px 0 24px' }}>
              <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 18, fontWeight: 700, color: '#111827', marginBottom: 14, padding: '0 16px' }}>Shop by Category</h3>
                ...
                {categories.map(cat => {
                  ...
                  return (
                    <button
                      ...
                      className="sp-card"
                      style={{
                        position: 'relative' as const, borderRadius: 18, overflow: 'hidden', border: 'none', cursor: 'pointer',
                        width: 132, minWidth: 132, maxWidth: 132, aspectRatio: '0.95', flexShrink: 0,
                        background: `linear-gradient(135deg, ${theme.primary}22, ${theme.secondary}22)`, padding: 0,
                        transition: 'transform 0.15s ease',
                      }}
                    >
                      {rep?.imageUrl ? (
                        <img src={getOptimizedImageUrl(rep.imageUrl, 300, 0.95)} alt={cat} style={{ width: '100%', height: '100%', objectFit: 'cover' as const, position: 'absolute' as const, inset: 0 }} />
                      ) : (
                      ...
                      <div style={{ position: 'absolute' as const, bottom: 10, left: 10, right: 10, textAlign: 'left' as const }}>
                        <p style={{ color: '#fff', fontSize: 12, fontWeight: 800, overflow: 'hidden', textOverflow: 'ellipsis' as const, whiteSpace: 'nowrap' as const }}>{cat}</p>
                        <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: 10 }}>{inCat.length} item{inCat.length !== 1 ? 's' : ''}</p>
                      </div>
                    </button>
                  );
                })}
          ...
          {/* Discover More - full-width squared strip, matching trust badge treatment */}
          ...
          {newArrivals.length > 0 && (
            <div style={{ padding: '0 16px 24px' }}>
              <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 18, fontWeight: 700, color: '#111827', marginBottom: 12 }}>New Arrivals</h3>
              <div style={{ display: 'flex', gap: 12, overflowX: 'auto' as const, paddingBottom: 4 }}>
                {newArrivals.map(product => (
                  <div key={product.id} style={{ width: 150, minWidth: 150, maxWidth: 150, flexShrink: 0 }}>
                    <ProductCard product={product} theme={theme} slug={slug!} wishlistIds={wishlistIds} />
                  </div>
                ))}
              </div>
            </div>
                ...
                {bundleProducts.map((p, i) => (
                  <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderBottom: i < bundleProducts.length - 1 ? '1px solid #F3F4F6' : 'none' }}>
                    <div style={{ width: 40, height: 40, borderRadius: 8, overflow: 'hidden', background: '#F3F4F6', flexShrink: 0 }}>
                      {(p.images?.[0] || p.imageUrl) ? <img src={p.images?.[0] || p.imageUrl} alt={p.name} style={{ width: '100%', height: '100%', objectFit: 'cover' as const }} /> : <ProductIcon iconKey={p.emoji} size={18} color={theme.primary} />}
                    </div>
            ...
            >
              {gallery.map((img, i) => (
                <div key={i} style={{ minWidth: '100%', height: '100%', scrollSnapAlign: 'start' as const, flexShrink: 0 }}>
                  <img src={img} alt={`${product.name} ${i + 1}`} style={{ width: '100%', height: '100%', objectFit: 'cover' as const }} />
                </div>
              ))}
            </div>
            {gallery.length > 1 && (
      ...
      {gallery.length > 1 && (
        <div style={{ display: 'flex', gap: 10, padding: '0 16px 6px', overflowX: 'auto' as const }}>
          {gallery.map((img, i) => (
            ...
            >
              <img src={img} alt={`Thumbnail ${i + 1}`} style={{ width: '100%', height: '100%', objectFit: 'cover' as const, display: 'block' }} />
            </button>
          ))}
        </div>
      )}
