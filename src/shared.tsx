...
// SalesPilot's own commission on every storefront order (separate from whatever Paystack's
// own gateway fee is) - matches the same shape as Bumpa's own transaction fee. By default this
// comes out of the merchant's payout later (handled manually, off-app), but a merchant can
// choose to have it added visibly onto the customer's total instead, the same way many stores
// let customers choose to cover the payment processing fee.
...
// choose to have it added visibly onto the customer's total instead, the same way many stores
// let customers choose to cover the payment processing fee.
export const PLATFORM_ORDER_FEE_RATE = 0.015;

export function getPlatformOrderFee(subtotal: number): number {
  return Math.round(subtotal * PLATFORM_ORDER_FEE_RATE);
}
