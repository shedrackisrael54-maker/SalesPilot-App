...
import { NAIRA, THEME_NICHES, PRODUCT_CATEGORIES, PRODUCT_ICON_MAP, PRODUCT_ICON_KEYS, ProductIcon, compressImage, uploadToCloudinary, isValidCSSColor, isColorOptionGroup, NIGERIAN_STATES, PAYSTACK_PUBLIC_KEY, PLATFORM_LOGO_URL } from './shared';
...
function AppLogo({ size }: { size: number }) {
  if (PLATFORM_LOGO_URL) {
    return <img src={PLATFORM_LOGO_URL} alt="SalesPilot" style={{ width: size, height: size, borderRadius: size >= 44 ? 12 : 8, objectFit: 'cover' as const, flexShrink: 0 }} />;
  }
  return (
...
function AuthProvider({ children }: any) {
  ...
  const loginWithOAuthProvider = async (provider: any) => {
    ...
    if (merchantSnap.exists()) {
      ...
      const profile = { name: cred.user.displayName || '', email: cred.user.email || '', storeName: '', onboardingComplete: false };
      await setDoc(doc(db, 'merchants', cred.user.uid), profile);
      setUser({ uid: cred.user.uid, ...profile, isStaff: false });
    }
  };
...
function DashboardInfoCardFace({ card, onSelect }: { card: DashboardInfoCard; onSelect: (card: DashboardInfoCard) => void }) {
  ...
  return (
    ...
    >
      {card.imageUrl ? (
        <>
          <img src={card.imageUrl} alt="" style={{ position: 'absolute' as const, inset: 0, width: '100%', height: '100%', objectFit: 'cover' as const }} loading="lazy" />
          <div style={{ position: 'absolute' as const, inset: 0, background: 'linear-gradient(0deg, rgba(0,0,0,0.65), rgba(0,0,0,0.25))' }} />
        </>
      ) : (
        <>
      ...
      <div style={{ background: C.bg, minHeight: '100vh', fontFamily: 'Inter, sans-serif' }}>
        <div style={{ background: activeInfoCard.imageUrl ? '#111' : gradient, padding: activeInfoCard.imageUrl ? '20px 20px 40px' : '20px 20px 28px', position: 'relative' as const, overflow: 'hidden' }}>
          {activeInfoCard.imageUrl ? (
            <>
              <img src={activeInfoCard.imageUrl} alt="" style={{ position: 'absolute' as const, inset: 0, width: '100%', height: '100%', objectFit: 'cover' as const }} />
              <div style={{ position: 'absolute' as const, inset: 0, background: 'linear-gradient(0deg, rgba(0,0,0,0.7), rgba(0,0,0,0.25))' }} />
            </>
          ) : (
...
// A short, tasteful splash shown once when the app is first opened - the real page loads
// underneath it the whole time, so this never actually slows anything down, it just gives a
// polished first moment before revealing the app. The same behavior carries over automatically
// if this is ever wrapped as a native app later, since it's just how the page loads.
function SplashScreen({ onDone }: { onDone: () => void }) {
  const [fadingOut, setFadingOut] = useState(false);
  useEffect(() => {
    const t1 = setTimeout(() => setFadingOut(true), 900);
    const t2 = setTimeout(onDone, 1300);
    return () => { clearTimeout(t1); clearTimeout(t2); };
  }, []);
  return (
    <div style={{ position: 'fixed' as const, inset: 0, background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999, opacity: fadingOut ? 0 : 1, transition: 'opacity 0.4s ease' }}>
      <div style={{ animation: 'sp-splash-in 0.6s ease forwards' }}>
        <AppLogo size={72} />
      </div>
      <style>{`@keyframes sp-splash-in { from { opacity: 0; transform: scale(0.8); } to { opacity: 1; transform: scale(1); } }`}</style>
    </div>
  );
}
