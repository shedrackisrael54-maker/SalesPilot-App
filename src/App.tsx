...
import { doc, setDoc, getDoc, onSnapshot, collection, deleteDoc, addDoc, query, orderBy, limit as fsLimit } from 'firebase/firestore';

import { NAIRA, THEME_NICHES, PRODUCT_CATEGORIES, PRODUCT_ICON_MAP, PRODUCT_ICON_KEYS, ProductIcon, compressImage, uploadToCloudinary, isValidCSSColor, isColorOptionGroup, NIGERIAN_STATES, PAYSTACK_PUBLIC_KEY } from './shared';
...
import { getPlatformOrderFee } from './shared';
...
const PRO_ONLY_ACTIONS = ['staff', 'email-campaign', 'connected-tools', 'analytics'];
...
function AuthProvider({ children }: any) {
  ...
  const signup = async (email: string, password: string, name: string) => {
    ...
    const profile = { name, email, storeName: '', onboardingComplete: false };
  ...
  return (
    <AuthContext.Provider value={{ user, authReady, signup, loginWithPassword, updateProfile, logout }}>
      {children}
...
const DASHBOARD_INFO_CARDS: DashboardInfoCard[] = [
  ...
  },
  {
    ...
    content: [
      ...
      { text: 'Market smarter. Run coupons to bring in hesitant buyers, send email campaigns to remind past customers you exist, or connect Facebook Pixel so your ads actually learn who buys from you, all built in, no extra tools to pay for.' },
  ...
  },
  {
    id: 'features',
    icon: 'features',
    teaserTitle: 'Features You Might Be Missing',
    teaserSubtitle: 'Powerful tools, hiding in plain sight',
    teaserCta: 'Tap to explore',
    fullTitle: 'Features You Might Be Missing',
    content: [
      { text: 'A lot of what makes SalesPilot powerful is not shouted about, it is just there when you need it. Here is what you might not have found yet.' },
      { heading: 'Barcode Scanning', text: 'Add barcodes to your products, then scan instead of typing during checkout on POS. Faster sales, fewer mistakes, especially useful if you sell items with a lot of similar names or variants.' },
      { heading: 'Delivery Couriers', text: 'Set up the couriers you actually use with your own delivery price. Customers pick one at checkout, and you can see exactly who they chose on the order, so there is no back and forth figuring out how to get their order to them.' },
      { heading: 'Custom Checkout Questions', text: 'Need to ask something before an order goes through? Gift message, delivery instructions, how they heard about you? Add your own questions, mark the important ones as required, and the answers show up right on the order.' },
      { heading: 'Platform Fee Choice', text: 'You decide who covers SalesPilot small transaction fee, you or your customer. Either way, it is clearly worked out for you so there is never a surprise at payout time.' },
      { heading: 'Terms and Conditions', text: 'Set your own store policy on refunds, delivery, and disputes. Customers agree to it before checking out, and it is saved exactly as they saw it at that moment, real protection if a disagreement ever comes up later.' },
      { text: 'Explore the More menu whenever you can. Most of what is here started as something quietly waiting to be found.' },
    ],
  },
  {
    id: 'support',
    ...
    content: [
      { text: "Running a business by yourself is hard. Some days it is stock that will not add up, a customer who will not pay, or just not knowing if you are doing things the right way. SalesPilot was built by one person who understands that, because they are building it the same way you are building your business, alone, from a phone, figuring it out as they go." },
      { text: 'That means when something is not working or you are not sure how to do something, you are not shouting into a void. Reach out directly, explain what is going on, and you will get a real answer from someone who actually built the thing you are using, not a call center reading from a script.' },
      { text: 'This also means SalesPilot grows the way it should. Features get added because real sellers asked for them, not because a boardroom decided it. If something is missing that would genuinely make your business easier to run, say so. That is exactly how most of what you are using today came to exist.' },
      { text: 'You did not start your business to become a tech expert. You started it to sell, to earn, to build something that is yours. SalesPilot whole job is to handle the technical side so you can focus on that. And whenever it is not doing that job well enough, that is on us to fix, and we will.' },
...
function DashboardInfoCardFace({ card, onSelect }: { card: DashboardInfoCard; onSelect: (card: DashboardInfoCard) => void }) {
  ...
  return (
    <button
      ...
      style={{
        flex: '0 0 78%', scrollSnapAlign: 'start' as const, background: gradient, borderRadius: 20, padding: 20,
...
function DashboardInfoModal({ card, onClose }: { card: DashboardInfoCard | null; onClose: () => void }) {
  useEffect(() => {
    if (!card) return;
  ...
  const gradient = DASHBOARD_INFO_GRADIENT_MAP[card.icon];
  return (
    <div onClick={onClose} style={{ position: 'fixed' as const, inset: 0, background: 'rgba(20,42,69,0.6)', zIndex: 100, display: 'flex', alignItems: 'flex-end' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: C.white, width: '100%', maxHeight: '82dvh', borderRadius: '24px 24px 0 0', display: 'flex', flexDirection: 'column' as const, overflow: 'hidden' }}>
        <div style={{ background: gradient, padding: '22px 20px 20px', position: 'relative' as const, flexShrink: 0 }}>
          <div style={{ position: 'absolute' as const, width: 100, height: 100, borderRadius: '50%', background: 'rgba(255,255,255,0.08)', top: -40, right: -20 }} />
          <button onClick={onClose} style={{ position: 'absolute' as const, top: 18, right: 18, background: 'rgba(255,255,255,0.2)', border: 'none', borderRadius: 10, width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', zIndex: 1 }}>
            <X size={16} color="white" />
          </button>
          <div style={{ width: 42, height: 42, borderRadius: 12, background: 'rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 12, position: 'relative' as const, zIndex: 1 }}>
            <Icon size={21} color="white" />
          </div>
          <h3 style={{ fontSize: 19, fontWeight: 800, color: 'white', textAlign: 'left' as const, position: 'relative' as const, zIndex: 1 }}>{card.fullTitle}</h3>
        </div>
        <div style={{ overflowY: 'auto' as const, overscrollBehavior: 'contain' as const, padding: '20px 20px 48px', WebkitOverflowScrolling: 'touch' as const }}>
          {card.content.map((block, i) => (
            <div key={i} style={{ marginBottom: 16, textAlign: 'left' as const }}>
              {block.heading && (
      ...
      <div style={{ background: `linear-gradient(135deg, ${C.navy} 0%, ${C.navyLight} 100%)`, padding: '16px 20px 24px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 40, height: 40, borderRadius: '50%', background: C.green, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 700, color: 'white', border: '2px solid rgba(255,255,255,0.3)' }}>
              {user?.name?.[0] || 'M'}
            </div>
      ...
      <div style={{ padding: '0 16px', marginTop: -16 }}>
              ...
              {trialExpired ? (
                ...
                <>
                  ...
                  <p style={{ fontSize: 12, color: trialDaysLeft <= 3 ? '#92400E' : C.gray }}>
                    {trialDaysLeft <= 3 ? 'Upgrade soon to avoid losing access' : `Enjoying ${user?.plan === 'pro' ? 'Pro' : 'Basic'}? You can upgrade anytime`}
                  </p>
                </>
              )}
            </div>
        ...
        {!setupComplete && (
          <div onClick={() => onNavigate?.('complete-store')} style={{ display: 'flex', alignItems: 'center', gap: 10, background: C.white, borderRadius: 14, padding: '12px 14px', marginBottom: 16, cursor: 'pointer', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
            ...
            <div style={{ flex: 1 }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: C.dark }}>Your store is live! Finish setting it up</p>
        ...
        <DashboardInfoModal card={activeInfoCard} onClose={() => setActiveInfoCard(null)} />
      </div>
    </div>
  );
}
...
const NIGERIAN_BANKS = [
  ...
  'Ecobank', 'Keystone Bank', 'FCMB', 'Heritage Bank', 'Kuda Bank', 'Opay', 'Moniepoint', 'Palmpay', 'Other',
];
...
// Lets a merchant choose who covers SalesPilot's own commission on each online order - either
// the merchant absorbs it (deducted from their payout later, invisible to the customer), or
// the customer covers it (added as a clearly labeled line at checkout, same idea as a payment
// processing fee many stores pass on).
function PlatformFeeScreen({ onBack }: { onBack: () => void }) {
  const { user, updateProfile } = useAuth();
  const [feePaidByCustomer, setFeePaidByCustomer] = useState<boolean>(!!user?.feePaidByCustomer);
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);
  const [error, setError] = useState('');

  const exampleSubtotal = 100000;
  const exampleFee = getPlatformOrderFee(exampleSubtotal);

  const handleSave = async () => {
    setError('');
    setSaving(true);
    try {
      await updateProfile({ feePaidByCustomer });
      setSaved(true);
      setTimeout(() => setSaved(false), 2000);
    } catch (err: any) {
      setError(err?.message || 'Could not save. Please try again.');
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100vh', fontFamily: 'Inter, sans-serif' }}>
      <div style={{ background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, padding: '20px 20px 28px', display: 'flex', alignItems: 'center', gap: 14 }}>
        ...
        <h2 style={{ color: 'white', fontSize: 19, fontWeight: 700 }}>Platform Fee</h2>
      </div>
      <div style={{ background: C.white, borderRadius: '24px 24px 0 0', marginTop: -16, padding: 20, paddingBottom: 100 }}>
        <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
          <p style={{ fontSize: 13, color: C.gray, lineHeight: 1.6 }}>
            SalesPilot takes a small {(0.015 * 100).toFixed(1)}% commission on every online order
            (separate from Paystack's own payment processing fee). Choose who covers it.
          </p>

          <button
            onClick={() => setFeePaidByCustomer(false)}
            style={{ textAlign: 'left' as const, background: !feePaidByCustomer ? `${C.navy}0d` : C.white, border: `2px solid ${!feePaidByCustomer ? C.navy : C.border}`, borderRadius: 14, padding: 16, cursor: 'pointer' }}
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
              {!feePaidByCustomer && <CheckCircle2 size={16} color={C.navy} />}
              <p style={{ fontSize: 14, fontWeight: 700, color: C.dark }}>I'll cover the fee</p>
            </div>
            <p style={{ fontSize: 12.5, color: C.gray, lineHeight: 1.5 }}>
              Customers see and pay only your product prices. Our commission is deducted from
              your payout instead.
            </p>
          </button>

          <button
            onClick={() => setFeePaidByCustomer(true)}
            style={{ textAlign: 'left' as const, background: feePaidByCustomer ? `${C.navy}0d` : C.white, border: `2px solid ${feePaidByCustomer ? C.navy : C.border}`, borderRadius: 14, padding: 16, cursor: 'pointer' }}
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
              {feePaidByCustomer && <CheckCircle2 size={16} color={C.navy} />}
              <p style={{ fontSize: 14, fontWeight: 700, color: C.dark }}>Customer covers the fee</p>
            </div>
            <p style={{ fontSize: 12.5, color: C.gray, lineHeight: 1.5 }}>
              A clearly labeled "Platform fee" line is added to the customer's total at checkout.
              You keep your full product price.
            </p>
          </button>

          <div style={{ background: C.bg, borderRadius: 12, padding: 14 }}>
            <p style={{ fontSize: 12, color: C.gray, marginBottom: 4 }}>Example on a {NAIRA}{exampleSubtotal.toLocaleString()} order</p>
            <p style={{ fontSize: 13.5, fontWeight: 700, color: C.dark }}>
              SalesPilot's cut: {NAIRA}{exampleFee.toLocaleString()}
              {feePaidByCustomer ? ' - added to what the customer pays' : ' - taken from your payout'}
            </p>
          </div>
...
function CompleteStoreScreen({ onBack }: { onBack: () => void }) {
  ...
  if (activeItem === 'bank') {
    return (
      <ChecklistItemScreen title="Bank details" onBack={() => setActiveItem(null)} saving={saving} error={error}
        onSave={() => saveField({ bankName, accountNumber, accountName })}>
        <div>
          <label style={label}>Bank name</label>
          <select style={{ ...inp, appearance: 'none' } as any} value={bankName} onChange={e => setBankName(e.target.value)}>
        ...
        <div>
          ...
          <input style={inp} placeholder="As it appears on your bank account" value={accountName} onChange={e => setAccountName(e.target.value)} />
        </div>
        <p style={{ fontSize: 12, color: C.gray }}>This is where payments from your store will be sent. Keep it accurate.</p>
      </ChecklistItemScreen>
    );
  }
...
function AnalyticsScreen({ onBack }: { onBack: () => void }) {
  ...
  const grossProfit = totalRevenue - periodExpenses;
          ...
          <>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
              ...
              <div style={{ background: C.blueLight, borderRadius: 14, padding: 14 }}>
                <p style={{ fontSize: 11.5, color: C.blue, fontWeight: 700, marginBottom: 4 }}>Gross Profit</p>
                <p style={{ fontSize: 18, fontWeight: 800, color: C.dark }}>{NAIRA}{grossProfit.toLocaleString()}</p>
              </div>
...
function EmailCampaignScreen({ onBack }: { onBack: () => void }) {
  const { user } = useAuth();
  const [invoices, setInvoices] = useState<any[]>([]);
  const [posSales, setPosSales] = useState<any[]>([]);
  const [orders, setOrders] = useState<Order[]>([]);
  ...
  const handleSend = async () => {
    ...
    setSendResult({ sent: sentCount, total: filteredRecipients.length });
    setSending(false);
  };
  ...
  if (sendResult) {
    return (
      <div style={{ background: C.bg, minHeight: '100vh', fontFamily: 'Inter, sans-serif', display: 'flex', flexDirection: 'column' as const, alignItems: 'center', justifyContent: 'center', padding: 24, textAlign: 'center' as const }}>
        ...
        <p style={{ fontSize: 17, fontWeight: 800, color: C.dark, marginBottom: 6 }}>Campaign sent!</p>
        <p style={{ fontSize: 13, color: C.gray, marginBottom: 24 }}>Delivered to {sendResult.sent} of {sendResult.total} customers.</p>
      ...
      <div style={{ background: C.white, borderRadius: '24px 24px 0 0', marginTop: -16, padding: 20 }}>
        ...
        {(() => {
          ...
          return selected.customerPhone ? (
            ...
            <button
              onClick={() => { navigator.clipboard.writeText(invoiceText); setCopiedInvoice(true); setTimeout(() => setCopiedInvoice(false), 2000); }}
...
function MoreScreen({ logout, user, onNavigate }: any) {
  ...
  if (subActive) {
    subscriptionLabel = `${user.plan === 'pro' ? 'Pro' : 'Basic'} active`;
  ...
  const menuItems = [
    ...
    { Icon: CreditCard, label: 'Platform Fee', sub: 'Choose who covers our commission', action: 'platform-fee' },
    { Icon: FileText, label: 'Terms & Conditions', sub: 'Set your store policy for checkout', action: 'terms' },
    { Icon: Mail, label: 'Email Campaign', sub: 'Send a message to your customers', action: 'email-campaign' },
    ...
    { Icon: CreditCard, label: 'Subscription', sub: subscriptionLabel, action: 'subscription' },
    ...
    <div style={{ background: C.bg, minHeight: '100vh', paddingBottom: 80, fontFamily: 'Inter, sans-serif' }}>
      <div style={{ background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, padding: '20px 20px 40px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <div style={{ width: 60, height: 60, borderRadius: '50%', background: C.green, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, fontWeight: 700, color: 'white', border: '3px solid rgba(255,255,255,0.3)' }}>{user?.name?.[0] || 'M'}</div>
          <div>
      ...
      <div style={{ background: C.white, borderRadius: '24px 24px 0 0', marginTop: -20, padding: 20 }}>
        {menuItems.filter(item => canAccessMoreAction(user, item.action)).map(item => {
          const locked = PRO_ONLY_ACTIONS.includes(item.action) && !isPro;
          return (
            <div key={item.label} onClick={() => handleItemClick(item.action)} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '16px 0', borderBottom: `1px solid ${C.border}`, cursor: 'pointer' }}>
              <div style={{ width: 46, height: 46, background: C.bg, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><item.Icon size={20} color={C.navy} /></div>
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <p style={{ fontSize: 15, fontWeight: 600, color: C.dark, marginBottom: 2 }}>{item.label}</p>
                  {locked && <span style={{ background: C.navy, color: 'white', fontSize: 9, fontWeight: 800, padding: '2px 6px', borderRadius: 20, letterSpacing: 0.3 }}>PRO</span>}
                </div>
                <p style={{ fontSize: 13, color: C.gray }}>{item.sub}</p>
              </div>
              <ChevronRight size={18} color={C.gray} />
            </div>
          );
        })}
        <button onClick={logout} style={{ background: '#FEE2E2', color: '#EF4444', border: 'none', borderRadius: 14, padding: 16, fontSize: 15, fontWeight: 700, cursor: 'pointer', width: '100%', marginTop: 24 }}>Logout</button>
      </div>
    </div>
  );
}
...
function Dashboard() {
  ...
  if (moreView === 'platform-fee') {
    return <PlatformFeeScreen onBack={() => setMoreView(null)} />;
  }
  if (moreView === 'terms') {
    return <TermsScreen onBack={() => setMoreView(null)} />;
  }
  if (moreView === 'email-campaign') {
    return <EmailCampaignScreen onBack={() => setMoreView(null)} />;
  }
  if (moreView === 'reviews') {
    ...
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: 'Inter, sans-serif' }}>
      <div style={{ background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, padding: '48px 24px 80px', textAlign: 'center' as const }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 20 }}>
          <div style={{ width: 44, height: 44, background: C.green, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, color: 'white', fontSize: 16 }}>SP</div>
      ...
      <div style={{ background: C.white, borderRadius: '28px 28px 0 0', marginTop: -28, padding: '28px 24px', minHeight: '60vh' }}>
        <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
          ...
          <div><label style={{ fontSize: 13, fontWeight: 600, color: C.dark, display: 'block', marginBottom: 8 }}>Password</label><input style={inp} name="password" type="password" placeholder="Min. 6 characters" value={form.password} onChange={handle} /></div>
          <button disabled={loading} style={{ background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, color: 'white', border: 'none', borderRadius: 14, padding: 16, fontSize: 16, fontWeight: 700, cursor: 'pointer', marginTop: 8, opacity: loading ? 0.7 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }} onClick={handleSignup}>{loading ? 'Creating account...' : <>Create account <ArrowRight size={16} /></>}</button>
    ...
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ background: C.white, borderRadius: '28px 28px 0 0', marginTop: -28, padding: '28px 24px', minHeight: '60vh' }}>
        <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
          {error && <div style={{ background: '#FEE2E2', color: '#EF4444', borderRadius: 12, padding: '12px 14px', fontSize: 13, fontWeight: 600, wordBreak: 'break-word' as const, overflowWrap: 'break-word' as const, maxWidth: '100%', boxSizing: 'border-box' as const }}>{error}</div>}
          <div><label style={{ fontSize: 13, fontWeight: 600, color: C.dark, display: 'block', marginBottom: 8 }}>Email address</label><input style={inp} name="email" type="email" placeholder="amaka@gmail.com" value={form.email} onChange={handle} /></div>
          <div><label style={{ fontSize: 13, fontWeight: 600, color: C.dark, display: 'block', marginBottom: 8 }}>Password</label><input style={inp} name="password" type="password" placeholder="Your password" value={form.password} onChange={handle} /></div>
...
const PLANS = [
  { id: 'basic', name: 'Basic', desc: 'Everything to run your store solo', prices: { quarterly: 10000, biannual: 18000, annual: 30000 }, features: ['Unlimited products, no cap', 'Storefront + real checkout (Paystack or WhatsApp)', 'Automatic inventory tracking, online & offline', 'POS & branded invoices', 'Coupons, Bundles & tiered pricing', 'Customer reviews & basic CRM', 'Order tracking for customers', 'Cart Timer & Sale Countdown', '1 free theme per store niche'], popular: false },
  { id: 'pro', name: 'Pro', desc: 'For sellers ready to grow', prices: { quarterly: 20000, biannual: 40000, annual: 80000 }, features: ['Everything in Basic', 'Staff accounts with role permissions', 'Email marketing campaigns', 'Customer segments & groups', 'Advanced Analytics (channels, best/worst sellers)', 'Facebook Pixel / Google Analytics tracking', 'All 32 Brand Studio themes', 'Free custom domain (annual billing only)', 'Priority support'], popular: true },
];
...
function SubscriptionScreen({ onBack }: { onBack: () => void }) {
  ...
  const [billing, setBilling] = useState<'quarterly' | 'biannual' | 'annual'>((user?.billing as any) || 'quarterly');
  const [selectedPlan, setSelectedPlan] = useState(user?.plan === 'pro' ? 'pro' : 'basic');
  ...
  if (subscriptionActive) {
    statusLabel = `${user.plan === 'pro' ? 'Pro' : 'Basic'} active until ${new Date(expiresAt!).toLocaleDateString()}`;
  } else if (user?.trialStart) {
  ...
  const handlePay = () => {
    ...
    const amount = planObj.prices[billing];
  ...
  if (success) {
    return (
      <div style={{ minHeight: '100vh', background: C.bg, fontFamily: 'Inter, sans-serif', display: 'flex', flexDirection: 'column' as const, alignItems: 'center', justifyContent: 'center', padding: 24, textAlign: 'center' as const }}>
        ...
        <p style={{ fontSize: 13.5, color: C.gray, marginBottom: 26 }}>You're now subscribed to {selectedPlan === 'pro' ? 'Pro' : 'Basic'}.</p>
      ...
      <div style={{ padding: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 20, background: C.bg, borderRadius: 12, padding: 5 }}>
          {[{ id: 'quarterly', label: 'Quarterly' }, { id: 'biannual', label: '6 Months' }, { id: 'annual', label: 'Yearly' }].map(b => (
            <button key={b.id} onClick={() => setBilling(b.id as any)} style={{ flex: 1, background: billing === b.id ? C.white : 'transparent', border: 'none', padding: '10px 4px', borderRadius: 10, color: billing === b.id ? C.navy : C.gray, fontSize: 12, cursor: 'pointer', fontWeight: 700 }}>{b.label}</button>
          ))}
        </div>
        <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16, marginBottom: 24 }}>
          {PLANS.map(p => (
            <div key={p.id} onClick={() => setSelectedPlan(p.id)} style={{ background: C.white, border: `2px solid ${selectedPlan === p.id ? C.navy : C.border}`, borderRadius: 20, padding: 20, cursor: 'pointer', position: 'relative' as const, boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
              {p.popular && <div style={{ position: 'absolute' as const, top: -12, left: 20, background: C.navy, color: 'white', fontSize: 11, fontWeight: 700, padding: '4px 14px', borderRadius: 20 }}>Most Popular</div>}
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14 }}>
                <div>
                  <h3 style={{ fontSize: 20, color: C.dark, marginBottom: 4, fontWeight: 700 }}>{p.name}</h3>
                  <p style={{ fontSize: 12, color: C.gray }}>{p.desc}</p>
                </div>
                <div style={{ textAlign: 'right' as const }}>
                  <span style={{ fontSize: 20, fontWeight: 800, color: C.navy, display: 'block' }}>{NAIRA}{p.prices[billing].toLocaleString()}</span>
                  <span style={{ fontSize: 11, color: C.gray }}>/{billing === 'quarterly' ? '3mo' : billing === 'biannual' ? '6mo' : 'yr'}</span>
                </div>
              </div>
              <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 14px' }}>
                {p.features.map(f => (
                  <li key={f} style={{ fontSize: 13, color: C.gray, padding: '3px 0', display: 'flex', alignItems: 'center', gap: 8 }}>
                    <CheckCircle2 size={14} color={C.green} /> {f}
                  </li>
                ))}
              </ul>
              <div style={{ background: selectedPlan === p.id ? C.navy : C.bg, borderRadius: 12, padding: 12, textAlign: 'center' as const, fontSize: 14, fontWeight: 700, color: selectedPlan === p.id ? 'white' : C.gray, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                {selectedPlan === p.id && <CheckCircle2 size={14} />} {selectedPlan === p.id ? `${p.name} selected` : `Choose ${p.name}`}
              </div>
            </div>
          ))}
        </div>
        ...
        >
          {processing ? 'Processing...' : `Pay ${NAIRA}${PLANS.find(p => p.id === selectedPlan)!.prices[billing].toLocaleString()} with Paystack`}
        </button>
      </div>
    </div>
  );
    ...
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: 'Inter, sans-serif' }}>
      <div style={{ background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, padding: '24px 24px 40px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 24 }}>
          <div style={{ width: 36, height: 36, background: C.green, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, color: 'white', fontSize: 13 }}>SP</div>
          <span style={{ fontSize: 18, fontWeight: 700, color: 'white' }}>SalesPilot</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center' }}>
          {['Store Setup', 'Choose Plan', "You're Live"].map((label, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', flex: i < 2 ? 1 : 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <div style={{ width: 28, height: 28, borderRadius: '50%', background: i <= step ? C.green : 'rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, color: 'white', fontWeight: 700 }}>{i < step ? <CheckCircle2 size={14} /> : i + 1}</div>
                <span style={{ fontSize: 11, color: i <= step ? 'white' : 'rgba(255,255,255,0.5)', whiteSpace: 'nowrap' as const }}>{label}</span>
              </div>
              {i < 2 && <div style={{ flex: 1, height: 1, background: i < step ? C.green : 'rgba(255,255,255,0.2)', margin: '0 8px' }} />}
            </div>
          ))}
        </div>
      </div>
      <div style={{ padding: 24 }}>
        {step === 0 && (
          <div>
            <h2 style={{ fontSize: 24, color: C.dark, marginBottom: 6, marginTop: 8 }}>Set up your store</h2>
            <p style={{ color: C.gray, marginBottom: 28, fontSize: 14 }}>This is what your customers will see</p>
            <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
              <div><label style={{ fontSize: 13, fontWeight: 600, color: C.dark, display: 'block', marginBottom: 8 }}>Store name</label><input style={inp} name="name" placeholder="e.g. Amaka Boutique" value={store.name} onChange={handle} /></div>
              ...
              <button style={{ background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, color: 'white', border: 'none', borderRadius: 14, padding: 16, fontSize: 16, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }} onClick={() => { if (store.name && store.category && store.phone) setStep(1); }}>Continue <ArrowRight size={16} /></button>
            </div>
          </div>
        )}
        {step === 1 && (
          <div>
            <h2 style={{ fontSize: 24, color: C.dark, marginBottom: 4, marginTop: 8 }}>Start your free trial</h2>
            <p style={{ color: C.gray, marginBottom: 20, fontSize: 14 }}>14 days free - no payment required</p>
            <div style={{ display: 'flex', gap: 6, marginBottom: 20, background: C.bg, borderRadius: 12, padding: 5 }}>
              {[{ id: 'quarterly', label: 'Quarterly' }, { id: 'biannual', label: '6 Months' }, { id: 'annual', label: 'Yearly' }].map(b => (
            ...
            <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16, marginBottom: 24 }}>
              {PLANS.map(p => (
                <div key={p.id} onClick={() => setPlan(p.id)} style={{ background: C.white, border: `2px solid ${plan === p.id ? C.navy : C.border}`, borderRadius: 20, padding: 20, cursor: 'pointer', position: 'relative' as const, boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
                  {p.popular && <div style={{ position: 'absolute' as const, top: -12, left: 20, background: C.navy, color: 'white', fontSize: 11, fontWeight: 700, padding: '4px 14px', borderRadius: 20 }}>Most Popular</div>}
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14 }}>
                    <div><h3 style={{ fontSize: 20, color: C.dark, marginBottom: 4, fontWeight: 700 }}>{p.name}</h3><p style={{ fontSize: 12, color: C.gray }}>{p.desc}</p></div>
                    <div style={{ textAlign: 'right' as const }}>
                      <span style={{ fontSize: 20, fontWeight: 800, color: C.navy, display: 'block' }}>{NAIRA}{p.prices[billing as keyof typeof p.prices].toLocaleString()}</span>
                      <span style={{ fontSize: 11, color: C.gray }}>/{billing === 'quarterly' ? '3mo' : billing === 'biannual' ? '6mo' : 'yr'}</span>
                      <div style={{ background: C.greenLight, color: C.green, fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 20, marginTop: 4 }}>14 days FREE</div>
                    </div>
                  </div>
                  <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 14px' }}>
                    {p.features.map(f => <li key={f} style={{ fontSize: 13, color: C.gray, padding: '3px 0', display: 'flex', alignItems: 'center', gap: 8 }}><CheckCircle2 size={14} color={C.green} /> {f}</li>)}
                  </ul>
                  <div style={{ background: plan === p.id ? C.navy : C.bg, borderRadius: 12, padding: 12, textAlign: 'center' as const, fontSize: 14, fontWeight: 700, color: plan === p.id ? 'white' : C.gray, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>{plan === p.id && <CheckCircle2 size={14} />} {plan === p.id ? `Start ${p.name} Trial Free` : `Try ${p.name} Free`}</div>
                </div>
              ))}
            </div>
            <div style={{ display: 'flex', gap: 12 }}>
              <button style={{ flex: 1, background: C.bg, border: 'none', borderRadius: 14, padding: 16, fontSize: 15, fontWeight: 600, color: C.gray, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }} onClick={() => setStep(0)}><ArrowLeft size={16} /> Back</button>
              <button style={{ flex: 2, background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, color: 'white', border: 'none', borderRadius: 14, padding: 16, fontSize: 15, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }} onClick={async () => { if (plan) { await updateProfile({ storeName: store.name, storeSlug: store.slug, category: store.category, phone: store.phone, plan, billing, onboardingComplete: true, trialStart: new Date().toISOString() }); setStep(2); } }}>Launch my store <ArrowRight size={16} /></button>
            </div>
          </div>
        )}
        {step === 2 && (
          <div style={{ textAlign: 'center' as const, paddingTop: 48 }}>
            ...
            <h2 style={{ fontSize: 26, color: C.dark, marginBottom: 8, fontWeight: 700 }}>Your store is live!</h2>
            <p style={{ color: C.gray, marginBottom: 32, fontSize: 14 }}>14-day free trial has started</p>
            <div style={{ display: 'flex', alignItems: 'center', background: '#F0FFF4', border: `1.5px solid ${C.green}`, borderRadius: 14, padding: '14px 16px', gap: 12, marginBottom: 16 }}>
              <span style={{ flex: 1, color: C.navy, fontSize: 14, textAlign: 'left' as const, fontWeight: 600 }}>{window.location.host}/{store.slug}</span>
              <button style={{ background: C.navy, color: 'white', border: 'none', borderRadius: 8, padding: '8px 14px', fontWeight: 700, cursor: 'pointer', fontSize: 13 }} onClick={() => navigator.clipboard.writeText(`${window.location.host}/${store.slug}`)}>Copy</button>
            </div>
