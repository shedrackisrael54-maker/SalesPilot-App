...
function DashboardScreen({ user, onNavigate }: any) {
  ...
  // money to go. Redirecting to the setup checklist here, instead of letting the store open
  // half-configured, is what makes finishing it feel necessary rather than optional busywork.
  const handleViewStore = () => {
    if (!storeLink) return;
    if (!setupComplete) {
      setViewStoreBlockedMsg("Almost there! Finish setting up your store first.");
      setMoreView('complete-store');
      return;
    }
...
function Dashboard() {
  ...
  if (moreView === 'complete-store') {
    return <CompleteStoreScreen onBack={() => { setMoreView(null); setViewStoreBlockedMsg(''); }} incomingMsg={viewStoreBlockedMsg} />;
  }
  if (moreView === 'reports') {
