...
import type { StoreTheme, ThemeNiche, DescriptionSection, OptionGroup, Product, Review, Order, Bundle, Courier } from './shared';
...
function CouriersScreen({ onBack }: { onBack: () => void }) {
  ...
  const handleSave = async () => {
    ...
    for (const c of couriers) {
      ...
      if (c.customerPrice < 0 || c.costPrice < 0) { setError('Prices cannot be negative.'); return; }
    }
    setSaving(true);
    try {
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100vh', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ background: C.white, borderRadius: '24px 24px 0 0', marginTop: -16, padding: 20, paddingBottom: 100 }}>
        <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
          <p style={{ fontSize: 13, color: C.gray, lineHeight: 1.6 }}>
            Add the couriers you actually use. Customers pick one at checkout, seeing only your
            customer price - your cost price is just for your own records, so you can track your
            margin on delivery. If you don't add any couriers here, checkout falls back to your
            Shipping Rates by state instead.
          </p>
          ...
          {couriers.map((c) => (
            <div key={c.id} style={{ border: `1.5px solid ${C.border}`, borderRadius: 14, padding: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
                <div style={{ flex: 1 }}>
                  <label style={{ fontSize: 11.5, color: C.gray, display: 'block', marginBottom: 4 }}>Your cost</label>
                  <input style={inp} type="number" placeholder="0" value={c.costPrice || ''} onChange={e => updateCourier(c.id, { costPrice: Number(e.target.value) })} />
                </div>
                <div style={{ flex: 1 }}>
                  <label style={{ fontSize: 11.5, color: C.gray, display: 'block', marginBottom: 4 }}>Customer pays</label>
              ...
              />
              {c.customerPrice > c.costPrice && (
                <p style={{ fontSize: 11.5, color: C.green, marginTop: 8, fontWeight: 600 }}>
                  You earn {NAIRA}{(c.customerPrice - c.costPrice).toLocaleString()} margin per delivery with this courier.
                </p>
              )}
            </div>
          ))}
