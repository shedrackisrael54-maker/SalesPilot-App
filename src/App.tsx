...
const ONBOARDING_PRIORITIES = [
  ...
  'Track my inventory automatically',
  'Sell in person with POS',
  'Send invoices to customers',
  'Understand my sales and profit',
  'Add staff to help me',
  'Run marketing (coupons, email)',
];

// Suggests which plan best fits what the merchant said matters to them, and a personalised
// first action for their "You're Live" screen - mirrors the real, proven Bumpa onboarding
// pattern of asking priorities up front, then tailoring the plan recommendation and the very
// next step around those answers, instead of showing every new merchant the exact same thing.
function getRecommendedPlan(priorities: string[]): 'basic' | 'pro' {
  const proSignals = ['Add staff to help me', 'Run marketing (coupons, email)', 'Understand my sales and profit'];
  return priorities.some(p => proSignals.includes(p)) ? 'pro' : 'basic';
}

function getFirstAction(priorities: string[]): string {
  const map: { [key: string]: string } = {
    'Sell products online': 'Add your first product, then share your store link',
    'Track my inventory automatically': 'Add your first product and set a real stock quantity',
    'Sell in person with POS': 'Try recording your first sale under POS',
    'Send invoices to customers': 'Create your first Invoice',
    'Understand my sales and profit': 'Add a sale, then check Analytics to see it reflected',
    'Add staff to help me': 'Add your first team member under More, then Staff Accounts',
    'Run marketing (coupons, email)': 'Set up your first coupon under More',
  };
  return map[priorities[0]] || 'Add your first product to get started';
}
...
function Onboarding() {
  ...
  const inp = { width: '100%', padding: '14px 16px', borderRadius: 12, border: `1.5px solid ${C.border}`, fontSize: 15, outline: 'none', boxSizing: 'border-box' as const, background: C.white, color: C.dark };
  const recommendedPlan = getRecommendedPlan(priorities);
  const togglePriority = (p: string) => {
    if (priorities.includes(p)) { setPriorities(priorities.filter(x => x !== p)); return; }
    ...
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: 'Inter, sans-serif' }}>
      <div style={{ background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, padding: '24px 24px 40px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center' }}>
          {['Priorities', 'Store Setup', 'Choose Plan', "You're Live"].map((label, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', flex: i < 3 ? 1 : 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <div style={{ width: 26, height: 26, borderRadius: '50%', background: i <= step ? C.green : 'rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: 'white', fontWeight: 700, flexShrink: 0 }}>{i < step ? <CheckCircle2 size={13} /> : i + 1}</div>
                <span style={{ fontSize: 10, color: i <= step ? 'white' : 'rgba(255,255,255,0.5)', whiteSpace: 'nowrap' as const }}>{label}</span>
              </div>
              {i < 3 && <div style={{ flex: 1, height: 1, background: i < step ? C.green : 'rgba(255,255,255,0.2)', margin: '0 6px' }} />}
            </div>
          ))}
        </div>
      </div>
      <div style={{ padding: 24 }}>
        {step === 0 && (
          <div>
            <h2 style={{ fontSize: 24, color: C.dark, marginBottom: 6, marginTop: 8 }}>What matters most to you?</h2>
            <p style={{ color: C.gray, marginBottom: 24, fontSize: 14 }}>Pick up to 3. This helps us show you the right plan and the right first step.</p>
            <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 10, marginBottom: 28 }}>
              {ONBOARDING_PRIORITIES.map(p => {
                ...
                return (
                  <button key={p} onClick={() => togglePriority(p)} style={{ display: 'flex', alignItems: 'center', gap: 12, textAlign: 'left' as const, background: selected ? `${C.navy}0d` : C.white, border: `2px solid ${selected ? C.navy : C.border}`, borderRadius: 14, padding: '14px 16px', cursor: 'pointer' }}>
                    <div style={{ width: 20, height: 20, borderRadius: 6, border: `2px solid ${selected ? C.navy : C.border}`, background: selected ? C.navy : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      {selected && <CheckCircle2 size={13} color="white" />}
                    </div>
                    <span style={{ fontSize: 14, fontWeight: 600, color: C.dark }}>{p}</span>
                  </button>
                );
              })}
            </div>
            <button
              ...
              style={{ width: '100%', background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, color: 'white', border: 'none', borderRadius: 14, padding: 16, fontSize: 16, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, opacity: priorities.length === 0 ? 0.5 : 1 }}
              onClick={() => setStep(1)}
            >
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
              ...
              <div style={{ display: 'flex', gap: 12 }}>
                ...
                <button style={{ flex: 1, background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, color: 'white', border: 'none', borderRadius: 14, padding: 16, fontSize: 16, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }} onClick={() => { if (store.name && store.category && store.phone) { setPlanTab(recommendedPlan); setStep(2); } }}>Continue <ArrowRight size={16} /></button>
              </div>
            </div>
          </div>
        )}
        {step === 2 && (
          <div>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              ...
              <button style={{ flex: 1, background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, color: 'white', border: 'none', borderRadius: 14, padding: 16, fontSize: 15, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }} onClick={async () => { setPlan(planTab); await updateProfile({ storeName: store.name, storeSlug: store.slug, category: store.category, phone: store.phone, plan: planTab, billing, priorities, onboardingComplete: true, trialStart: new Date().toISOString() }); setStep(3); }}>Start free trial <ArrowRight size={16} /></button>
            </div>
          </div>
        )}
        {step === 3 && (
          <div style={{ textAlign: 'center' as const, paddingTop: 40 }}>
            ...
            <p style={{ color: C.gray, marginBottom: 28, fontSize: 14 }}>Your 14 day free trial has started</p>
            ...
            {priorities.length > 0 && (
              <div style={{ background: C.white, border: `1.5px solid ${C.border}`, borderRadius: 16, padding: 18, marginBottom: 20, textAlign: 'left' as const }}>
                <p style={{ fontSize: 12, fontWeight: 800, color: C.navy, textTransform: 'uppercase' as const, letterSpacing: 0.5, marginBottom: 6 }}>Your first step</p>
                <p style={{ fontSize: 14, fontWeight: 600, color: C.dark, lineHeight: 1.5 }}>{getFirstAction(priorities)}</p>
              </div>
            )}
