...
const DASHBOARD_INFO_CARDS: DashboardInfoCard[] = [
  ...
  },
  {
    ...
    content: [
      ...
      { text: 'Barcode scanning. Add barcodes to your products, then scan instead of typing during checkout on POS. Faster sales, fewer mistakes, especially useful if you sell items with a lot of similar names or variants.' },
      { text: 'Delivery couriers. Set up the couriers you actually use with your own delivery price. Customers pick one at checkout, and you can see exactly who they chose on the order, so there is no back and forth figuring out how to get their order to them.' },
      { text: 'Custom checkout questions. Need to ask something before an order goes through? Gift message, delivery instructions, how they heard about you? Add your own questions, mark the important ones as required, and the answers show up right on the order.' },
      { text: 'Platform fee choice. You decide who covers SalesPilot small transaction fee, you or your customer. Either way, it is clearly worked out for you so there is never a surprise at payout time.' },
      { text: 'Terms and conditions. Set your own store policy on refunds, delivery, and disputes. Customers agree to it before checking out, and it is saved exactly as they saw it at that moment, real protection if a disagreement ever comes up later.' },
...
function DashboardInfoCardFace({ card, onSelect }: { card: DashboardInfoCard; onSelect: (card: DashboardInfoCard) => void }) {
  ...
  return (
    <button
      ...
      style={{ flex: '0 0 78%', scrollSnapAlign: 'start' as const, background: C.white, borderRadius: 16, padding: 16, textAlign: 'left' as const, border: `1px solid ${C.border}`, boxShadow: '0 2px 10px rgba(0,0,0,0.05)', cursor: 'pointer' }}
    >
      <div style={{ width: 38, height: 38, borderRadius: 10, background: C.bg, color: C.navy, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 10 }}>
        <Icon size={19} color={C.navy} />
      </div>
      <p style={{ fontSize: 15, fontWeight: 700, color: C.dark, marginBottom: 4 }}>{card.teaserTitle}</p>
      <p style={{ fontSize: 12.5, color: C.gray, marginBottom: 10, lineHeight: 1.4 }}>{card.teaserSubtitle}</p>
      <p style={{ fontSize: 12, fontWeight: 700, color: C.green }}>{card.teaserCta}</p>
    </button>
  );
}
...
function DashboardInfoModal({ card, onClose }: { card: DashboardInfoCard | null; onClose: () => void }) {
  if (!card) return null;
  return (
    <div onClick={onClose} style={{ position: 'fixed' as const, inset: 0, background: 'rgba(20,42,69,0.55)', zIndex: 100, display: 'flex', alignItems: 'flex-end' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: C.white, width: '100%', maxHeight: '85vh', borderRadius: '20px 20px 0 0', overflowY: 'auto' as const, padding: 20 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <h3 style={{ fontSize: 18, fontWeight: 700, color: C.dark }}>{card.fullTitle}</h3>
          <button onClick={onClose} style={{ background: C.bg, border: 'none', borderRadius: 10, width: 34, height: 34, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
            <X size={17} color={C.dark} />
          </button>
        </div>
        {card.content.map((block, i) => (
          <div key={i} style={{ marginBottom: 13 }}>
            {block.heading && (
              <p style={{ fontSize: 12, fontWeight: 800, color: C.green, textTransform: 'uppercase' as const, letterSpacing: 0.5, marginBottom: 6, marginTop: i > 0 ? 6 : 0 }}>{block.heading}</p>
            )}
            <p style={{ fontSize: 14, color: C.dark, lineHeight: 1.6 }}>{block.text}</p>
          </div>
        ))}
      </div>
    </div>
