...
function SplashScreen({ fadingOut }: { fadingOut: boolean }) {
  return (
    <div style={{ position: 'fixed' as const, inset: 0, background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, display: 'flex', flexDirection: 'column' as const, alignItems: 'center', justifyContent: 'center', zIndex: 999, opacity: fadingOut ? 0 : 1, transition: 'opacity 0.4s ease' }}>
      <div style={{ display: 'flex', flexDirection: 'column' as const, alignItems: 'center', gap: 16, animation: 'sp-splash-in 0.7s ease forwards' }}>
        <AppLogo size={84} />
        <span style={{ fontSize: 26, fontWeight: 800, color: 'white', letterSpacing: 0.3 }}>SalesPilot</span>
      </div>
      <style>{`@keyframes sp-splash-in { from { opacity: 0; transform: scale(0.85) translateY(8px); } to { opacity: 1; transform: scale(1) translateY(0); } }`}</style>
    </div>
  );
}
...
// time, never just decoration sitting in front of more loading underneath it. Only ever shown
// once per device, the very first time someone opens the app - checked via localStorage, which
// persists across visits (unlike sessionStorage, which would show it again every new tab).
function SplashGate() {
  const { authReady } = useAuth();
  const [minTimeElapsed, setMinTimeElapsed] = useState(false);
  const [fadingOut, setFadingOut] = useState(false);
  const [showSplash, setShowSplash] = useState(() => {
    try { return !localStorage.getItem('sp_splash_seen'); } catch { return true; }
  });
  ...
  useEffect(() => {
    if (minTimeElapsed && authReady && !fadingOut) {
      setFadingOut(true);
      try { localStorage.setItem('sp_splash_seen', '1'); } catch {}
