...
import { NAIRA, THEME_NICHES, PRODUCT_CATEGORIES, PRODUCT_ICON_MAP, PRODUCT_ICON_KEYS, ProductIcon, compressImage, uploadToCloudinary, isValidCSSColor, isColorOptionGroup, NIGERIAN_STATES, PAYSTACK_PUBLIC_KEY, PLATFORM_LOGO_URL, getOptimizedImageUrl, SUPPORT_EMAIL } from './shared';
...
function HelpSupportScreen({ onBack, onNavigate }: { onBack: () => void; onNavigate: (action: string) => void }) {
  const faqs = [
    { q: 'How do I get paid?', a: 'Paystack settles payments to the platform account, and payouts to merchants are handled directly. If you have questions about a specific payout, contact support.' },
...
function CompleteStoreScreen({ onBack }: { onBack: () => void }) {
  ...
  const verifyAccount = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setResolveError(data.error || 'Could not verify this account.');
        return;
      }
  ...
  // Once the account is verified, creates (or updates) a real Paystack Subaccount for this
  // merchant - from this point on, checkout can automatically split every payment so the
  // merchant's share settles straight to their own bank account, no manual payout needed.
  const handleSaveBankDetails = async () => {
    ...
    setSavingBank(true);
    try {
      const res = await fetch('https://sales-pilot-payment.vercel.app/api/create-subaccount', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          businessName: user?.storeName || resolvedName,
          bankCode,
          accountNumber,
      ...
      const data = await res.json();
      if (!res.ok) {
        setError(data.error || 'Could not set up automatic payouts. Please try again.');
        return;
      }
      await updateProfile({
        bankName, bankCode, accountNumber, accountName: resolvedName,
    ...
    return (
      <ChecklistItemScreen title="Bank details" onBack={() => setActiveItem(null)} saving={savingBank} error={error}
        ...
        <p style={{ fontSize: 12, color: C.gray }}>
          Once verified and saved, payments from your store will automatically split, your share
          settles directly to this account, no manual payout needed.
        </p>
      </ChecklistItemScreen>
    );
  }
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100vh', paddingBottom: 80, fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ background: C.white, borderRadius: '24px 24px 0 0', marginTop: -20, padding: 20 }}>
        ...
        <button onClick={() => onNavigate?.('subscription')} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 14, background: `linear-gradient(135deg, ${C.navy}, ${C.navyLight})`, border: 'none', borderRadius: 16, padding: 18, cursor: 'pointer', marginBottom: 24 }}>
          <div style={{ width: 46, height: 46, background: 'rgba(255,255,255,0.15)', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <Rocket size={21} color="white" />
          </div>
          <div style={{ flex: 1, textAlign: 'left' as const }}>
            <p style={{ fontSize: 15, fontWeight: 700, color: 'white', marginBottom: 2 }}>Upgrade your account</p>
            <p style={{ fontSize: 12.5, color: 'rgba(255,255,255,0.75)' }}>{subscriptionLabel}</p>
          </div>
          <ChevronRight size={18} color="white" />
        </button>

        {MORE_MENU_GROUPS.map(group => {
          const visibleItems = group.items.filter(item => canAccessMoreAction(user, item.action));
...
function Signup() {
  ...
  const sendVerificationCode = async () => {
    ...
    try {
      ...
      setError(`Could not save your verification code (database step failed): ${err?.message || err?.code || 'unknown error'}`);
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(`Could not send the email (server responded with ${res.status}): ${detail || 'no further detail returned'}`);
        setSendingCode(false);
        return;
      }
      setStep('verify');
      ...
      setError(`Could not reach the email server (network step failed): ${err?.message || 'unknown error'}`);
    } finally {
      setSendingCode(false);
    }
  };
...
const PLANS = [
  { id: 'basic', name: 'Starter', desc: 'Everything you need to run your store', prices: { quarterly: 10000, biannual: 18000, annual: 30000 }, features: ['Unlimited product listings', 'Online storefront with secure checkout', 'Automatic inventory tracking, online and offline', 'Point of Sale and branded invoices', 'Coupons, bundles, and tiered pricing', 'Real analytics on your sales, profit, and customers', 'Customer reviews and CRM', 'Order tracking for your customers', 'Cart timer and sale countdown tools', 'One brand theme per store category'], popular: false },
  { id: 'pro', name: 'Growth', desc: 'Built for growing businesses', prices: { quarterly: 20000, biannual: 40000, annual: 80000 }, features: ['Everything included in Starter', 'Staff accounts with role based permissions', 'Email marketing campaigns', 'Customer segmentation and groups', 'Facebook Pixel and Google Analytics integration', 'Full access to all 32 brand themes', 'Free custom domain with annual billing', 'Priority customer support'], popular: true },
];
