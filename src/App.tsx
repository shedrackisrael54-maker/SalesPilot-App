...
function DashboardScreen({ user, onNavigate }: any) {
  ...
  const storeLink = user?.storeSlug ? `https://salespilot.store/${user.storeSlug}` : null;
...
function ProductForm({ product, onSave, onCancel, externalError }: { product: Product | null; onSave: (p: Product) => void; onCancel: () => void; externalError?: string }) {
  ...
  const shareLink = product && user?.storeSlug
    ? `https://salespilot.store/${user.storeSlug}/product/${product.id}`
    : null;
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              {user?.storeSlug && (
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    const link = `https://salespilot.store/${user.storeSlug}/product/${product.id}`;
...
function MoreScreen({ logout, user, onNavigate }: any) {
  ...
  const handleItemClick = (action: string) => {
    if (action === 'share-store') {
      ...
      const link = `https://salespilot.store/${user.storeSlug}`;
      navigator.clipboard.writeText(link);
      if (navigator.share) {
...
const PLANS = [
  { id: 'basic', name: 'Basic', desc: 'Perfect for getting started', prices: { quarterly: 10000, biannual: 18000, annual: 30000 }, features: ['Storefront + real checkout (Paystack or WhatsApp)', 'Automatic inventory tracking, online & offline', 'POS & branded invoices', 'Coupons', 'Customer reviews', 'Up to 20 products', 'SalesPilot branding'], popular: false },
  { id: 'pro', name: 'Pro', desc: 'For sellers ready to grow', prices: { quarterly: 20000, biannual: 40000, annual: 80000 }, features: ['Everything in Basic', 'Unlimited products', 'No SalesPilot branding', 'Free custom domain (annual billing only)', 'Product Bundles', 'Wholesale/tiered pricing', 'Min & Max order quantities', 'Order tracking for customers', 'Cart Timer & Sale Countdown', 'Analytics (best sellers, profit, customers)', 'Facebook Pixel / Google Analytics', 'Staff accounts', 'Priority support'], popular: true },
];
            ...
            <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
              ...
              <div><label style={{ fontSize: 13, fontWeight: 600, color: C.dark, display: 'block', marginBottom: 8 }}>Store URL</label>
                <div style={{ display: 'flex', border: `1.5px solid ${C.border}`, borderRadius: 12, overflow: 'hidden' }}>
                  <span style={{ padding: '14px 10px', fontSize: 12, color: C.gray, background: C.bg, borderRight: `1px solid ${C.border}`, whiteSpace: 'nowrap' as const }}>salespilot.store/</span>
        ...
        {step === 2 && (
          <div style={{ textAlign: 'center' as const, paddingTop: 48 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', background: '#F0FFF4', border: `1.5px solid ${C.green}`, borderRadius: 14, padding: '14px 16px', gap: 12, marginBottom: 16 }}>
              <span style={{ flex: 1, color: C.navy, fontSize: 14, textAlign: 'left' as const, fontWeight: 600 }}>salespilot.store/{store.slug}</span>
              <button style={{ background: C.navy, color: 'white', border: 'none', borderRadius: 8, padding: '8px 14px', fontWeight: 700, cursor: 'pointer', fontSize: 13 }} onClick={() => navigator.clipboard.writeText(`salespilot.store/${store.slug}`)}>Copy</button>
            </div>
