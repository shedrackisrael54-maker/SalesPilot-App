...
function DashboardScreen({ user, onNavigate }: any) {
  ...
  // Someone who skips straight to viewing their storefront right after signup can easily never
  // come back to add their bank details - which means real customer orders with nowhere for the
  // money to go. This delegates to the onNavigate prop rather than handling the block itself,
  // since the moreView/message state that actually shows the redirect and its reason lives one
  // level up, in Dashboard() - this component only owns its own tab content, not that state.
  const handleViewStore = () => {
    if (!storeLink) return;
    if (!setupComplete) { onNavigate('store'); return; }
...
function CompleteStoreScreen({ onBack, incomingMsg }: { onBack: () => void; incomingMsg?: string }) {
  const { user, updateProfile } = useAuth();
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100vh', paddingBottom: 80, fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ background: C.white, borderRadius: '24px 24px 0 0', marginTop: -16, padding: 20, minHeight: 'calc(100vh - 90px)' }}>
        {incomingMsg && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: C.orangeLight, borderRadius: 12, padding: '12px 14px', marginBottom: 16 }}>
            <AlertCircle size={17} color={C.orange} style={{ flexShrink: 0 }} />
            <p style={{ fontSize: 13, fontWeight: 600, color: '#92400E' }}>{incomingMsg}</p>
          </div>
        )}
...
function MoreScreen({ logout, user, onNavigate }: any) {
  ...
  // Mirrors the same check Dashboard uses to gate View Store - a merchant who taps "My Store"
  // from this menu before finishing setup gets the same redirect, not a second, inconsistent
  // path that lets them slip through here instead.
  const moreSetupComplete = !!(
    user?.bankName && user?.accountNumber && user?.accountName &&
    user?.state && user?.zipcode &&
    user?.contactWhatsapp && user?.contactEmail &&
    user?.storeDescription && user?.logoUrl
  );
  ...
  const handleItemClick = (action: string) => {
    if (action === 'store') {
      ...
      if (!moreSetupComplete) { onNavigate('complete-store'); return; }
...
function Dashboard() {
  ...
  // Explains WHY a merchant was redirected here instead of to their actual storefront, when
  // setup isn't finished yet. Lives in this component specifically because this is the one that
  // owns moreView and genuinely renders CompleteStoreScreen - a message set anywhere else has
  // no way to reach this render.
  const [viewStoreBlockedMsg, setViewStoreBlockedMsg] = useState('');
  ...
  const handleDashboardNavigate = (action: string) => {
    ...
    if (action === 'store') {
      if (!user?.storeSlug) return;
      if (!setupComplete) { setViewStoreBlockedMsg("Almost there! Finish setting up your store first."); setMoreView('complete-store'); return; }
      window.open(`${window.location.origin}/${user.storeSlug}`, '_blank');
      return;
    }
    if (action === 'share-store') {
  ...
  if (moreView === 'complete-store') {
    return <CompleteStoreScreen onBack={() => { setMoreView(null); setViewStoreBlockedMsg(''); }} incomingMsg={!setupComplete ? (viewStoreBlockedMsg || "Almost there! Finish setting up your store first.") : ''} />;
  }
  if (moreView === 'reports') {
